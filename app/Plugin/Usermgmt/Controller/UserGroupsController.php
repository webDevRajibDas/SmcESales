	...
	public function addGroup() {
		if ($this->request -> isPost()) {
			...
			if ($this->UserGroup->addValidate()) {
				...
				$this->Session->setFlash(__('The group is successfully added'), 'flash/success');
	...
	public function editGroup($groupId=null) {
		...
		if (!empty($groupId)) {
			if ($this->request -> isPut()) {
				...
				if ($this->UserGroup->addValidate()) {
					$this->UserGroup->save($this->request->data,false);
					$this->Session->setFlash(__('The group is successfully updated'), 'flash/success');
					$this->redirect('/admin/allGroups');
				}
			} else {
		...
		if (!empty($groupId)) {
			if ($this->request -> isPost()) {
				if ($this->UserGroup->delete($groupId, false)) {
					$this->Session->setFlash(__('The group is successfully deleted'), 'flash/success');
				}
			}
			$this->redirect('/admin/allGroups');
		} else {
