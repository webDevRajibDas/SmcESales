 ...
 */
class ApiDataDist118RetrivesController extends AppController 
{
    ...
    {
    	$restricted_office=array(16,18,25,15,19,26,27,28,29,22,23,24);
						...
						else
						{
							...
							if ($this->DistMemo->save($MemoData)) 
							{
								...
								if ($memo_id) 
								{
									...
									$this->DistMemoDetail->saveAll($total_product_data);
									$res['status'] = 1;
									$res['message'] = 'Memos has been updated successfuly.';
								}
							}
						}
					}
	...
	public function dist_order_delivery_schedule(){
		...
		foreach($order_results as $key => $order_info)
		{
			...
			if($order_info['DistOrder']['status']==2 && $order_info['DistOrder']['processing_status']==1)
			{
	...
	public function dist_order_delivery_unprocess(){
		...
		$order_results = $this->DistOrderDeliveryScheduleOrderDetail->find('all', array(
			'conditions' => array(
				'DistOrderDeliveryScheduleOrderDetail.dist_order_no' => $json_data['order_numbers'],
			),
			'joins' => array(
				array(
					'alias' => 'DistOrder',
		...
		foreach($order_results as $key => $order_info)
		{
			...
			$dist_order_no = $order_info['DistOrderDeliveryScheduleOrderDetail']['dist_order_no'];
