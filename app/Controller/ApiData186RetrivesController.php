 ...
 */
class ApiData186RetrivesController extends AppController
{
    ...
    public function get_stamp_target_info_bk() // temporary backup for immidiet support for multi product card
    {
        $this->loadModel('StoreBonusCard');
        $this->loadModel('BonusCard');
        $json_data = $this->request->input('json_decode', true);
    ...
    public function get_stamp_target_info()
    {
        ...
        $territory_id = $json_data['territory_id'];
        $conditions = array(
            'Memo.memo_date BETWEEN ? and ? ' => array('2022-07-01', '2023-06-30'),
            'Memo.gross_value >=' => 0,
            'Memo.status !=' => 0,
        ...
        $target_list = $this->Memo->find('all', array(
            'conditions' => $conditions,
            'joins' => array(
                array(
                    'table' => 'memo_details',
            ...
            'group' => array(
                ...
                'Memo.id HAVING SUM(MemoDetail.sales_qty) >=10'
            ),
            'fields' => array(
                'Market.territory_id  as territory_id',
                'Market.id as market_id',
                'Memo.outlet_id  as outlet_id',
                'Outlet.bonus_type_id as bonus_type_id',
                'Memo.id',
                'SUM(MemoDetail.sales_qty) as achieve_quantity',
            ),
            'recursive' => -1
        ));
        /* pr($target_list);
        ...
        exit; */
        /* getting target quantity */
        /* $bonus_card_info_big = $this->BonusCard->find('first', array(
            'conditions' => array('BonusCard.id' => 18),
            'recursive' => -1
        ));
        $bonus_card_info_small = $this->BonusCard->find('first', array(
            'conditions' => array('BonusCard.id' => 17),
            'recursive' => -1
        )); */
        ...
        foreach ($target_list as $each_tl) {
            ...
            $data['outlet_id'] = $each_tl['outlet_id'];
            $data['product_id'] = 47;
            $data['fiscal_year_id'] = 24;
            $data['bonus_party_id'] = $each_tl['bonus_type_id'];
            $data['achieve_quantity'] = $each_tl['achieve_quantity'] + (isset($data_array[$each_tl['outlet_id']]['product_bonus_target']['achieve_quantity']) ? $data_array[$each_tl['outlet_id']]['product_bonus_target']['achieve_quantity'] : 0);

            // $stamp_calculation = (floor($each_tl['achieve_quantity'] / $each_tl['bonus_type_id'] == 1 ? $bonus_card_info_small['BonusCard']['min_qty_per_memo'] : $bonus_card_info_big['BonusCard']['min_qty_per_memo']));
            $stamp_calculation = (floor($each_tl['achieve_quantity'] / ($each_tl['bonus_type_id'] == 1 ? 10 : 20)));


            $data['stamp'] = $stamp_calculation + (isset($data_array[$each_tl['outlet_id']]['product_bonus_target']['stamp']) ? $data_array[$each_tl['outlet_id']]['product_bonus_target']['stamp'] : 0);

            $data['target_quantity'] = ($each_tl['bonus_type_id'] == 1 ? 200 : 500);
            $data_array[$each_tl['outlet_id']]['product_bonus_target'] = $data;
        }
        /*  pr(array_values($data_array));
        exit; */
        $this->LoadModel('FiscalYear');
        $start_date_fiscal_year = $this->FiscalYear->query("SELECT fy_v.start_date
                FROM app_versions app_v
        ...
        $this->set(array(
            'bonus_target' => array_values($data_array),
            'fiscal_years' => $fiscal_year_array,
            '_serialize' => array('bonus_target', 'fiscal_years')
        ));
    }
    ...
    public function get_policy_list_v2()
    {
        ...
        $territory_info = $this->Territory->find(
            ...
            array(
                ...
                'fields' => array('Office.id', 'User.user_group_id', 'User.id', 'Territory.parent_id'),
                'recursive' => 0
            )
        );
        ...
        if ($territory_info['Territory']['parent_id'] > 0) {
            $territory_id = $territory_info['Territory']['parent_id'];
        }
