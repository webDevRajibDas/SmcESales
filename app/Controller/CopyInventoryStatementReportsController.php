 ...
 */
class CopyInventoryStatementReportsController extends AppController
{
    ...
    public function admin_index($id = null)
    {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($reports_type == 1) {
                //$this->dd('reports_type 1'); exit;
                //For Product List Condition Update
                if ($source) {
                ...
                $aso_stock_opening_results = $this->Store->find('all', array(
                    ...
                    'fields' => array(
                        'sum(RptDailyTranBalance.opening_balance) AS opening_balance,Product.id as product_id'
                    ),
                    'group' => array(
                        'Product.id'
                    ),
                ...
                $aso_stock_closing_results = $this->Store->find('all', array(
                    ...
                    'fields' => array(
                        'sum(RptDailyTranBalance.closing_balance) AS closing_balance,Product.id as product_id'
                    ),
                    'group' => array(
                        'Product.id'
                    ),
                ...
                //$this->dd($aso_stock_closing_results);exit;
                ...
                //pr($aso_final_opening_closing);exit;
                ...
                if ($is_day_wise_closing == 1) {
                    ...
                    $aso_day_stock_closing_results = $this->Store->find('all', array(
                        ...
                        'fields' => array('sum(RptDailyTranBalance.closing_balance) AS closing_balance,Product.id as product_id', 'RptDailyTranBalance.tran_date'),
                        'group' => array('RptDailyTranBalance.tran_date', 'Product.id as product_id'),
                        'recursive' => -1,
                    ));
                ...
                $so_stock_opening_results = $this->Store->find('all', array(
                    ...
                    'fields' => array('sum(RptDailyTranBalance.opening_balance) AS opening_balance,Product.id as product_id'),
                    'group' => array('Product.id'),
                    'recursive' => -1,
                ));
                
                //$this->dd($so_stock_opening_results);
                ...
                $so_stock_closing_results = $this->Store->find('all', array(
                    ...
                    'fields' => array('sum(RptDailyTranBalance.closing_balance) AS closing_balance, Product.id as product_id'),
                    'group' => array('Product.id'),
                    'recursive' => -1,
                ));
                ...
                if ($is_day_wise_closing == 1) {
                    ...
                    $so_day_wise_closing_results = $this->Store->find('all', array(
                        ...
                        'fields' => array('sum(RptDailyTranBalance.closing_balance) AS closing_balance,Product.id as product_id', 'RptDailyTranBalance.tran_date'),
                        'group' => array('RptDailyTranBalance.tran_date', 'Product.id'),
                        'recursive' => -1,
                    ));
...
//Test
                    $o_received_query_results = $this->Store->find('all', array(
                        ...
                        'fields' => array('sum(InventoryAdjustmentDetail.quantity) AS qty ,CurrentInventory.product_id'),
                        
                        'group' => array('CurrentInventory.product_id'),
                        'recursive' => -1,
                    ));
                    
                   //$this->dd($o_received_query_results);
                    //echo $this->Store->getLastQuery();exit;
                ...
                if ($so_id || $territory_id) {
					...
					echo "2";
					$this->dd($issue_query_results);exit;
                    /*echo $this->Store->getLastQuery();
                    ...
                    $o_issue_query_results = $this->Store->find('all', array(
                        ...
                        'fields' => array('sum(InventoryAdjustmentDetail.quantity) AS qty, CurrentInventory.product_id', 'InventoryAdjustment.transaction_type_id'),
                        
                        'group' => array('CurrentInventory.product_id', 'InventoryAdjustment.transaction_type_id'),
                        'recursive' => -1,
                    ));
                    //echo $this->Store->getLastQuery();
                    //$this->dd($o_issue_query_results);
                    //exit;
...
//                $newCondition = array(
//                    'is_active' => 1,
//                    //'CASE WHEN Product.is_virtual = 0 THEN Product.is_virtual = 1 END'
//
//                );
                $product_results = $this->Product->find('all', array(
                    ...
                    'fields' => array('Product.id', 'Product.name', 'Product.product_type_id', 'ProductType.name', 'Product.source','Product.is_virtual'),
                    'recursive' => -1,
                ...
                //$this->dd($product_list);
                
            } elseif ($reports_type == 2) {
                //$this->dd('$reports_type == 2');exit();
        ...
        $this->set(compact('offices', 'territories','region_offices', 'office_id', 'request_data', 'so_list', 'product_list'));
    }
}
