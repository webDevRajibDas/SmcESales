 ...
 */
class ApiDataDist122RetrivesController extends AppController
{
	...
	{	//15,16,18,19,22,23,24,25,26,27,28,29,44
		...
		/* else if (date('H:i') >= "00:00" && date('H:i') < "06:30")
				...
				if ($count == 0) {
					...
					if ($this->DistOrder->save($OrderData)) {
						...
						if ($order_id) {
							...
							if (date('Y-m-d', strtotime($result['order_date'])) >=  '2022-11-01' && date('Y-m-d', strtotime($result['order_date'])) <=  '2022-11-30' && $result['gross_value'] >= 1000) {
								$update_memo = $this->order_discount_update($result['order_number']);
							}
							$res['status'] = 1;
		...
		if (!empty($json_data['memo_list'])) {
			...
			foreach ($json_data['memo_list'] as $result) {
				...
				$order_detials = $this->DistOrderDetail->find('all', array(
					...
					'fields' => array('DistOrderDetail.dist_order_id as dist_order_id', 'DistOrderDetail.product_id as product_id', 'DistOrderDetail.measurement_unit_id as measurement_unit_id', 'DistOrderDetail.sales_qty as sales_qty', 'DistOrderDetail.price as price', 'DistOrderDetail.is_bonus as is_bonus', 'DistOrderDetail.product_price_id as product_price_id', 'DistOrder.order_date'),
					'recursive' => -1
				));
				...
				foreach ($order_detials as $order_detial) {
					...
					$s_order_detials = $this->DistOrderDeliveryScheduleOrderDetail->find('first', array(
						'conditions' => array(
							...
							/* 'DistOrderDeliveryScheduleOrderDetail.is_bonus' => 0, */
						...
						if ($count == 0) {
							...
							if ($this->DistMemo->save($MemoData)) {
								...
								if ($memo_id) {
									...
									if (date('Y-m-d', strtotime($result['memo_date'])) >=  '2022-11-01' && date('Y-m-d', strtotime($result['memo_date'])) <=  '2022-11-30' && $result['gross_value'] >= 1000) {
										$update_memo = $this->memo_discount_update($result['memo_number']);
									}
								}
	...
	public function dist_order_ready_for_process_list()
	{
		...
		if ($order_date && ($order_date >= date('Y-m-d', strtotime('-10days')) || $json_data['sales_person_id'] == 21486)) {
			$conditions['DistOrder.order_date'] = $order_date;
		} else {
			if ($json_data['sales_person_id'] != 21486)
			...
			try {
				...
				$order_detail_results = $this->DistOrderDetail->find('all', array(
					...
					'fields' => array(
						...
						'SUM(DistCurrentInventory.qty) as qty'
					),
					'group' => array(
						'Product.base_measurement_unit_id',
						'Product.sales_measurement_unit_id',
						'DistOrderDetail.product_id HAVING SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base  is null then 1 else ProductMeasurement.qty_in_base END),0)) > CASE WHEN SUM(DistCurrentInventory.qty) is null THEN 0 ELSE SUM(DistCurrentInventory.qty) END',
					),
					'recursive' => -1
				));
				/* echo $this->DistOrder->getLastQuery();
				...
				$sql = "
					...
					select
						...
						ROUND(CASE WHEN SUM(DistCurrentInventory.qty) >= SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0)) 
							THEN 
							...
							ELSE
								CASE WHEN SUM(DistCurrentInventory.qty) is null THEN 0 ELSE SUM(DistCurrentInventory.qty) END
						...
						0 as is_bonus,
						CASE WHEN SUM(DistCurrentInventory.qty) >= SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0)) 
							THEN 
								1
							ELSE
								0
					...
					FROM dist_order_details AS DistOrderDetail
							JOIN products AS Product ON(DistOrderDetail.product_id=Product.id)
				...
				if (empty($order_detail_results)) {
					...
					$sql = "UPDATE DistCurrentInventory 
							...
							inner join(
							SELECT
								DistOrderDetail.product_id as prod_id,
				...
				$sql = "UPDATE DistCurrentInventory 
							...
							SELECT
								...
								FROM dist_order_details AS DistOrderDetail
													...
													WHERE DistOrderDetail.dist_order_id=$dist_order_id
													GROUP BY 
														DistOrderDetail.product_id
							)order_qty on order_qty.prod_id=DistCurrentInventory.product_id
							where DistCurrentInventory.store_id= $store_id
    ...
    */
	...
	function order_discount_update($order_no)
	{
		$this->LoadModel('DistOrder');
		$memo_data = $this->DistOrder->query("select 
		m.id as memo_id,
		sum(md.price*md.sales_qty) as total_memo,
		FLOOR((sum(md.price*md.sales_qty)*10/100)) as discount
		from dist_orders m
		inner join dist_order_details md on m.id=md.dist_order_id
		where 
			m.order_date between '2022-11-01' and '2022-11-30' 
			and m.dist_order_no='$order_no'
			...
			group by
				m.id
			having
				sum(md.price*md.sales_qty)>=1000");
		if (count($memo_data)) {

			$sql = "update m set m.total_discount=(case when m.total_discount is null then 0 else m.total_discount end)+eligible_memo.discount,m.discount_value=(case when m.discount_value is null then 0 else m.discount_value end)+eligible_memo.discount,m.gross_value=(m.gross_value-eligible_memo.discount) from dist_orders m 
            inner join (select 
            m.id as memo_id,
            sum(md.price*md.sales_qty) as total_memo,
            FLOOR((sum(md.price*md.sales_qty)*10/100)) as discount
            ...
            where 
                ...
                group by
                    m.id
                having
                    sum(md.price*md.sales_qty)>=1000) eligible_memo on eligible_memo.memo_id=m.id";
			$this->DistOrder->query($sql);
		}
	}

	function memo_discount_update($order_no)
	{
		$this->LoadModel('DistOrder');
		$memo_data = $this->DistOrder->query("select 
		m.id as memo_id,
		...
		where 
			m.memo_date between '2022-11-01' and '2022-11-30' 
			and m.dist_memo_no='$order_no'
			and md.product_id in (
								135 ,
								136 ,
								137 ,
		...
		if (count($memo_data)) {
			...
			$sql = "update m set m.total_discount=(case when m.total_discount is null then 0 else m.total_discount end)+eligible_memo.discount,m.discount_value=(case when m.discount_value is null then 0 else m.discount_value end)+eligible_memo.discount,m.gross_value=(m.gross_value-eligible_memo.discount) from dist_memos m 
            inner join (select 
			m.id as memo_id,
			sum(md.price*md.sales_qty) as total_memo,
			FLOOR((sum(md.price*md.sales_qty)*10/100)) as discount
