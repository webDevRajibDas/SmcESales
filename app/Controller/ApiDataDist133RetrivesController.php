 ...
 */
class ApiDataDist133RetrivesController extends AppController
{
		...
		if (!empty($json_data['order_list'])) {
			...
			foreach ($json_data['order_list'] as $result) {
				...
				if (count($order_id_arr) == 0) {
					...
					//pr($order_id_arr);
					if ($order_id_arr['DistOrder']['status'] == 2 && in_array($order_id_arr['DistOrder']['processing_status'], array(1, 2))) {
						$res['status'] = 1;
						//$res['memo_no'] = $result['order_number'];
