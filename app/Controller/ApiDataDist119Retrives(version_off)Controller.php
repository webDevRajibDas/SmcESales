 ...
 */
class ApiDataDist119RetrivesController extends AppController 
{
            ...
            if (!empty($user_info)) {
                ...
                if(empty($user_info['User']['mac_id']))
                {
                    ...
                    else
                    {
						...
						$so_info = $this->SalesPerson->find('first', array(
							'fields' => array('SalesPerson.name','SalesPerson.id','SalesPerson.dist_sales_representative_id','DistSalesRepresentative.id','DistSalesRepresentative.name','DistSalesRepresentative.dist_distributor_id','DistDistributor.id','DistDistributor.name','DistStore.id','Office.id','Office.office_name','Office.phone','Office.address','SalesPerson.territory_id'),
							'conditions' => array('SalesPerson.id' => $so_id),
							'joins' => array(
								array(
						...
						{
							$login['status'] = 0;
	                        $login['message'] =  'User Activity Block For This Area';
	                        $res = $login;
	                        $this->set(array(
	                            'response' => array($res),
	                            '_serialize' => array('response')
	...
	public function dist_data_pull(){
		...
		/* added by naser due to product bonus policy stopped by smc . now provide by open bonus. 
			for open need a entry in open bonus table in apps db
		*/
        ...
        $conditions['Bonus.mother_product_id !=']=array_keys($bonus_product_id);
		...
		foreach ($bonus_product as $key => $val) {
        	$product_history['product_history'][] = array(
				...
				'product_id' 		=> $val['Bonus']['mother_product_id'],
				'combination_id' 	=> 'PB',
			);
        }
		...
		// Comment by Naser after discussion about turn off product bonus policy.
		/*$this->loadModel('Bonus');
		$last_update_date = ($json_data['bonuses']['last_update']=='all')?'all':date('Y-m-d H:i:s', strtotime($json_data['bonuses']['last_update']));
					...
					if ($this->DistOrder->save($OrderData)) 
					{
						...
						if ($order_id) 
						{
							...
							foreach ($result['order_details'] as $val) 
							{
								...
								$order_details['DistOrderDetail']['measurement_unit_id'] = $punits_pre['sales_measurement_unit_id'];
								...
								if ($stock_hit) {
									...
									$this->dist_booking_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 3, $OrderData['order_date'], $booking_bonus_qty);
								}
		...
		if($inventory_info)
		{
			if ($update_type == 'deduct') 
			{
				...
				if($inventory_info['DistCurrentInventory']['bonus_booking_qty'] < $booking_bonus_qty){
					...
					$update_array['DistCurrentInventory']['bonus_booking_qty'] = $inventory_info['DistCurrentInventory']['bonus_booking_qty'] - $booking_bonus_qty;
				}
	...
	public function dist_create_memo() {
		...
		if (!empty($json_data['memo_list'])) 
		{
			...
			$inventory_results = $this->DistCurrentInventory->find('all', array(
				...
				'fields' => array('product_id', 'sum(qty) as qty','product_id', 'sum(bonus_qty) as bonus_qty'),
				'group'=>array('product_id'),
				//'order' => array('DistCurrentInventory.product_id' => 'asc'),
				'recursive' => -1
			));
			//pr($inventory_results);
			...
			foreach($inventory_results as $inventory_result){
				...
				$inventory_bonus_qty_info[$inventory_result['DistCurrentInventory']['product_id']]=$inventory_result[0]['bonus_qty'];
			}
			...
			{	
				...
				$order_detials = $this->DistOrderDetail->find('all', array(
					...
					'fields' => array(
						'DistOrderDetail.dist_order_id as dist_order_id',
						'DistOrderDetail.product_id as product_id',
						'DistOrderDetail.measurement_unit_id as measurement_unit_id',
						'DistOrderDetail.sales_qty as sales_qty',
						'DistOrderDetail.price as price',
						'DistOrderDetail.is_bonus as is_bonus',
						'DistOrderDetail.product_price_id as product_price_id'),
					'recursive' => -1
				));
					...
					foreach($memo_product1 as $p_price => $memo_product2){
						foreach($memo_product2 as $is_bonus_c => $memo_product){
							...
							if($memo_product['invoice_qty'] < $memo_product['sales_qty'])
							{
								...
								if($is_bonus || $memo_product['price']<=0){
				...
				if($inventory_check)
				{
								...
								if(!isset($memo_products2[$o_product_id][$b_track]))
								{
									...
									if($o_val['measurement_unit_id']>0)
									{
										if ($o_val['measurement_unit_id'] == $punits_pre['base_measurement_unit_id']) {
											$base_quantity = $o_val['sales_qty'];
											$invoice_qty = $o_val['invoice_qty'];
										} else {
									...
									$this->dist_memo_update_current_inventory($base_quantity, $o_product_id, $store_id, 'add', 4, $result['memo_date'], $o_is_bonus, $invoice_qty);
								}
							}
						}
					}
					...
					if($market_info)
					{
						...
						if($count==0)
						{
							...
							$MemoData['total_discount'] = $json_data['total_discount'];
							...
							{																
								...
								if ($memo_id) 
								{
									...
									foreach ($result['memo_details'] as $val) 
									{
										...
										/*echo $update_type;
										echo '<br>';
										echo $product_id. '--'. $base_quantity.'<br>CC';
										echo '<br>';
										echo '<br>';*/
																			
										$this->dist_memo_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, $tran_type_id, $result['memo_date'], $is_bonus, $invoice_qty);
							...
							if ($this->DistMemo->save($MemoData)) 
							{
								...
								if ($memo_id) 
								{
									...
									foreach ($result['memo_details'] as $val) 
									{
										...
										$invoice_qty = 0;
										$update_type = 'deduct';									
										$this->dist_memo_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 3, $result['memo_date'],  $val['is_bonus'], $invoice_qty);
	...
	public function dist_memo_delete($id = 0, $sr_code = 0, $office_id = 0) {
		...
		for ($memo_detail_count = 0; $memo_detail_count < count($memo_id_arr['DistMemoDetail']); $memo_detail_count++) 
		{
			...
			$update_type = 'add';
			$this->dist_memo_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 4, $memo_id_arr['DistMemo']['memo_date'], $is_bonus, $invoice_qty);
	...
	public function dist_order_delivery_schedule(){
		...
		foreach($order_products_list as $order_details_id => $order_info)
		{
			...
			foreach($order_info as $order_id => $order_detail_result)
			{
				...
				if($is_bonus){
					$product_stock_qty = @$inventory_bouns_qty_info[$product_id];
				}else{
					$product_stock_qty = @$inventory_qty_info[$product_id];
				}
				...
				if($product_stock_qty >= $sales_qty)
				{
					...
					if($is_bonus)
					{
						$inventory_bouns_qty_info[$product_id] = $n_product_stock_qty;
						// $product_stock_qty = @$inventory_bouns_qty_info[$product_id];
					}
					else
					{
        ...
        if($this->DistOrderDeliverySchedule->save($order_data_list))
		{
			...
			foreach($order_data_list['DistOrderDeliveryScheduleOrderDetail'] as $dist_order_no => $order_details)
			{
				...
				foreach($order_details as $key => $result)
				{
					...
					if($dist_stock_check && $result['status'] && $stock_deduction[$result['order_id']]==1)
					{
						...
						$update_type = 'deduct';
						$this->dist_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 3, $result['order_date'], $is_bonus, $invoice_qty);
						
						if($is_bonus==1){
							$booking_bonus_qty = $base_quantity;
							$base_quantity=0;	
						}else{
							$booking_bonus_qty = 0;	
						}
						$this->dist_booking_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 3, $order_date, $booking_bonus_qty);
					}
					else
					{
			...
			$this->DistOrderDeliveryScheduleOrderDetail->saveAll($total_order_detail);
	...
	private function dist_stock_check($store_id, $order_id)
    {
		...
		$order_datas=$this->DistOrderDetail->find('all',array(
			...
			'fields'=>array('sales_qty','product_id','price','is_bonus'),
			//'group'=>array('DistOrderDetail.product_id'),
			'recursive'=>-1
		));
		
		foreach($order_datas as $result)
		{
			//pr($result);
			$product_id = $result['DistOrderDetail']['product_id'];
			$qty = $result['DistOrderDetail']['sales_qty'];
			$price = $result['DistOrderDetail']['price'];
			$is_bonus = $result['DistOrderDetail']['is_bonus'];
			...
			if($price>0){
				...
				$current_inventory=$this->DistCurrentInventory->find('all',array(
					...
					'group'=>array('DistCurrentInventory.store_id','ProductMeasurement.qty_in_base','DistCurrentInventory.product_id HAVING (sum(DistCurrentInventory.bonus_qty))  >= ROUND((case when ProductMeasurement.qty_in_base is null then 1 else ProductMeasurement.qty_in_base end) *'.$qty.',0)'),
					'fields'=>array('DistCurrentInventory.store_id','DistCurrentInventory.product_id','sum(DistCurrentInventory.qty) as qty')
				));
			}
	...
	public function dist_update_current_inventory($quantity, $product_id, $store_id, $update_type = 'deduct', $transaction_type_id = 0, $transaction_date = '', $is_bonus=0, $invoice_qty=0) {
	...
	public function dist_order_delivery_unprocess(){
		...
		foreach($cancelled_order_list as $cancelled_orders)
		{
			...
			foreach($order_results as $key => $order_info)
			{
				...
				$this->dist_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 4, $order_date, $is_bonus, $invoice_qty);
	...
	public function dist_product_sales_report()
	{
		...
		foreach($products as $product_id => $product_name)
		{
			...
			$product_total_sales = $this->DistMemo->find('all', array(
				...
				'fields'=> array('sum(DistMemoDetail.price*DistMemoDetail.sales_qty) AS total_price','sum(DistMemoDetail.sales_qty) AS total_qty', 'COUNT(DistMemo.dist_order_no) AS total_ec'),
	...
	public function dist_product_bonus_report()
	{
		...
		foreach($products as $product_id => $product_name)
		{
			...
			$product_total_sales = $this->DistMemo->find('all', array(
				...
				'fields'=> array('sum(DistMemoDetail.sales_qty) AS total_qty','COUNT(DistMemo.dist_order_no) AS total_ec'),
				//'group' => array('DistMemoDetail.product_id'),
				'recursive' => -1
				)
			);
	...
	public function dist_product_processed_order_report()
	{
		...
		$path = APP . 'logs/';
        $myfile = fopen($path . "dist_product_processed_order_report.txt", "a") or die("Unable to open file!");
        fwrite($myfile, "\n" . $this->current_datetime() . ':' . $this->request->input());
        fclose($myfile);
		...
		$this->set(array(
            'product_sales_report' => $product_results,
            '_serialize' => array('product_sales_report')
        ));
	}
	...
	public function dist_sales_targets(){
        ...
        foreach ($sr_product_targets as $val) 
		{
			...
			$s_data['achieve_qty'] 		= $val['DistSaleTargetMonth']['target_quantity_achievement'];
			$s_data['target_amount'] 	= $val['DistSaleTargetMonth']['target_amount'];
			$s_data['achieve_amount'] 		= $val['DistSaleTargetMonth']['target_amount_achievement'];
            $info_array[] 		= $s_data;
        }
			...
			foreach($json_data['order_numbers'] as $val)
			{
				...
				$output.= '<table border="1" cellspacing="0" cellspacing="0" style="width:100%; margin:10px 0 20px; font-size:10px; font-family:Gotham, Arial, sans-serif">
							...
							{	
								...
								{
									$total = $val['DistOrderDetail']['sales_qty']*$val['DistOrderDetail']['price'];
									$output.= '<tr>
											<td align="left">'.$val['Product']['name'].'</td>
											<td align="right">'.sprintf('%.2f',$val['DistOrderDetail']['vat']).'</td>
											<td align="right">'.$val['DistOrderDetail']['price'].'</td>
								...
								{
									$bonus.= $val['Product']['name'].'('.$val['DistOrderDetail']['sales_qty'].'), ';
								}
								elseif(!$val['DistOrderDetail']['is_bonus'] && !$val['DistOrderDetail']['price']>0)
								{
						...
						$output.= '<tr>
									...
									<td align="right"><strong>'.sprintf('%.2f', $order_info['DistOrder']['gross_value']).'</strong></td>
								</tr>';
						$output.= '<tr>
	...
	public function force_dist_memo_del(){
		...
		/*$sql = "select dist_order_no, count (*) as total 
from dist_memos where dist_order_no!='0'
GROUP BY
    dist_order_no
HAVING 
    COUNT(*) > 1";*/
	
		/*$sql = "select dm.dist_order_no, count (*) as total 
from dist_memos dm 
left join dist_memo_details dmd on dmd.dist_memo_id=dm.id
where dm.dist_order_no!='0' and dmd.id is null 
and dm.dist_order_no='O20465200205182651'
GROUP BY
    dm.dist_order_no
HAVING 
    COUNT(*) > 1";*/
	
	$sql = "select dm.id, dm.dist_order_no 
from dist_memos dm 
left join dist_memo_details dmd on dmd.dist_memo_id=dm.id
where dm.dist_order_no!='0' and dmd.id is null 
and dm.dist_order_no='O20542200507134831'";

		$memo_list = $this->DistMemo->query($sql);
		
		
		
		//$json_data = $this->request->input('json_decode', true);
		//$dist_order_no = $json_data['dist_order_no'];
		...
		foreach($memo_list as $memo_result){
		
			
			
			$dist_order_no = $memo_result[0]['dist_order_no'];
			...
			if($dist_order_no)
			{
				...
				$dist_memo_no = $memo_id_arr['DistMemo']['dist_memo_no'];
				$this->DistMemoDetail->deleteAll(array('DistMemoDetail.dist_memo_id' => $memo_id));
				$this->DistMemo->deleteAll(array('DistMemo.id' => $memo_id));
				
				$res['status'] = 1;
				
				$res['message'].= 'Order Num: '.$dist_order_no.' -- Memo id: '.$memo_id.'<br>';        
				//echo '<br>';
