...
class ApiData172RetrivesController extends AppController {
    ...
    public function user_login() {
        ...
        $path = APP . 'logs/';
        $myfile = fopen($path . "login_172.txt", "a") or die("Unable to open file!");
            ...
            if (is_numeric($json_data['market_id']) || true) {
                ...
                if ($count == 0) {
                    ...
                    foreach ($json_data['memo_details'] as $val) {
                        ...
                        $memo_details['price'] = $val['price']-(isset($val['discount_amount'])?$val['discount_amount']:0);
    ...
    public function get_memo_list() {
        ...
        foreach ($memo_list as $m) {
            ...
            foreach ($m['MemoDetail'] as $each_memo_details) {
                ...
                $mdata['memo_details'][$mm]['vat'] = $this->get_vat_by_product_id_memo_date($product_id,$m['Memo']['memo_date'],$m['Memo']['is_distributor'],$mdata['outlet_id']);
                $mm++;
            }
   ...
   /*
    ...
    public function update_territory_wise_collection_deposit_balance($so_id) {
        ...
        foreach ($instrument_type as $key => $ins_data) {
            $collection_data = $this->Collection->find('first', array(
                ...
                'conditions' => array('Collection.territory_id' => $territory_id, 'Collection.type' => $key,'Collection.memo_date >='=>'2018-10-01'),
                'joins' => array(
                    array(
                        'table' => 'sales_people',
                        'alias' => 'SalesPerson',
            ...
            $deposit_data = $this->Deposit->find('first', array(
                ...
                'conditions' => array('Deposit.territory_id' => $territory_id, 'Deposit.type' => $key,'Deposit.deposit_date >='=>'2018-10-01'),
                'joins' => array(
                    array(
                        'table' => 'sales_people',
                        'alias' => 'SalesPerson',
	...
	public function get_policy_list() {
		...
		$conditions =array('GroupWiseDiscountBonusPolicy.is_distributor' => 0);
        ...
        $policy_list = $this->GroupWiseDiscountBonusPolicy->find('all', array(
			...
			'contain' => array(
				'GroupWiseDiscountBonusPolicyToOffice' => array(
					'conditions' => array(
						'GroupWiseDiscountBonusPolicyToOffice.office_id' => $office_id  // <-- Notice this addition
					),
					//'order' => 'GroupWiseDiscountBonusPolicyToOffice.office_id DESC',
					/*'limit' => 6,
					'Trailer' => array(
						'limit' => 1
					),*/
