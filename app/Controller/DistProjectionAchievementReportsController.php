 ...
 */
class DistProjectionAchievementReportsController extends AppController {
...
public $uses = array('Memo', 'Office', 'Territory', 'OutletCategory', 'Outlet', 'Product', 'Brand', 'ProductCategory','DistMemo','DistMemoDetail','DistOutlet','DistSalesRepresentative','DistSaleTargetMonth');
...
public function admin_index($id = null) 
{
	...
	$this->Session->delete('outlet_lists');
		...
		$this->set('page_title', 'Distributor Projection and Achievement Analysis Report');
		$srs=array();
		...
		$fiscal_years_result = $this->FiscalYear->find('all', 
			array(
		...
		if ($this->request->is('post') || $this->request->is('put')){
			$office_conditions = array('Office.parent_office_id'=>$this->data['DistProjectionAchievementReports']['region_office_id']);
		}
		...
		{
			$office_id = isset($this->request->data['DistProjectionAchievementReports']['office_id']) != '' ? $this->request->data['DistProjectionAchievementReports']['office_id'] : $office_id;
			...
			$territories = $this->Territory->find('list', array(
				...
				'order' => array('Territory.name' => 'asc')
                       ...
                       $this->set(compact('territories'));
                        ...
                        $srs_pre = $this->DistSalesRepresentative->find('all', array(
                        ...
                        $srs=array();
                        foreach($srs_pre as $sk=>$sv)
                        {
                            $code=$sv['DistSalesRepresentative']['code'];
                            $srs[$code]=$sv['DistSalesRepresentative']['name'];
                        }
		...
		{
			if ($this->request->data['DistProjectionAchievementReports']['indicators'] && $this->request->data['DistProjectionAchievementReports']['indicator_unit']) 
			{
				...
				$region_office_ids = $request_data['DistProjectionAchievementReports']['region_office_id'];
				$office_ids = $request_data['DistProjectionAchievementReports']['office_id'];
				$sr_ids = $request_data['DistProjectionAchievementReports']['sr_id'];	
                                $territory_ids = $sr_ids;
				$qumulative = $request_data['DistProjectionAchievementReports']['qumulative'];	
				$fiscal_year_id = $request_data['DistProjectionAchievementReports']['fiscal_year_id'];


				$q_list = $request_data['DistProjectionAchievementReports']['q_list'];	
				$indicators_data = $request_data['DistProjectionAchievementReports']['indicators'];
				$indicator_unit = $request_data['DistProjectionAchievementReports']['indicator_unit'];
                                ...
                                if(is_array($qumulative) && count($qumulative)>0)
				{
                                    if($territory_ids)
                                    {
                                         $find_territories[$sr_ids]=$sr_ids;                                        
                                         $summery_area_name="Sale Representative : ".$this->get_name_from_code($office_id,$sr_ids);
                                         $find_offices=array();  
                                    }
                                    else if($office_id)
                                    {
                                         $find_territories=$this->get_sr_ids($office_id,2);
                                         $find_offices[$office_id]=$office_id;                                           
                                         $summery_area_name="Area Office : ".$this->get_name("Office",$office_id);
                                    }
                                    else if($region_office_ids)
                                    {
                                         $find_territories=$this->get_sr_ids($region_office_ids,3);
                                         $find_offices=array();                                         
			...
			$this->set(compact('details'));	
				...
				if($request_data['DistProjectionAchievementReports']['columns']=='product')
				{
						//for products list
					$conditions = array('NOT' => array('Product.product_category_id'=>32), 'is_active' => 1);
					if($request_data['DistProjectionAchievementReports']['product_id'])$conditions['id'] = $request_data['DistProjectionAchievementReports']['product_id'];
					$product_type = isset($this->request->data['DistProjectionAchievementReports']['product_type']) != '' ? $this->request->data['DistProjectionAchievementReports']['product_type'] : '';
				...
				elseif($request_data['DistProjectionAchievementReports']['columns']=='brand')
				{
						//for products list
					$conditions = array('NOT' => array('Product.product_category_id'=>32), 'is_active' => 1);
					if($request_data['DistProjectionAchievementReports']['brand_id'])$conditions['brand_id'] = $request_data['DistProjectionAchievementReports']['brand_id'];
				...
				elseif($request_data['DistProjectionAchievementReports']['columns']=='category')
				{																
						//for products list
					$conditions = array('NOT' => array('Product.product_category_id'=>32), 'is_active' => 1);
					if($request_data['DistProjectionAchievementReports']['product_category_id'])$conditions['product_category_id'] = $request_data['DistProjectionAchievementReports']['product_category_id'];
					$product_type = isset($this->request->data['DistProjectionAchievementReports']['product_type']) != '' ? $this->request->data['DistProjectionAchievementReports']['product_type'] : '';
					if($product_type)$conditions['source'] = $product_type;
					$product_list = $this->Product->find('list', array(
						'conditions'=> $conditions,
					...
					$row_count=1;
                                        ...
                                        if($details)
                                        {
                                            ...
                                            {  
                                                ...
                                                foreach ($all_regoin_ids as $ak => $av) {
                                                    ...
                                                    $find_territories=$this->get_sr_ids($ak,3);
                                           ...
                                           $tables_data[$ak]['area_name']="Region Name:".$av; 
                                            ...
                                            else if($details_type=="office_wise")
                                            {
                                                ...
                                                foreach ($all_regoin_ids as $ak => $av) {
                                                    ...
                                                    $find_territories=$this->get_sr_ids($ak,2);
						...
						$row_count++;
                                           ...
                                           $tables_data[$ak]['area_name']="Area Office Name:".$av; 
                                            ...
                                            else if($details_type=="sr_wise")
                                            {
                                                
                                                
                                                
                                                $all_regoin_ids=$this->get_sr_ids($office_ids,2);   
                                                $details_rows=$all_regoin_ids;
                                                foreach ($all_regoin_ids as $ak => $av) {
                                                     $rows_data="";
                                                     $target_data_arr=array();
						...
						$row_count++;
                                           ...
                                           $tables_data[$ak]['data']=$rows_data; 
                                           $tables_data[$ak]['area_name']="Sales Representative Name:".$av; 
                                        }
			...
			else if(!$this->request->data['DistProjectionAchievementReports']['indicators'])
			{
...
public function getInfo($territory_id=0)
{
    ...
    public function get_target_data($month_id,$fiscal_year_id,$unit_type,$office_ids=array(),$territory_ids=array()) {
        ...
        if(is_array($office_ids) && count($office_ids)>0)
        {
           ...
           $conditions['DistSaleTargetMonth.aso_id']=$office_ids;
        }
       
        if(is_array($territory_ids) && count($territory_ids)>0)
        {            
            $territory_ids=array_keys($territory_ids); 
           $conditions['DistSaleTarget.dist_sales_representative_id']=$territory_ids;
        }

    	if($fiscal_year_id){			
    		$total_target_result = $this->DistSaleTargetMonth->find('all', 
    			array(
    				'conditions'=>$conditions,

    				'fields'=> array('product_id','sum(target_amount) as total_amount','sum(target_quantity) as total_quantity'),
    				'group' => array('DistSaleTargetMonth.product_id'), 
    				'recursive' => 0
    				)
    			);	
    	}
    	...
    	foreach ($total_target_result as $tk => $tv) {
    		$product_id=$tv['DistSaleTargetMonth']['product_id'];
    ...
    public function get_month_info($fiscal_year_id) {
    	...
    	$month_result = $this->Month->find('all', 
    		array(
   ...
   */
    	...
    	$territory_result = $this->Territory->find('list', 
    		array(    			
			'conditions'=> $conditions,
    			'recursive' => 0
    			)
  ...
  public function get_sr_ids($office_id,$from) {
            $conditions=array();
            if($from==2)
            {
                $conditions=array("DistSalesRepresentative.office_id" => $office_id);
    	...
    	$sr_result = $this->DistSalesRepresentative->find('list', 
    		array(    			
			'conditions'=> $conditions,
    			'recursive' => 0
    			)
   ...
   */
     ...
     public function get_all_office_ids($office_id,$from) {
    	...
    	$office_result = $this->Office->find('list', 
    		array(    			
			'conditions'=> $conditions,
    			'recursive' => -1
    			)
     ...
     public function get_name($modal,$id) {
    	...
    	$office_result = $this->$modal->find('list', 
    		array(    			
   ...
   public function get_name_from_code($office_id,$code) {
        $this->loadModel('DistSalesRepresentative');
        
    	$office_result = $this->DistSalesRepresentative->find('all', 
    		array(    			
			'conditions'=> array('office_id'=>$office_id,'code'=>$code),
    			'recursive' => -1
    			)
    		);
      
    	return $office_result[0]['DistSalesRepresentative']['name'];
	 ...
	 public function get_sr_list() {
        ...
        $territory = $this->DistSalesRepresentative->find('all', array(
            'conditions' => array('DistSalesRepresentative.office_id' => $office_id),
            'order' => array('DistSalesRepresentative.name' => 'asc'),
            'recursive' => -1
        ));
        ...
        $data=array();
        foreach($territory as $sk=>$sv)
        {
            $code=$sv['DistSalesRepresentative']['code'];
            //$data['id'][$code]=$sv['DistSalesRepresentative']['name'];
            $data[$code]['id']=$code;
            $data[$code]['name']=$sv['DistSalesRepresentative']['name'];
        }
        
        if (!empty($data)) {
            echo json_encode(array_merge($rs, $data));
        } else {
            echo json_encode($rs);
        }
        $this->autoRender = false;
