 ...
 */
class TransactionListStockSosController extends AppController
{
	...
	public $uses = array('Product', 'ProductCategory', 'Memo', 'Office', 'Territory', 'Division', 'District', 'Thana', 'Market', 'OutletCategory', 'Outlet', 'TerritoryAssignHistory', 'SalesPerson', 'Brand', 'RptDailyTranBalance', 'Store', 'TransactionType', 'Challan', 'ReturnChallan', 'GiftItem', 'DoctorVisit');
	...
	public function admin_index($id = null)
	{
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			$date_from = date('Y-m-d', strtotime($request_data['TransactionListStockSos']['date_from']));
			$date_to = date('Y-m-d', strtotime($request_data['TransactionListStockSos']['date_to']));
			$this->set(compact('date_from', 'date_to'));

			$type = $this->request->data['TransactionListStockSos']['type'];
			$this->set(compact('type'));

			$region_office_id = isset($this->request->data['TransactionListStockSos']['region_office_id']) != '' ? $this->request->data['TransactionListStockSos']['region_office_id'] : $region_office_id;
			...
			$office_id = isset($this->request->data['TransactionListStockSos']['office_id']) != '' ? $this->request->data['TransactionListStockSos']['office_id'] : $office_id;
			$this->set(compact('office_id'));

			$territory_id = isset($this->request->data['TransactionListStockSos']['territory_id']) != '' ? $this->request->data['TransactionListStockSos']['territory_id'] : 0;
			$this->set(compact('territory_id'));

			$so_id = isset($this->request->data['TransactionListStockSos']['so_id']) != '' ? $this->request->data['TransactionListStockSos']['so_id'] : 0;
			$this->set(compact('so_id'));

			$unit_type = $this->request->data['TransactionListStockSos']['unit_type'];
			//$this->set(compact('unit_type'));
			$unit_type_text = ($unit_type == 2) ? 'Base' : 'Sales';
			$this->set(compact('unit_type_text'));

			$product_type_id = isset($this->request->data['TransactionListStockSos']['product_type_id']) != '' ? $this->request->data['TransactionListStockSos']['product_type_id'] : 0;


			$product_ids = isset($this->request->data['TransactionListStockSos']['product_id']) != '' ? $this->request->data['TransactionListStockSos']['product_id'] : 0;
			$brand_ids = isset($this->request->data['TransactionListStockSos']['brand_id']) != '' ? $this->request->data['TransactionListStockSos']['brand_id'] : 0;
			$product_category_ids = isset($this->request->data['TransactionListStockSos']['product_category_id']) != '' ? $this->request->data['TransactionListStockSos']['product_category_id'] : 0;
			...
			$outlet_category_id = isset($this->request->data['TransactionListStockSos']['outlet_category_id']) != '' ? $this->request->data['TransactionListStockSos']['outlet_category_id'] : 0;
			...
			if ($office_id) {
				$con['AND'] = array(
					'OR' => array(
						array('Challan.sender_store_id' => $territoty_stores[$territory_id]),
						array('Challan.receiver_store_id' => $territoty_stores[$territory_id])
					),
				);
			...
			$challan_q_results = $this->Challan->find('all', array(
				...
				'fields' => array('Challan.challan_no',  'Challan.received_date', 'Challan.transaction_type_id', 'Challan.sender_store_id', 'Challan.receiver_store_id', 'Store.office_id', 'Store.territory_id', 'ChallanDetail.product_id', 'ChallanDetail.measurement_unit_id', 'sum(ChallanDetail.challan_qty) as challan_qty'),
				'group' => array('Challan.challan_no', 'Challan.received_date', 'Challan.transaction_type_id', 'Challan.sender_store_id', 'Challan.receiver_store_id', 'Store.office_id', 'Store.territory_id', 'ChallanDetail.product_id', 'ChallanDetail.measurement_unit_id'),
				'recursive' => -1,
			));
			...
			foreach ($challan_q_results as $result) {
				...
				$challan_results[$result['Challan']['received_date']][$result['Challan']['transaction_type_id']][$result['Challan']['challan_no']][$result['ChallanDetail']['product_id']] =
					array(
						...
						'challan_date' 			=> $result['Challan']['received_date'],
			...
			if ($office_id) {
				$con['AND'] = array(
					'OR' => array(
						array('ReturnChallan.sender_store_id' => $territoty_stores[$territory_id]),
						array('ReturnChallan.receiver_store_id' => $territoty_stores[$territory_id])
					),
				);
			}
			...
			if ($product_ids) $g_con['GiftItemDetail.product_id'] = $product_ids;
			if ($brand_ids) $g_con['Product.brand_id'] = $brand_ids;
			if ($product_category_ids) $g_con['Product.product_category_id'] = $product_category_ids;
			if ($product_type_id) $g_con['Product.product_type_id'] = $product_type_id;
			...
			$g_query_results = $this->GiftItem->find('all', array(
				...
				'fields' => array('sum(ROUND((GiftItemDetail.quantity*CASE WHEN ProductMeasurement.qty_in_base is null then 1 ELSE ProductMeasurement.qty_in_base END),0)) AS gift_qty', 'GiftItem.date', 'GiftItemDetail.product_id', 'Product.sales_measurement_unit_id'),

				'group' => array('GiftItem.date', 'GiftItemDetail.product_id', 'Product.sales_measurement_unit_id'),
				'recursive' => -1,
			));
			...
			//exit;
			$r_gift_issue = array();
			foreach ($g_query_results as $result) {
				$challan_qty = ($unit_type == 2) ? $result[0]['gift_qty'] : $this->unit_convertfrombase($result['GiftItemDetail']['product_id'], $result['Product']['sales_measurement_unit_id'], $result[0]['gift_qty']);

				$r_gift_issue[$result['GiftItem']['date']][$result['GiftItemDetail']['product_id']] =
					array(
						'product_id' 			=> $result['GiftItemDetail']['product_id'],
						'chalan_date' 			=> $result['GiftItem']['date'],
						'challan_qty' 			=> sprintf("%01.2f", $challan_qty),
					);
			}
			$this->set(compact('r_gift_issue'));
			...
			if ($product_ids) $g_con['DoctorVisitDetail.product_id'] = $product_ids;
			if ($brand_ids) $g_con['Product.brand_id'] = $brand_ids;
			if ($product_category_ids) $g_con['Product.product_category_id'] = $product_category_ids;
			if ($product_type_id) $g_con['Product.product_type_id'] = $product_type_id;
			//pr($g_con);
			$g_doc_query_results = $this->DoctorVisit->find('all', array(
				...
				'fields' => array('sum(ROUND((DoctorVisitDetail.quantity * CASE WHEN ProductMeasurement.qty_in_base is null then 1 ELSE ProductMeasurement.qty_in_base END),0)) AS gift_qty', 'DoctorVisit.visit_date', 'DoctorVisitDetail.product_id', 'Product.sales_measurement_unit_id'),
				'group' => array('DoctorVisit.visit_date', 'DoctorVisitDetail.product_id', 'Product.sales_measurement_unit_id'),
				'recursive' => -1,
			));
			$r_doc_gift_issue = array();
			foreach ($g_doc_query_results as $result) {
				$challan_qty = ($unit_type == 2) ? $result[0]['gift_qty'] : $this->unit_convertfrombase($result['DoctorVisitDetail']['product_id'], $result['Product']['sales_measurement_unit_id'], $result[0]['gift_qty']);

				$r_doc_gift_issue[$result['GiftItem']['date']][$result['DoctorVisitDetail']['product_id']] =
					array(
						'product_id' 			=> $result['DoctorVisitDetail']['product_id'],
						'chalan_date' 			=> $result['DoctorVisit']['visit_date'],
						'challan_qty' 			=> sprintf("%01.2f", $challan_qty),
					);
			}
			$this->set(compact('r_doc_gift_issue'));
		}
