 ...
 */
class ApiDataDist135RetrivesController extends AppController
{

	public $components = array('RequestHandler', 'Usermgmt.UserAuth');


	public function mac_check($mac_id, $so_id,$sync=1)
	{
		...
		if($sync==1){
			...
			$office_id = $so_info['SalesPerson']['office_id'];
			/*---- User Activity block for some area :start -----------*/
			
			if($this->office_wise_login_block($office_id))
			{
				return false;
			}
		}
			
		/*---- User Activity block for some area :END -------------*/
	...
	public function office_wise_login_block($office_id)
	{	
		//15,16,18,19,22,23,24,25,26,27,28,29,44
		/* 
			08-08:20
			19	Dhaka West Sales Office
			15	Dhaka East Sales Office

			08:20-08:40
			23	Rangpur Sales Office
			26	Mymensingh Sales Office

			08:40-09:00
			28	Kushtia Sales Office
			24	Rajshahi Sales Office

			09:00-09:20
			27	Khulna Sales Office
			22	Bogura Sales Office

			09:20-09:40
			44	Dhaka Metro
			25	Sylhet Sales Office

			09:40-10:00
			16	Cumilla Sales Office
			18	Chattogram Sales Office

			10:00-10:20
			29	Barisal Sales Office
			//19,15,23,26,28,24,27,22,44,25,16,18
			//15,16,18,19,22,23,24,25,26,27,28,29,44
		*/
		date_default_timezone_set('Asia/Dhaka');
		
		if (date('H:i') >= "12:00" && date('H:i') < "07:30")
			$restricted_office = array(15,16,18,19,22,23,24,25,26,27,28,29,44);
		else if (date('H:i') >= "18:00" && date('H:i') < "19:00")
			$restricted_office = array(15,16,18); //------------evening time slot----\\
		else if (date('H:i') >= "19:00" && date('H:i') < "20:00")
			$restricted_office = array(19,22,23);
		else if (date('H:i') >= "20:00" && date('H:i') < "21:00")
			$restricted_office = array(24,25,26);
		else if (date('H:i') >= "21:00" && date('H:i') < "22:00")
			$restricted_office = array(27,28,29);
		else if (date('H:i') >= "22:00" && date('H:i') < "22:30")
			$restricted_office = array(44);
		else if (date('H:i') >= "22:30" && date('H:i') < "23:59")
			$restricte0d_office = array(15,16,18,19,22,23,24,25,26,27,28,29,44);
		else
			$restricted_office = array(); 
			//$restricted_office = array(15,16,18,19,22,23,24,25,26,27,28,29,44);

		if (in_array($office_id, $restricted_office)) {
			return false;
		} else {
			return true;
		}
			...
			if (!empty($user_info)) {
				...
				if (empty($user_info['User']['mac_id'])) {
					...
					if ($mac_exist) {
						...
						$mac_check = $this->mac_check($json_data['mac'], $so_id,0);
						
						if (!$mac_check) {

							$mac['status'] = 0;
							$mac['message'] = 'Not Match Your Login Time Slot';
							$res = array($mac);
							$this->set(array(
								'response' => $res,
								'_serialize' => array('response')
						...
						$qry = "select distinct dist_tso_id from dist_tso_mapping_histories
						  where office_id=$office_id and is_change=1 and dist_distributor_id=$distributor_id and 
							'" . $cdate . "' between effective_date and 
							case 
						...
						$dist_data = $this->SalesPerson->query($qry);
							//pr($qry);exit;
						$dist_ids = array();
						...
						$ae_data = $this->SalesPerson->query($qry2);
						//pr($ae_data);die();
						$ae_ids = array();
						...
						$user_info['user_info'] = array(
							...
							'db_id' 			=> $so_info['DistDistributor']['id'],
	...
	public function dist_data_pull()
	{
		...
		foreach ($products as $val) {
			$product['product'][] = array(
				...
				'is_virtual' 					=> $val['Product']['is_virtual'],
				'parent_id' 					=> $val['Product']['parent_id'],
		...
		if ($last_update_date != 'all') $conditions['ProductPricesV2.updated_at >'] = $last_update_date;
		$conditions['ProductPriceSectionV2.is_sr'] = 1;
		$results = $this->ProductPricesV2->find('all', array(
			'conditions' => $conditions,
			'joins' => array(
				array(
			...
			'fields' => array(
				'ProductPricesV2.id',
				'ProductPricesV2.product_id',
				'ProductPricesV2.target_custommer',
				'ProductPricesV2.institute_id',
				'ProductPricesV2.general_price',
				'ProductPricesV2.effective_date',
				'ProductPricesV2.vat',
				'ProductPricesV2.updated_at'
			),
			'group' => array(
				'ProductPricesV2.id',
				'ProductPricesV2.product_id',
		...
		foreach ($results as $val) {
			$product_price['product_price'][] = array(
				'price_id' 					=> $val['ProductPricesV2']['id'],
				'product_id' 				=> $val['ProductPricesV2']['product_id'],
				'target_custommer' 			=> $val['ProductPricesV2']['target_custommer'],
				'institute_id' 				=> $val['ProductPricesV2']['institute_id'],
				'price' 					=> $val['ProductPricesV2']['general_price'],
				'effective_date' 			=> $val['ProductPricesV2']['effective_date'],
				'vat' 						=> $val['ProductPricesV2']['vat'],
			);
		}
		...
		foreach ($order_list as $m) {
			...
			foreach ($m['DistOrderDetail'] as $each_order_details) {
				...
				$mdata['order_details'][$mm]['price_id'] = $each_order_details['product_price_id'];
				$mdata['order_details'][$mm]['order_number'] = $m['DistOrder']['dist_order_no'];
				if ($each_order_details['other_info']) {
					$other_info = json_decode($each_order_details['other_info'], 1);
					$selected_set = $other_info['selected_set'];
					$provided_qty = $other_info['provided_qty'];
					$mdata['order_details'][$mm]['selected_set'] = $selected_set;
					$mdata['order_details'][$mm]['provided_qty'] = $provided_qty;
				}
		...
		foreach ($memo_list as $m) {
			...
			foreach ($m['DistMemoDetail'] as $each_memo_details) {
				...
				$mdata['memo_details'][$mm]['price_id'] = $each_memo_details['product_price_id'];
	...
	public function dist_market_list()
	{
		...
		$office_id = $json_data['office_id'];
		$distributor_id = $json_data['distributor_id'];
		$store_id = $json_data['store_id'];
		$sr_id = $json_data['sales_representative_id'];
		$sr_code = $json_data['sales_representative_code'];
				...
				if (is_numeric($val['temp_id'])) {
					...
					if ($this->DistOutlet->save($data)) {
						...
						if ($val['images']) {
							...
							$val['images'] = json_decode($val['images'], 1);
	...
	public function dist_create_order()
	{
		...
		if (!empty($json_data['order_list'])) {
            ...
            $tso_id = $json_data['tso_id'];
            $ae_id = $json_data['ae_id'];
			$sale_type_id = 10;
            ...
            //change 
			$myfile = fopen($path . "dist_create_order_rnd.txt", "a") or die("Unable to open file!");
			fwrite($myfile, "\n" . $this->current_datetime() . ':' . $this->request->input());
			fclose($myfile);
			...
			foreach ($json_data['order_list'] as $result) {
				...
				$thana_id = $result['thana_id'];
				$order_date = date('Y-m-d', strtotime($result['order_date']));
				$count_order_info = $this->DistOrder->find('first', array(
					'conditions' => array(
						'DistOrder.dist_order_no' => $result['order_number']
					),
					'fields'=>array('DistOrder.id', 'DistOrder.dist_order_no'),
					'recursive'=>-1
				));

				$total_product_data = array();

				if (empty($count_order_info)) {
					...
					if ($this->DistOrder->save($OrderData)) {
						...
						$this->dist_oc_ec_calculation($result['gross_value'], $result['outlet_id'], $sr_code, $office_id, $OrderData['order_date'], $OrderData['order_time'], 2);
						...
						if ($order_id) {
							...
							foreach ($result['order_details'] as $val) {
								...
								$order_details['DistOrderDetail']['product_id'] = $val['product_id'];
								$order_details['DistOrderDetail']['virtual_product_id'] = $val['virtual_product_id'];
								$product_id = $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'];
								...
								$order_details['DistOrderDetail']['product_price_id'] = $val['price_id'];
								$order_details['DistOrderDetail']['product_combination_id'] = $val['combination_id'];
					...
					$old_order_id = $count_order_info['DistOrder']['id'];

					$this->dist_order_delete_new($old_order_id, $sr_code, $office_id, $store_id);
	...
	public function dist_order_delete_new($id = 0, $sr_code = 0, $office_id = 0, $store_id=0){
		...
		$gross_value = $order_id_arr['DistOrder']['gross_value'];
		$outlet_id = $order_id_arr['DistOrder']['outlet_id'];
		$order_date = $order_id_arr['DistOrder']['order_date'];
		$order_time = $order_id_arr['DistOrder']['order_time'];

		$this->dist_oc_ec_calculation($gross_value, $outlet_id, $sr_code, $office_id, $order_date, $order_time, 2);

		for ($order_detail_count = 0; $order_detail_count < count($order_id_arr['DistOrderDetail']); $order_detail_count++) {
			
			$product_id = $order_id_arr['DistOrderDetail'][$order_detail_count]['virtual_product_id'] ? $order_id_arr['DistOrderDetail'][$order_detail_count]['virtual_product_id'] : $order_id_arr['DistOrderDetail'][$order_detail_count]['product_id'];
	...
	public function dist_order_cancel($id = 0)
	{
		...
		$office_id = $json_data['office_id'];
		$distributor_id = $json_data['db_id'];
		$store_id = $json_data['store_id'];
		$sr_id = $json_data['sr_id'];
		$sr_code = $json_data['sr_code'];
		...
		if ($order_id_arr) {
			if ($order_id_arr['DistOrder']['status'] == 3) {
				...
				$this->DistOrder->id = $order_id_arr['DistOrder']['id'];
				if ($this->DistOrder->id) {
					$this->DistOrder->updateAll(
						...
						array('DistOrder.id' => $order_id_arr['DistOrder']['id']) //condition
					);
				}
	...
	{
		return true; //added for elemminating deadlock
	...
	public function dist_oc_ec_calculation($gross_value, $outlet_id, $sr_code, $office_id, $order_date, $order_time, $cal_type){
		...
		if ($gross_value > 0) {
			...
			$ocheck = $this->DistOrder->find('first', array(
				'conditions' => array(
						...
						if ($cal_type == 1) {
							if ($is_pharma_type == 1) {
								...
								$update_fields_arr = array(
									'DistSaleTargetMonth.outlet_coverage_pharma_achievement' => "DistSaleTargetMonth.outlet_coverage_pharma_achievement+1",
									'DistSaleTargetMonth.effective_call_pharma_achievement' => "DistSaleTargetMonth.effective_call_pharma_achievement+1"
								);

							} else if ($is_pharma_type == 0) {

								$update_fields_arr = array(
									'DistSaleTargetMonth.outlet_coverage_non_pharma_achievement' => "DistSaleTargetMonth.outlet_coverage_non_pharma_achievement+1",
									'DistSaleTargetMonth.effective_call_non_pharma_achievement' => "DistSaleTargetMonth.effective_call_non_pharma_achievement+1"
								);
	...
	public function dist_create_memo_old()
	{
		$this->loadModel('DistOrder');
		$this->loadModel('DistOrderDetail');
		$this->loadModel('DistMemo');
		...
		if (!empty($json_data['memo_list'])) {
			...
			$datasource = $this->DistMemo->getDataSource();
			...
			foreach ($json_data['memo_list'] as $result) {
				...
				$order_detials = $this->DistOrderDetail->find('all', array(
					...
					'fields' => array('DistOrderDetail.dist_order_id as dist_order_id', 'DistOrderDetail.product_id as product_id', 'DistOrderDetail.virtual_product_id as virtual_product_id', 'DistOrderDetail.measurement_unit_id as measurement_unit_id', 'DistOrderDetail.sales_qty as sales_qty', 'DistOrderDetail.price as price', 'DistOrderDetail.is_bonus as is_bonus', 'DistOrderDetail.product_price_id as product_price_id', 'DistOrder.order_date'),
					'recursive' => -1
				));
				...
				foreach ($order_detials as $order_detial) {
					...
					$s_order_detials = $this->DistOrderDeliveryScheduleOrderDetail->find('first', array(
						'conditions' => array(
							...
							'DistOrderDeliveryScheduleOrderDetail.virtual_product_id' => $order_detial[0]['virtual_product_id'],
					...
					$order_date = date('Y-m-d', strtotime($order_detial['DistOrder']['order_date']));
					if ($memo_date < $order_date) {

						$res['status'] = 0;
						$res['message'] = 'Memo date not less then order date!!!';
						$this->set(array(
							'memo' => $res,
							'_serialize' => array('memo')
						));
						return 0;
					} elseif ($order_date < date('Y-m-d', strtotime('-10days'))) {
						$res['status'] = 0;
						$res['message'] = 'Cannot create memo for 10days previous order!!!';
						$this->set(array(
							'memo' => $res,
							'_serialize' => array('memo')
						));
					...
					$order_products[($order_detial[0]['virtual_product_id'] ? $order_detial[0]['virtual_product_id'] : $order_detial[0]['product_id'])] = array(
						'product_id'			=> ($order_detial[0]['virtual_product_id'] ? $order_detial[0]['virtual_product_id'] : $order_detial[0]['product_id']),
						...
						'invoice_qty' 			=>	$order_detial[0]['sales_qty'],
					);


					$order_products2[($order_detial[0]['virtual_product_id'] ? $order_detial[0]['virtual_product_id'] : $order_detial[0]['product_id'])][$b_track] = array(
						'product_id'			=> ($order_detial[0]['virtual_product_id'] ? $order_detial[0]['virtual_product_id'] : $order_detial[0]['product_id']),
						'measurement_unit_id' 	=>	$order_detial[0]['measurement_unit_id'],
						'sales_qty' 			=>	$order_detial[0]['sales_qty'],
						'price' 				=>	$order_detial[0]['price'],
				...
				foreach ($result['memo_details'] as $val) {
					...
					$memo_products[($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'])][$val['price']][$val['is_bonus']] = array(
						'product_id'			=> ($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']),
						...
						//'product_price_id' 		=>	0,
						'order_sales_qty' 		=>	isset($order_products2[($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'])][$b_track]['sales_qty']) ? $order_products2[($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'])][$b_track]['sales_qty'] : 0,
						'invoice_qty' 			=>	isset($order_products2[($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'])][$b_track]['invoice_qty']) ? $order_products2[($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'])][$b_track]['invoice_qty'] : 0,
					);

					$memo_products2[($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'])][$b_track] = array(
						'product_id'			=> ($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']),
						'measurement_unit_id' 	=>	isset($val['measurement_unit_id']) ? $val['measurement_unit_id'] : 0,
						'sales_qty' 			=>	$val['quantity'],
						'price' 				=>	$val['price'],
					...
					try {
							...
							foreach ($order_products2 as $o_product_id => $or_data) {
								foreach ($or_data as $b_track => $o_val) {
									if (!isset($memo_products2[$o_product_id][$b_track])) {
										...
										$updateinventory = $this->dist_memo_update_current_inventory($base_quantity, $o_product_id, $store_id, 'add', 4, $result['memo_date'], $invoice_qty);
										if (!$updateinventory) {
											throw new Exception();
										}
									}
						...
						if ($market_info) {
							...
							if ($count == 0) {
								...
								if (!$this->DistMemo->save($MemoData)) {
									...
									if ($memo_id) {
										...
										foreach ($result['memo_details'] as $val) {
											...
											$memo_details['DistMemoDetail']['product_id'] = $val['product_id'];
											$memo_details['DistMemoDetail']['virtual_product_id'] = $val['virtual_product_id'];
											$product_id = $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'];
											$punits_pre = $this->search_array($product_id, 'id', $product_list);
											//pr($punits_pre);//die();
											...
											$memo_details['DistMemoDetail']['product_price_id'] = $val['price_id'];
											$memo_details['DistMemoDetail']['product_combination_id'] = $val['combination_id'];
											...
											$dbupdateinventory = $this->dist_memo_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, $tran_type_id, $result['memo_date'], $invoice_qty);

											if (!$dbupdateinventory) {
												throw new Exception();
											}
										}
									...
									if (!$this->DistOrder->updateAll(array('DistOrder.processing_status' => 2), array('DistOrder.id' => $dist_order_id))) {
										throw new Exception();
									}

									if (!$this->DistOrderDeliveryScheduleOrder->updateAll(array('DistOrderDeliveryScheduleOrder.processing_status' => 2), array('DistOrderDeliveryScheduleOrder.dist_order_no' => $dist_order_no))) {
										throw new Exception();
									}
								}
							} else {
								...
								$deleteoldmemo = $this->dist_memo_delete($old_memo_id, $sr_code, $office_id);
								//exit;
								...
								if (!$this->DistMemo->save($MemoData)) {
									...
									if ($memo_id) {
										...
										foreach ($result['memo_details'] as $val) {
											...
											$updatedbinventoy = $this->dist_memo_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 3, $result['memo_date'], $invoice_qty);
						...
						$datasource->rollback();
						$res['status'] = 0;
						$res['message'] = 'Memo created failed. Please try again.';
						/*$res[$res_array_count] = array(
	...
	public function dist_create_memo()
	{
		...
		if (!empty($json_data['memo_list'])) {
			...
			foreach ($json_data['memo_list'] as $result) {
				...
				$count_memo_check = $this->DistMemo->find('first', array(
					'conditions' => array(
						'DistMemo.dist_order_no' => $result['order_number']
					),
					'fields'=>array('DistMemo.id', 'DistMemo.dist_order_no', 'DistMemo.dist_memo_no'),
					'recursive'=>-1
				));
					...
					try{
						...
						if ($market_info) {
							...
							if (empty($count_memo_check)) {
								...
								$old_memo_id = $count_memo_check['DistMemo']['id'];
								
								$deleteoldmemo = $this->dist_memo_delete_new($old_memo_id, $sr_code, $office_id, $store_id);
	...
	public function dist_memo_delete_new($id = 0, $sr_code = 0, $office_id = 0, $store_id)
	{
		$delete_memo_variable = 0;
		...
		for ($memo_detail_count = 0; $memo_detail_count < count($memo_id_arr['DistMemoDetail']); $memo_detail_count++) {
			$product_id = $memo_id_arr['DistMemoDetail'][$memo_detail_count]['virtual_product_id'] ? $memo_id_arr['DistMemoDetail'][$memo_detail_count]['virtual_product_id'] : $memo_id_arr['DistMemoDetail'][$memo_detail_count]['product_id'];
		...
		$delete_memo_variable = 1;

		return $delete_memo_variable;
	...
	public function dist_order_ready_for_process_list()
	{
		...
		if ($order_date && ($order_date >= date('Y-m-d', strtotime('-10days')) || $json_data['sales_person_id'] == 21087)) {
			$conditions['DistOrder.order_date'] = $order_date;
		} else {
			if ($json_data['sales_person_id'] != 21087)
				$conditions['DistOrder.order_date >='] = date('Y-m-d', strtotime('-10days'));
		}
	...
	public function dist_order_delivery_schedule()
	{
		...
		foreach ($order_results as $key => $order_info) {
			...
			if ($previous_order_status['DistOrder']['status'] == 2 && $previous_order_status['DistOrder']['processing_status'] == 1) {
				$res_data[$res_array_count] = array(
					'dist_order_no' => $dist_order_no,
					'status' => 1,
				);
			...
			try {
				$datasource->begin();
				/* if (!$this->DistOrderDeliveryScheduleOrder->deleteAll(array('DistOrderDeliveryScheduleOrder.dist_order_no' => $dist_order_no), false)) {
					throw new Exception();
				}
				if (!$this->DistOrderDeliveryScheduleOrderDetail->deleteAll(array('DistOrderDeliveryScheduleOrderDetail.dist_order_no' => $dist_order_no), false)) {
					throw new Exception();
				} */


				$order_detail_results = $this->DistOrderDetail->find('all', array(
					'conditions' => array(
					...
					'joins' => array(
						...
						array(
							'alias' => 'Product',
							'table' => 'products',
							'conditions' => 'CASE WHEN DistOrderDetail.virtual_product_id is null OR DistOrderDetail.virtual_product_id=0 then DistOrderDetail.product_id else DistOrderDetail.virtual_product_id END=Product.id'
						),
						array(
							'alias' => 'ProductMeasurement',
							'table' => 'product_measurements',
							'type' => 'left',
							'conditions' => 'ProductMeasurement.product_id=Product.id and ProductMeasurement.measurement_unit_id=DistOrderDetail.measurement_unit_id'
						),
						array(
							'alias' => 'DistCurrentInventory',
							'table' => 'dist_current_inventories',
							'type' => 'left',
							'conditions' => 'DistCurrentInventory.product_id=Product.id and DistCurrentInventory.store_id=' . $store_id
						),
					),
					'fields' => array(
						'Product.base_measurement_unit_id',
						'Product.sales_measurement_unit_id',
						'DistOrderDetail.product_id',
						'DistOrderDetail.virtual_product_id',
						'SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base  is null then 1 else ProductMeasurement.qty_in_base END),0)) as qty',
						'MAX(DistCurrentInventory.qty) as qty'
					),
					'group' => array(
						'Product.base_measurement_unit_id',
						'Product.sales_measurement_unit_id',
						'DistOrderDetail.product_id',
						'DistOrderDetail.virtual_product_id HAVING SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base  is null then 1 else ProductMeasurement.qty_in_base END),0)) > CASE WHEN MAX(DistCurrentInventory.qty) is null THEN 0 ELSE MAX(DistCurrentInventory.qty) END',
					),
					'recursive' => -1
				));
				/* echo $this->DistOrder->getLastQuery();
				pr($order_detail_results);
				exit; */

				$sql = "
					DELETE FROM dist_order_delivery_schedule_order_details where dist_order_id=$order_id;
					INSERT INTO  dist_order_delivery_schedule_order_details
					(			
						[dist_order_delivery_schedule_id]
						,[dist_order_id]
						,[dist_order_no]
						,[measurement_unit_id]
						,[product_id]
					...
					select
						...
						$order_id order_id,
						'$dist_order_no' order_no,
						Product.sales_measurement_unit_id,
						DistOrderDetail.product_id,
						DistOrderDetail.virtual_product_id,
						ROUND(SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0))/(CASE WHEN ProductMeasurementSales.qty_in_base IS NULL THEN 1 ELSE ProductMeasurementSales.qty_in_base END),2,1) as order_qty,
						ROUND(CASE WHEN MAX(DistCurrentInventory.qty) >= SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0)) 
							THEN 
								SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0))
							ELSE
								CASE WHEN MAX(DistCurrentInventory.qty) is null THEN 0 ELSE MAX(DistCurrentInventory.qty) END
						END/(CASE WHEN ProductMeasurementSales.qty_in_base IS NULL THEN 1 ELSE ProductMeasurementSales.qty_in_base END),2,1) as invoice_qty,
						0 as is_bonus,
						CASE WHEN MAX(DistCurrentInventory.qty) >= SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0)) 
							THEN 
								1
							ELSE
								0
						END as status,
						'" . $this->current_datetime() . "' created_at,
						" . $json_data['sales_person_id'] . " created_by,
						'" . $this->current_datetime() . "' updated_at,
						" . $json_data['sales_person_id'] . " updated_by
					FROM dist_order_details AS DistOrderDetail
					JOIN products AS Product ON(CASE WHEN DistOrderDetail.virtual_product_id is null OR DistOrderDetail.virtual_product_id=0 then DistOrderDetail.product_id else DistOrderDetail.virtual_product_id END=Product.id)
					LEFT JOIN product_measurements AS ProductMeasurement ON( ProductMeasurement.product_id=Product.id AND ProductMeasurement.measurement_unit_id=DistOrderDetail.measurement_unit_id)
					LEFT JOIN product_measurements AS ProductMeasurementSales ON( ProductMeasurementSales.product_id=Product.id AND ProductMeasurementSales.measurement_unit_id=Product.sales_measurement_unit_id)
					LEFT JOIN dist_current_inventories AS DistCurrentInventory ON(DistCurrentInventory.product_id=Product.id AND DistCurrentInventory.store_id= $store_id)
					WHERE DistOrderDetail.dist_order_id=$order_id
					GROUP BY Product.base_measurement_unit_id,
							Product.sales_measurement_unit_id,
							DistOrderDetail.product_id,
							DistOrderDetail.virtual_product_id,
							ProductMeasurementSales.qty_in_base;
							
					DELETE FROM dist_order_delivery_schedule_orders where dist_order_no='$dist_order_no';";
				// echo $sql;
				if (!$this->DistOrder->query($sql)) {
					throw new Exception();
				}
				$processing_failed = 1;
				if (empty($order_detail_results)) {
					$processing_failed = 0;
					$sql = "UPDATE DistCurrentInventory 
							SET 
								DistCurrentInventory.qty=DistCurrentInventory.qty-order_qty.o_qty,
								DistCurrentInventory.invoice_qty=DistCurrentInventory.invoice_qty+order_qty.o_qty,
								DistCurrentInventory.transaction_type_id=3,
								DistCurrentInventory.transaction_date='" . date('Y-m-d') . "',
								DistCurrentInventory.updated_at=getdate()
							FROM dist_current_inventories AS DistCurrentInventory
							inner join(
								SELECT
									CASE WHEN DistOrderDetail.virtual_product_id is null OR DistOrderDetail.virtual_product_id=0 then DistOrderDetail.product_id else DistOrderDetail.virtual_product_id END as prod_id,
									SUM(ROUND((DistOrderDetail.sales_qty)*(CASE WHEN ProductMeasurement.qty_in_base IS NULL THEN 1 ELSE ProductMeasurement.qty_in_base END),0)) as o_qty
									...
									GROUP BY 
										DistOrderDetail.product_id,
										DistOrderDetail.virtual_product_id
							)order_qty on order_qty.prod_id=DistCurrentInventory.product_id
							where DistCurrentInventory.store_id= $store_id
						";
					if (!$this->DistOrder->query($sql)) {
						throw new Exception();
					}
				...
				if (!$this->DistOrder->updateAll(
					array(
						'DistOrder.processing_status' => $processing_status,
						'DistOrder.status' => $status,
						'DistOrder.action' => 0
					),   //fields to update
				...
				if (!empty($order_into) && !$this->DistOrderDeliveryScheduleOrder->saveAll($order_into)) {
					throw new Exception();
				}
				// throw new Exception();
				$datasource->commit();
	...
	private function dist_stock_check($store_id, $order_id)
	{
		...
		foreach ($order_datas as $data) {
			$product_id_chk = ($data['DistOrderDetail']['virtual_product_id'] ? $data['DistOrderDetail']['virtual_product_id'] : $data['DistOrderDetail']['product_id']);
			if ($data['DistOrderDetail']['measurement_unit_id']) {
				...
				if ($pro_measurement_info && $pro_measurement_info['ProductMeasurement']['qty_in_base'] && $product_info) {
					$prev_qty = isset($order_detail[$product_id_chk]) ? $order_detail[$product_id_chk] : 0;
					$order_detail[$product_id_chk] = $prev_qty + ($data['0']['sales_qty'] * $pro_measurement_info['ProductMeasurement']['qty_in_base']) / $product_info['ProductMeasurement']['qty_in_base'];
				} elseif (empty($pro_measurement_info) && $product_info) {
					$prev_qty = isset($order_detail[$product_id_chk]) ? $order_detail[$product_id_chk] : 0;
					$order_detail[$product_id_chk] = $prev_qty + ($data['0']['sales_qty']) / $product_info['ProductMeasurement']['qty_in_base'];
				} else {
					$prev_qty = isset($order_detail[$product_id_chk]) ? $order_detail[$product_id_chk] : 0;
					$order_detail[$product_id_chk] = $prev_qty + $data['0']['sales_qty'];
				}
			} else {
				$prev_qty = isset($order_detail[$product_id_chk]) ? $order_detail[$product_id_chk] : 0;
				$order_detail[$product_id_chk] = $prev_qty + $data['0']['sales_qty'];
		...
		try {
			foreach ($cancelled_order_list as $cancelled_orders) {
				...
				if ($this->DistOrderDeliveryScheduleOrder->dist_order_no) {
					if (!$this->DistOrderDeliveryScheduleOrder->updateAll(
				...
				foreach ($order_results as $key => $order_info) {
					...
					$product_id = $order_info['DistOrderDeliveryScheduleOrderDetail']['virtual_product_id'] ? $order_info['DistOrderDeliveryScheduleOrderDetail']['virtual_product_id'] : $order_info['DistOrderDeliveryScheduleOrderDetail']['product_id'];
					...
					if ($prev_processing_status == 1 || $prev_processing_status == 2) {
						if (!$this->dist_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 4, $order_date, $invoice_qty)) {
							throw new Exception();
						}
						/* if (!$this->dist_booking_update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 4, $order_date, $booking_bonus_qty)) {
							throw new Exception();
						} */
					}
				}
	...
	public function dist_process_order_list()
	{
		...
		// if ($order_date && $order_date >= date('Y-m-d', strtotime('-10days'))) {
		if ($order_date  && ($order_date >= date('Y-m-d', strtotime('-10days')) || $json_data['sales_person_id'] == 21087)) {
			$conditions['DistOrder.order_date'] = $order_date;
		} else {
			if ($json_data['sales_person_id'] != 21087)
	...
	public function dist_process_order_list_for_delivery()
	{
		...
		if ($route_id) $conditions['DistOrder.dist_route_id'] = $route_id;
		$start_date = isset($json_data['start_Date']) ? $json_data['start_Date'] : 0;
		if ($start_date && $start_date >= date('Y-m-d', strtotime('-10days'))) {
			$conditions['DistOrder.order_date >='] = $start_date;
		} else {
			$conditions['DistOrder.order_date >='] = date('Y-m-d', strtotime('-10days'));
		}
		...
		foreach ($order_list as $m) {
			...
			foreach ($m['DistOrderDetail'] as $each_order_details) {
				...
				//for invoice qty
				$product_id = $each_order_details['product_id'];
				$virtual_product_id = $each_order_details['virtual_product_id'];
	...
	public function dist_order_stock_info()
	{
		...
		/*---------------------------- Mac check --------------------------------*/
		$mac_check = $this->mac_check($json_data['mac'], $json_data['sales_person_id'],0);
		if (!$mac_check) {
			$mac['status'] = 0;
			$mac['message'] = 'Mac Id Not Match';
			$res = $mac;
		...
		foreach ($stock_info as $val) {
			unset($order_conditions['DistOrderDetail.virtual_product_id']);
			unset($order_conditions['DistOrderDetail.product_id']);
			if ($val['Product']['is_virtual'] > 0) {

				$order_conditions['DistOrderDetail.virtual_product_id'] = $val['Product']['id'];
			} else {
				$order_conditions['DistOrderDetail.virtual_product_id'] = 0;
	...
	public function dist_price_list()
	{
		...
		//if($last_update_date!='all')$conditions['Product.updated_at >'] = $last_update_date;
		$products = $this->Product->find('all', array(
			'fields' => array('Product.id', 'Product.name', 'Product.product_code', 'Product.product_category_id', 'Product.sales_measurement_unit_id', 'Product.product_type_id', 'Product.updated_at', 'Product.is_pharma', 'Product.order', 'Product.is_injectable', 'Product.is_virtual', 'Product.parent_id'),
			'conditions' => $conditions,
			'order' => array('Product.order' => 'asc'),
			'recursive' => -1
		));
		...
		/*
		comment by naser. because as new pricing call by new api.
	...
	public function dist_product_sales_report()
	{
		...
		foreach ($products as $product_id => $product_name) {
			...
			$product_total_sales = $this->DistMemo->find(
				'all',
				array(
					'conditions' => $conditions,
					'joins' => array(
			...
			$product_total_oc = $this->DistMemo->find(
				'all',
				array(
					'conditions' => $conditions,
					'joins' => array(
	...
	public function dist_product_order_report()
	{
		...
		foreach ($products as $product_id => $product_name) {
			...
			$product_total_sales = $this->DistOrder->find(
				'all',
				array(
					'conditions' => $conditions,
					'joins' => array(
			...
			$product_total_oc = $this->DistOrder->find(
				'all',
				array(
					'conditions' => $conditions,
					'joins' => array(
			...
			//pr($conditions);
			$order_results = $this->DistOrder->find(
				'all',
				array(
					'conditions' => $conditions,
					'joins' => array(
	...
	public function dist_sales_targets()
	{
		...
		$sr_product_targets = $this->DistSaleTargetMonth->find(
			'all',
			array(
	...
	public function get_outlet_group()
	{
		...
		/*---------------------------- Mac check --------------------------------*/
		$mac_check = $this->mac_check($json_data['mac'], $json_data['so_id'],0);
		/*if(!$mac_check)
        {
    ...
    */
	...
	/*---------------- new product pricing and bonus policy api ----------------------*/
	public function get_product_combination_v2()
	{
		...
		$conditions[] = array('ProductPriceSectionV2.is_sr' => 1);
		...
		foreach ($product_combination as $key => $p_data) {
			...
			$special_group_data = $this->ProductPriceOtherForSlabsV2->find('all', array(
				'conditions' => array(
					'ProductPriceOtherForSlabsV2.price_for' => 2,
			...
			foreach ($special_group_data as $data) {
				...
				$special_group_details = $this->SpecialGroup->find('all', array(
					...
					'fields' => array('SPO.reffrence_id'),
					'recursive' => -1
				));
				if ($special_group_details) {
					$assign_group[] = $data[0];
				} else {
					$outlet_group_data = $this->get_special_group_outlet_group_by_territory_id($group_id, $sr_id);
			...
			$p_data['ProductCombinationsV2']['price'] = $p_data['ProductCombinationsV2']['sr_price'];
	...
	public function get_special_group()
	{
		...
		foreach ($special_group_data as $data) {
			...
			$outlet_group_id = $this->get_special_group_outlet_group_by_territory_id($group_id, $sr_id);
			if ($outlet_group_id) {
				$data_array = $data;
			}
			if ($data_array) {
				...
				if (isset($special_group_territory_details) || $special_group_details) {
					if ($outlet_group_id)
						$other .= ' OR ';
	...
	public function get_combination_list()
	{
		...
		// $conditions[] = array('(CombinationsV2.create_for=2 OR CombinationsV2.create_for=5 OR CombinationsV2.create_for=7)');
	...
	private function get_special_group_outlet_group_by_territory_id($group_id, $sr_id)
	{
		$this->loadModel('SpecialGroupOtherSetting');
		$special_group_outlet_group_details = $this->SpecialGroupOtherSetting->find('list', array(
			...
			'joins' => array(
				...
				array(
					...
					'table' => 'dist_sr_route_mappings',
					'type' => 'INNER',
					'conditions' => 'Outlet.dist_route_id = DistSrRouteMapping.dist_route_id'
				)
			),
	...
	public function get_policy_list_v2()
	{
		...
		$this->DiscountBonusPolicy->unbindModel(array(
			'hasMany' => array(
				...
				'DiscountBonusPolicyToOutletGroupSo',
				'DiscountBonusPolicyToOutletCategorySo',
				'DiscountBonusPolicyToExcludingOutletGroupSo',
		...
		$policy_list = $this->DiscountBonusPolicy->find('all', array(
			...
			'joins' => array(
				array(
					...
					'alias' => 'Grp',
					'type' => 'Left',
					'conditions' => 'Grp.discount_bonus_policy_id=DiscountBonusPolicy.id and Grp.create_for=2 and Grp.for_so_sr=2'
				),
				array(
					'table' => 'discount_bonus_policy_settings',
					'alias' => 'GrpOg',
					'type' => 'Left',
					'conditions' => 'GrpOg.discount_bonus_policy_id=DiscountBonusPolicy.id and GrpOg.create_for=3 and GrpOg.for_so_sr=2'
				),
				array(
					'table' => 'outlet_group_to_outlets',
					'alias' => 'OutletGroup',
		...
		$special_policy_list = $this->DiscountBonusPolicy->find('all', array(
			...
			'joins' => array(
				array(
					...
					'alias' => 'Grp',
					'type' => 'inner',
					'conditions' => 'Grp.discount_bonus_policy_id=DiscountBonusPolicy.id and Grp.create_for=1 and Grp.for_so_sr=2'
				),
		...
		foreach ($special_policy_list as $data) {
			$group_id = $data['DiscountBonusPolicyToSpecialGroupSr'][0]['reffrence_id'];
			$special_group_details = $this->SpecialGroup->find('all', array(
				'conditions' => array(
					'SpecialGroup.id' => $group_id,
					'SPO.reffrence_id' => $office_id
	...
	public function get_bonus_campaign_list()
	{
		$this->loadModel('Product');
		$this->loadModel('BonusCampaign');
		...
		if (!empty($product_id)) {
			$conditions[] = array('BonusCampaignProductList.product_id' => $product_id);
		}
		if (!empty($start_date)) {
			$conditions[] = array('BonusCampaign.start_date >=' => $start_date);
		}
		if (!empty($end_date)) {
			$conditions[] = array('BonusCampaign.start_date <=' => $end_date);
		}

		$conditions[] = array('BonusCampaign.is_sr' => 2);


		$list = $this->BonusCampaign->find('all', array(
			'fields' => array(
				'BonusCampaign.id',
				'BonusCampaign.start_date',
				'BonusCampaign.end_date',
				'BonusCampaign.attachment',
				'BonusCampaignProductList.bonus_campaign_id'
			),
			'conditions' => $conditions,
			'joins' => array(
				array(
		...
		if (!empty($list)) {
			...
			foreach ($list as $key => $val) {
				...
				$pname = $this->get_bonus_camping_product_list($val['BonusCampaign']['id']);
				if (!empty($val['BonusCampaign']['attachment'])) {
					$link = BASE_URL . 'app/webroot/img/bonus_attachment/' . $val['BonusCampaign']['attachment'];
				} else {
					$link = '';
				}

				$bonus_details = $this->get_bonus_campaing_details($val['BonusCampaign']['id']);

				$rs[] = array(
					'id' => $val['BonusCampaign']['id'],
					'product_list' => $pname,
					'start_date' => $val['BonusCampaign']['start_date'],
					'end_date' => $val['BonusCampaign']['end_date'],
		...
		$this->set(array(
			'bonus_cam_info' => $rs,
			'_serialize' => array('bonus_cam_info')
		));
	}

	function get_bonus_campaing_details($id)
	{


		$this->loadModel('BonusCampaign');

		$options = array('conditions' => array('BonusCampaign.id' => $id));

		$detials = $this->BonusCampaign->find('first', $options);

		return $detials['BonusCampaign']['bonus_details'];
	}

	function get_bonus_camping_product_list($id)
	{
