 ...
 */
class DBCommissionReportsController extends AppController {
    ...
    public function admin_index() {
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($office_id)) {
                $string .= " and do.office_id=$office_id";
                $join .= "  INNER JOIN dist_tso_mappings   ON do.office_id= dist_tso_mappings.office_id 
                and do.distributor_id= dist_tso_mappings.dist_distributor_id";
            ...
            if (!empty($dist_tso_id)) {
                $string .= " and do.distributor_id in (select distinct dist_distributor_id from dist_tso_mappings 
                where office_id=$office_id and dist_tso_id=$dist_tso_id)";
            }
            if (!empty($dist_distributor_id)) {
                $string .= " and do.distributor_id=$dist_distributor_id";
            }
            ...
            if (!empty($product_id)) {
                $string .= " and dod.product_id=$product_id";
            }
			...
			$sql = "SELECT 
                    ...
                    count(distinct(do.dist_order_no)) as number_of_challans,
                    dist.name as distributor_name,
                    do.distributor_id,
                    SUM(dod.sales_qty) as total_challan_qty,
                    SUM(dod.sales_qty) as  total_received_qty,
                    SUM(dod.sales_qty*dod.price) as price,
                    products.id
                    $field
                from dist_orders AS do 
                INNER JOIN dist_order_details AS dod on do.id=dod.dist_order_id 
                INNER JOIN offices ON do.office_id= offices.id
                INNER JOIN products  ON dod.product_id= products.id
                INNER JOIN dist_distributors as dist  ON do.distributor_id= dist.id
                $join
                where  do.order_date between $date_from and $date_to and dod.price !=0
                $string 
                group by offices.parent_office_id,offices.office_name,offices.id$group,do.distributor_id,dist.name,products.id 
                ORDER BY offices.parent_office_id,offices.id,do.distributor_id,distributor_name,products.id
                ";
			//pr($sql);die();
            $data = $this->DistOrder->query($sql);
			
            if ($report_type == 2) {
                foreach ($data as $key => $value) {
                    $tso_id = $this->get_tso_name($value[0]['office_id'], $value[0]['distributor_id']);
                    !isset($sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['number_of_challans']) ? $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['number_of_challans'] = 0 : [];
                    !isset($sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['tp']) ? $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['tp'] = 0 : [];
                    !isset($sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['commissions']) ? $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['commissions'] = 0 : [];
                    !isset($sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['dp']) ? $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['dp'] = 0 : [];
                    !isset($sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['office_name']) ? $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['office_name'] = 0 : [];
                    !isset($sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['parent_office_id']) ? $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['parent_office_id'] = 0 : [];

                    $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['number_of_challans'] += $value[0]['number_of_challans'];
                    $tp = $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['tp'] += $value[0]['price'];
                    $commission = $this->get_commissions($date_from_plain, $date_to_plain, $value[0]['price'], $value[0]['office_id'], $value[0]['distributor_id'], $value[0]['id']);
                    $comm = $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['commissions'] += $commission;
                    $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['dp'] = $tp - $comm;
                    $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['office_name'] = $value[0]['office_name'];
                    $sanitize_array[$value[0]['office_id']][$tso_id][$value[0]['distributor_id']]['parent_office_id'] = $value[0]['parent_office_id'];
                }
            }
			
            //pr($sql);pr($this->request->data);pr($data);pr($sanitize_array);die();
            $offices = $this->Office->find('list', array(
            ...
            if(!empty($office_id)){
                ...
                /*$ae_info=$this->DistDistributor->query("SELECT 
                    ...
                    GROUP BY dist_area_executives.id,dist_area_executives.name 
                        ");*/
				$ae_info=$this->DistAreaExecutive->query("SELECT 
                        dist_area_executives.id as id,
                        dist_area_executives.name as name
                    from dist_area_executives 
                    where dist_area_executives.office_id=$office_id 
                        and dist_area_executives.is_active = 1
                    GROUP BY dist_area_executives.id,dist_area_executives.name 
                        ");
    ...
    public function get_commissions($date_from = '2019-04-10', $date_to = '2019-04-16', $tp = '123.00', $office_id = 16, $dist_distributor_id = 60, $product_id = 29) {
        ...
        if (count($date_array) == 0) {
            ...
            foreach ($date_array as $key => $value) {
                ...
                $distChallans = $this->DistOrder->query("SELECT 
                    do.office_id,
                    do.distributor_id,
                    SUM(dod.sales_qty) as challan_qty,
                    SUM(dod.sales_qty*dod.price) as price,
                    dod.product_id
                    from dist_orders AS do 
                    INNER JOIN dist_order_details AS dod on do.id=dod.dist_order_id 
                    where  do.order_date between $starting_date and $ending_date 
                    and do.office_id=$office_id 
                    and do.distributor_id=$dist_distributor_id 
                    and dod.product_id=$product_id
                    and dod.price !=0  
                    group by do.office_id,do.distributor_id,dod.product_id
                    ");
    ...
    public function get_total_challan($office_id=22, $dist_distributor_id=4,$date_from='2018-04-01',$date_to='2019-04-30') {
        ...
        $this->loadModel('DistChallan');
        $this->loadModel('DistOrder');
        $data = $this->DistOrder->query("
                    select distinct(do.dist_order_no),count(distinct(do.dist_order_no)), 
                    SUM(count(distinct(do.dist_order_no))) OVER() AS distinct_count
                    from dist_orders as do  
                    where do.order_date between $date_from_filter and $date_to_filter
                    and do.office_id=$office_id 
                    and do.distributor_id=$dist_distributor_id
                    group by do.office_id,do.distributor_id,do.dist_order_no,do.order_date");
...
where dist_tso_mappings.office_id=$office_id and dist_tso_mappings.dist_distributor_id=$dist_distributor_id");
    ...
    public function get_tso() {
        ...
        $data = $this->DistTso->find('all', array(
            'conditions' => array(
                ...
                'DistTso.dist_area_executive_id' =>$dist_area_executive_id
            ),
    ...
    public function get_area_executive() {
        $this->loadModel('DistAreaExecutive');
        $office_id = $this->request->data['office_id'];
        $data = $this->DistAreaExecutive->find('all', array(
            'conditions' => array(
