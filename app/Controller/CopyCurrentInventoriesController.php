 ...
 */
class CopyCurrentInventoriesController extends AppController
{
	...
	public function admin_index()
	{
		...
		//pr($this->request->data['CurrentInventory']);
		...
		$stores = $this->Store->find('all', array('conditions' => array('Store.id' => $srore_keys),'order' => array('Store.name' => 'asc'),'recursive' => 0));

		$store_all=array();
		foreach ($stores as $each_st)
		{
			$id=$each_st['Store']['id'];
			$name=$id=$each_st['Store']['id']."".$each_st['Store']['id'];
			$store_all[$id]=$name;
		}*/
		//pr($stores);exit;
