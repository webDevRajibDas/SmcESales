 ...
 */
class DistGroupWiseDiscountBonusPoliciesController extends AppController {
 ...
 * @var array
 */
 	public $uses = array('GroupWiseDiscountBonusPolicy','OutletGroup', 'Thana', 'Market', 'SalesPerson', 'Outlet', 'Product', 'GroupWiseDiscountBonusPolicyToOutlet', 'GroupWiseDiscountBonusPolicyToOutletGroup', 'GroupWiseDiscountBonusPolicyToOffice','GroupWiseDiscountBonusPolicyToOutletCategory','GroupWiseDiscountBonusPolicyOptionPriceSlab','GroupWiseDiscountBonusPolicyProduct','GroupWiseDiscountBonusPolicyOption','DistOutletCategory', 'GroupWiseDiscountBonusPolicyOptionBonusProduct');
 ...
 */
	public function admin_add() 
	{
		...
		$p_con =array('Product.is_distributor_product' => 1);
		$products = $this->Product->find('list', array('conditions' => $p_con, 'order'=>array('Product.order' => 'ASC')));
		$this->html = '';
		...
		if ($this->request->is('post')) {
			...
			$data_policy['GroupWiseDiscountBonusPolicy']['is_distributor'] = 1;
 ...
 */
	...
	public function admin_get_slab_list()
	{
		...
		/*------- start get prev effective date and next effective date ---------*/
		$effective_prev_date = $this->DistSrProductCombination->find('all',
			array(
				'conditions' => array(
					'DistSrProductCombination.product_id' => $product_id,
					'DistSrProductCombination.effective_date <=' => $current_date,
					'DistSrProductCombination.combination_id' => 0,
					),
				'fields' => array('DistSrProductCombination.effective_date')
			)
		);
		if(!empty($effective_prev_date)){
				$prev_short_list = array();
				foreach($effective_prev_date as $prev_date_key => $prev_date_val){
					array_push($prev_short_list,$prev_date_val['DistSrProductCombination']['effective_date']);
				}
				asort($prev_short_list);
				$prev_date = end($prev_short_list);
		}
		$effective_next_date = $this->DistSrProductCombination->find('all',
			array(
				'conditions' => array(
						'DistSrProductCombination.product_id' => $product_id,
						'DistSrProductCombination.effective_date >' => $current_date,
		...
		if(!empty($effective_next_date)){
				...
				foreach($effective_next_date as $next_date_key => $next_date_val){
					array_push($next_short_list,$next_date_val['DistSrProductCombination']['effective_date']);
				}
				rsort($next_short_list);
				$next_date = end($next_short_list);
		}
		if(isset($next_date)){
			$conditions['DistSrProductCombination.effective_date <'] = $next_date;
		}
		if(isset($prev_date)){
			$conditions['DistSrProductCombination.effective_date >='] = $prev_date;
		}
		$conditions['DistSrProductCombination.combination_id'] = 0;
		
		if(!empty($product_id))
		{
			$slab_list = $this->DistSrProductCombination->find('all', array(
				'conditions' => $conditions,
				'order' => array('DistSrProductCombination.effective_date' => 'desc'),
				'recursive' => -1
			));
		}else{
			$slab_list = '';
