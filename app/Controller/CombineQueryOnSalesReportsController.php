 ...
 */
class CombineQueryOnSalesReportsController extends AppController
{
	...
	public function admin_index($id = null)
	{
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			// pr($request_data);
			$request_data1 = $this->request->data['CombineQueryOnSalesReports'];
			//pr($request_data1);
			$product_rows = array();
			$product_ids = array();
			...
			$date_from = date('Y-m-d', strtotime($request_data['CombineQueryOnSalesReports']['date_from']));
			$date_to = date('Y-m-d', strtotime($request_data['CombineQueryOnSalesReports']['date_to']));
			$this->set(compact('date_from', 'date_to'));

			$type = $this->request->data['CombineQueryOnSalesReports']['type'];
			$this->set(compact('type'));

			$region_office_id = isset($this->request->data['CombineQueryOnSalesReports']['region_office_id']) != '' ? $this->request->data['CombineQueryOnSalesReports']['region_office_id'] : $region_office_id;
			...
			$office_id = isset($this->request->data['CombineQueryOnSalesReports']['office_id']) != '' ? $this->request->data['CombineQueryOnSalesReports']['office_id'] : $office_id;
			$this->set(compact('office_id'));

			$territory_id = isset($this->request->data['CombineQueryOnSalesReports']['territory_id']) != '' ? $this->request->data['CombineQueryOnSalesReports']['territory_id'] : 0;
			$this->set(compact('territory_id'));

			$so_id = isset($this->request->data['CombineQueryOnSalesReports']['so_id']) != '' ? $this->request->data['CombineQueryOnSalesReports']['so_id'] : 0;
			$this->set(compact('so_id'));

			/*$unit_type = $this->request->data['CombineQueryOnSalesReports']['unit_type'];
			...
			$outlet_category_id = isset($this->request->data['CombineQueryOnSalesReports']['outlet_category_id']) != '' ? $this->request->data['CombineQueryOnSalesReports']['outlet_category_id'] : 0;
			$results = array();
			foreach ($product_ids as $key_p => $pid) {
				...
				if ($pid) $conditions['MemoDetail.product_id'] = $pid;
				...
				if ($product_rows[$key_p]['memo_total'] == 'permemo') {
					$group[] = 'MemoDetail.memo_id';
					$fields[] = 'MemoDetail.memo_id';
				}
				if ($product_rows[$key_p]['volume_or_value'] == '1') {
					if ($product_rows[$key_p]['compotator'] == '>') {
						$group[count($group) - 1] .= ' HAVING SUM(MemoDetail.sales_qty) > ' . $product_rows[$key_p]['qty'];
						...
						$group[count($group) - 1] .= ' HAVING SUM(MemoDetail.sales_qty) BETWEEN ' . $product_rows[$key_p]['qty'] . ' AND ' . $product_rows[$key_p]['qty2'];
					}
				} else if ($product_rows[$key_p]['volume_or_value'] == '2') {
					if ($product_rows[$key_p]['compotator'] == '>') {
						$group[count($group) - 1] .= ' HAVING SUM(MemoDetail.sales_qty*MemoDetail.price) > ' . $product_rows[$key_p]['qty'];
					} elseif ($product_rows[$key_p]['compotator'] == '<') {
						$group[count($group) - 1] .= ' HAVING SUM(MemoDetail.sales_qty*MemoDetail.price) < ' . $product_rows[$key_p]['qty'];
					} elseif ($product_rows[$key_p]['compotator'] == '>=') {
						$group[count($group) - 1] .= ' HAVING SUM(MemoDetail.sales_qty*MemoDetail.price) >= ' . $product_rows[$key_p]['qty'];
						...
						$group[count($group) - 1] .= ' HAVING SUM(MemoDetail.sales_qty*MemoDetail.price) BETWEEN ' . $product_rows[$key_p]['qty'] . ' AND ' . $product_rows[$key_p]['qty2'];
					}
				}
				$q_results = $this->Memo->find('all', array(
					...
					'order' => array('sales_qty desc', 'Outlet.name asc'),
				...
				foreach ($q_results as $data) {
					$report_details_data[] = array(
						...
						'categories' => $outlet_categories[$data['Outlet']['category_id']],
						'mkt_thana_dis' => $data['Market']['name'] . '-' . $data['Thana']['name'] . '-' . $data['District']['name'],
						'office_name' => $offices[$data['Memo']['office_id']],
