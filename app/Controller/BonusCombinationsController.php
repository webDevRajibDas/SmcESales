 ...
 */
class BonusCombinationsController extends AppController {
 ...
 * @var array
 */
 	public $uses = array('OpenCombination', 'OpenCombinationProduct', 'Product','ProductType');
 ...
 */
	...
	public function admin_index($id = null) 
	{
		...
		if(isset($id))
		{
			...
			$conditions = array('OpenCombination.is_bonus' => 1, 'OC.product_id' => $product_id);
		}
		else
		{
			$joins = array();
			$conditions = array('OpenCombination.is_bonus' => 1);
		}
	...
	public function admin_add($product_id = null) 
	{
		...
		if ($this->request->is('post')) 
		{
			...
			if($this->request->data['OpenCombination']['product_id'])
			{
				...
				$this->request->data['OpenCombination']['is_bonus'] = 1; 
		...
		$this->set(compact('product_list', 'products', 'product_id','productTypes'));
 ...
 */
	public function admin_edit($id = null, $product_id = '') 
	{
		...
		$product_list = $this->html;
		$this->set(compact('slab_list', 'product_list', 'products','productTypes'));
	}
 ...
 */
	...
	public function get_product_by_type_id(){
		$rs = array(array('id' => '', 'name' => '---- Select -----'));
		$product_type_id = $this->request->data['product_type_id'];
		$conditions=array();
		if($product_type_id)
		{
			$conditions=array('Product.product_type_id' => $product_type_id);
		}
        $product_list = $this->Product->find('all', array(
	...
	public function get_product_list_by_type_id($product_type_id){
		// $product_type_id = $this->request->data['product_type_id'];
		$conditions=array();
		if($product_type_id)
		{
