 ...
 */
class ApiDataRetrivesController extends AppController {
    ...
    public function get_product_list() {
        ...
        $products = $this->Product->find('all', array(
            'fields' => array('Product.id', 'Product.name', 'Product.product_code', 'Product.product_category_id', 'Product.sales_measurement_unit_id', 'Product.product_type_id', 'Product.updated_at', 'Product.is_pharma', 'Product.order'),
            'conditions' => $conditions,
    ...
    public function get_products() {
        ...
        $products = $this->Product->find('all', array('fields' => array('Product.id', 'Product.name', 'Product.product_code', 'Product.product_category_id', 'Product.sales_measurement_unit_id', 'Product.product_type_id', 'Product.updated_at'), 'conditions' => array('Product.updated_at >' => $last_update_date), 'order' => array('Product.id' => 'asc'), 'recursive' => -1));
	...
	*/
    ...
    function get_challan_quantity($product_id, $batch_no,$expire_date,$array) {
        foreach ($array as $val) {
            if ($val['product_id'] == $product_id && $val['batch'] == $batch_no && $expire_date==$val['expire_date']) {
            ...
            if (is_numeric($json_data['market_id']) || true) {
                ...
                if ($count == 0) {
                    ...
                    foreach ($json_data['memo_details'] as $val) {
                        ...
                        $product_price_id = 0;
                        if ($val['price'] > 0) {
                            $product_price_id = $this->get_product_price_id($val['product_id'], $val['price'], $all_product_id);
                        }
    ...
    public function create_payment() {
        ...
        if (!empty($json_data['payment_list'])) {
            ...
            foreach ($json_data['payment_list'] as $val) {
                ...
                $data['type'] = $val['type'];
                $data['bank_account_id'] = $val['bank_branch_id'];
    ...
    public function update_credit_collection() {
        $this->LoadModel('SoCreditCollection');
        $json_data = $this->request->input('json_decode', true);
    ...
    public function create_installment_no() {
        ...
        foreach ($json_data as $data) {
            ...
            $data_insert['is_pushed'] = $data['isPushed'];
    ...
    public function update_territory_wise_collection_deposit_balance($so_id) {
        ...
        foreach ($instrument_type as $key => $ins_data) {
            $collection_data = $this->Collection->find('first', array(
                ...
                'conditions' => array('SalesPerson.territory_id' => $territory_id, 'Collection.type' => $key),
                'joins' => array(
                    array(
                        'table' => 'sales_people',
                        'alias' => 'SalesPerson',
            ...
            $deposit_data = $this->Deposit->find('first', array(
                ...
                'conditions' => array('SalesPerson.territory_id' => $territory_id, 'Deposit.type' => $key),
                'joins' => array(
                    array(
                        'table' => 'sales_people',
                        'alias' => 'SalesPerson',
