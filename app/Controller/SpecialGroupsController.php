 ...
 */
class SpecialGroupsController extends AppController {
 ...
 */
	...
	public $uses=array('SpecialGroup','OutletGroup','OutletCategory','SpecialGroupOtherSetting');
 ...
 */
	public function admin_index() {
		...
		$this->paginate = array('order' => array('SpecialGroup.id' => 'DESC'),'conditions'=>array('SpecialGroup.is_dist'=>0));
 ...
 */
	public function admin_add() {
		...
		if ($this->request->is('post')) {
			...
			$this->request->data['SpecialGroup']['is_dist'] =0;
			...
			try 
			{
			    ...
			    else
			    {
			    	...
			    	if(!empty($this->request->data['SpecialGroup']['territory_id']) && $this->request->data['SpecialGroup']['is_territory']==1)
			    	{
			    		$data['special_group_id']=$special_group_id;
			    		$data['create_for']=2; //2=territory
			    		foreach($this->request->data['SpecialGroup']['territory_id'] as $territory_id)
			    		{
			    			$data['reffrence_id']=$territory_id;
			    			$this->SpecialGroupOtherSetting->create();
			    			if(!$this->SpecialGroupOtherSetting->save($data))
						    {
						    	throw new Exception();
