 ...
 */
class ApiData189RetrivesController extends AppController
{
    ...
    public function user_login()
    {
        ...
        // $path = APP . 'logs/';
        // $myfile = fopen($path . "login_179.txt", "a") or die("Unable to open file!");
            ...
            if (!empty($user_info)) {
                ...
                if (empty($user_info['User']['mac_id'])) {
                    ...
                    if ($mac_exist) {
                        ...
                        if (!empty($store_info) && $user_info['SalesPerson']['territory_id']) {
                            $login['status'] = 1;
                            $login['message'] = 'Success';
                            $user_info['User']['outlet_delete_btn_hide_date'] = $version_info[0][0]['outlet_delete_btn_hide_date'];
                            /*territory id and his parent id Start */
                            $territory_info = $this->Territory->find('first', array(
                                'fields' => array('Territory.id', 'Territory.parent_id'),
                                'joins' => array(
                                    array(
                                        'table' => 'stores',
                                        'alias' => 'Store',
                                        'conditions' => 'Store.territory_id=Territory.id',
                                        'type' => 'Inner'
                                    ),
                                ),
                                'conditions' => array('Territory.id' => $user_info['SalesPerson']['territory_id']),
                                'recursive' => -1
                            ));

                            /*territory id and his parent id end */

                            /*territory id wise product group start */

                            $territory_id_product_group = $this->TerritoryProductGroup->find('all', array(
                                'fields' => array('TerritoryProductGroup.product_group_id', 'ProductGroup.name', 'ProductGroup.id'),
                                'conditions' => array('TerritoryProductGroup.territory_id' => $user_info['SalesPerson']['territory_id']),
                                'joins' => array(
                                    array(
                                        'alias' => 'ProductGroup',
                                        'table' => 'product_groups',
                                        'type' => 'INNER',
                                        'conditions' => 'ProductGroup.id = TerritoryProductGroup.product_group_id'
                                    )
                                ),
                                'recursive' => 0
                            ));
                            // pr($territory_id_product_group );exit;

                            $territory_wise_product_group = array();


                            foreach ($territory_id_product_group as $key => $value) {
                                // $products[$key] = $productlist[$key];
                                $territory_wise_product_group['territory_wise_data'][$key] = array(
                                    'id' => $value['ProductGroup']['id'],
                            ...
                            /*territory id wise product group end */

                            $res = array_merge($login, $user_info, $store_info, $office, $territory_info, $territory_wise_product_group);
                        } else {
                            $login['status'] = 0;
    ...
    /* ------------------- Start Territory wise product group ---------------------- */


    public function get_product_by_product_group()
    {
        ...
        $product_lists = $this->Product->find('list', array(
            'conditions' => array('Product.group_id' => array_values($group_id)),
        ...
        if ($product_lists) {
            ...
            // $all_product_id['id'] =  implode(",",$product_array);

        }


        $this->set(array(
            'product_group_list' => $product_array,
            '_serialize' => array('product_group_list')
        ));
    }
    ...
    public function get_outlet_list()
    {
        ...
        if ($is_spo_con == 1) {
            ...
            $this->LoadModel('SelectiveOutlet');
            $exitingoutletcatid = $this->SelectiveOutlet->find('list', array(
                'fields' => array('SelectiveOutlet.outlet_category_id')
            ));
            if (!empty($exitingoutletcatid)) {
                $conditions["Outlet.category_id"] = array_values($exitingoutletcatid);
            }
        }
        ...
        foreach ($outlet_list as $val) {
            ...
            $data['is_within_group'] = $val['Outlet']['is_within_group'];
    ...
    public function get_product_list()
    {
        ...
        $products = $this->Product->find('all', array(
            'fields' => array('Product.id', 'Product.name', 'Product.product_code', 'Product.product_category_id', 'Product.sales_measurement_unit_id', 'Product.product_type_id', 'Product.updated_at', 'Product.is_pharma', 'Product.order', 'Product.is_injectable', 'CASE WHEN ProductMeasurement.qty_in_base is null then 1 ELSE ProductMeasurement.qty_in_base END as qty_in_base', 'Product.base_measurement_unit_id', 'Product.is_virtual', 'Product.parent_id'),
            'conditions' => $conditions,
            'joins' => array(
                array(
    ...
    public function get_products()
    {
        ...
        $products = $this->Product->find('all', array('fields' => array('Product.id', 'Product.name', 'Product.product_code', 'Product.product_category_id', 'Product.sales_measurement_unit_id', 'Product.product_type_id', 'Product.updated_at', 'Product.is_injectable', 'Product.is_virtual', 'Product.parent_id'), 'conditions' => array('Product.updated_at >' => $last_update_date), 'order' => array('Product.id' => 'asc'), 'recursive' => -1));
            ...
            if (!empty($challans)) {
                foreach ($challans as $val) {
                    ...
                    foreach ($val['ChallanDetail'] as $cd) {
                        if ($cd['virtual_product_id'])
                            $cd['product_id'] = $cd['virtual_product_id'];
                ...
                try {
                    ...
                    if (!empty($challan_details)) {
                        foreach ($challan_details as $dval) {
                            ...
                            if ($dval['ChallanDetail']['virtual_product_id']) {
                                $dval['ChallanDetail']['product_id'] = $dval['ChallanDetail']['virtual_product_id'];
                            }
                        ...
                        if (!$this->ChallanDetail->saveAll($challan_details_array)) {
                            throw new Exception();
                        }
	...
	public function create_memo()
    {
        ...
        if (!empty($json_data)) {
            ...
            /*----------------------------------Check Memo Product is existing in Actual Group-----*/

            if (isset($json_data['Child_Territory_Id']) && isset($json_data['territory_id'])) {
                if ($json_data['Child_Territory_Id'] > 0) {

                    $product_group_territory_id = $json_data['Child_Territory_Id'];
                } else {

                    $product_group_territory_id = $json_data['territory_id'];
                }

                $product_group_check = $this->product_group_check($product_group_territory_id, $json_data['memo_details']);
                //  echo $product_group_check ? 'true' : 'false';exit;
            }
            ...
            if (!$product_group_check) {
                $res['status'] = 0;
                $res['message'] = 'product group not matched';
                $this->set(array(
                    'memo' => $res,
                    '_serialize' => array('memo')
                ));
            ...
            if (is_numeric($json_data['market_id']) || true) {
                ...
                /* ---------------- gross value validation : start (11-April-2022) ------------------ */
                $product_wise_value = 0;
                foreach ($json_data['memo_details'] as $val) {
                    $sales_qty = $val['sales_qty'];
                    // $price = $val['price'] > 0 ? ($val['price'] - (isset($val['discount_amount']) ? $val['discount_amount'] : 0)) : 0;
                    $price = $val['price'];
                    $product_wise_value += ($sales_qty * $price);
                }
                if (abs($json_data['gross_value'] - $product_wise_value) > 1) {
                    $res['status'] = 0;
                    $res['message'] = 'Gross value not match';
                    $this->set(array(
                        'memo' => $res,
                        '_serialize' => array('memo')
                    ));
                ...
                if ($count == 0) {
                    ...
                    $memo_data['is_distributor'] = 0;
                    $memo_data['child_territory_id'] = $json_data['Child_Territory_Id'];

                    $memo_data['total_discount'] = $json_data['total_discount'];
                    $datasource = $this->Memo->getDataSource();
                    try {
                        $datasource->begin();
                        if (!$this->Memo->save($memo_data)) {
                            throw new Exception();
                        }
                        ...
                        if ($punits['sales_measurement_unit_id'] == $punits['base_measurement_unit_id']) {
                            ...
                            $memo_details['virtual_product_id'] = $val['virtual_product_id'];
                            ...
                            if (!$val['price'] > 0) {
                                $b_product_id = $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'];
                                //$memo_date = date('Y-m-d', strtotime($json_data['memo_date']));
                            ...
                            if (!$this->update_current_inventory($sale_quantity, ($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']), $json_data['store_id'], 'deduct', 11, $memo_date)) {
                                throw new Exception();
                            }

                            // insert data into MemoDetail
                            $memo_details_array[] = $memo_details;
                            if (!$this->MemoDetail->saveAll($memo_details_array)) {
                                throw new Exception();
                            }
                        ...
                        if (!$this->MemoSyncHistory->updateAll(array(
                    ...
                    $datasource = $this->Memo->getDataSource();
                    try {
                        if (!$this->MemoDetail->deleteAll(array('MemoDetail.memo_id' => $memo_id))) {
                            throw new Exception();
                        }
                        if (!$this->Deposit->deleteAll(array('Deposit.memo_id' => $memo_id))) {
                            throw new Exception();
                        }
                        if (!$this->Collection->deleteAll(array('Collection.memo_id' => $memo_id))) {
                            throw new Exception();
                        }
                        if (!$this->InstallmentNo->deleteAll(array('InstallmentNo.memo_no' => $json_data['memo_no']), false)) {
                            throw new Exception();
                        }
                        if (!$this->StoreBonusCard->deleteAll(array('StoreBonusCard.memo_no' => $json_data['memo_no']))) {
                            throw new Exception();
                        }
                        if (!$this->SoCreditCollection->deleteAll(array('SoCreditCollection.memo_no' => $json_data['memo_no']), false)) {
                            throw new Exception();
                        }
                        ...
                        for ($memo_detail_count = 0; $memo_detail_count < count($memo_id_arr['MemoDetail']); $memo_detail_count++) {
                            ...
                            $update_type = 'add';
                            $memo_date = date('Y-m-d', strtotime($json_data['memo_date']));
                            if (!$this->update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 12, $memo_date)) {
                                throw new Exception();
                            }
    ...
    public function create_memo_bk()
    {
                ...
                if ($count == 0) {
                    ...
                    try {
                        ...
                        if ($json_data['gross_value'] >= 50000) {
                            ...
                            $notification_data['memo_amount'] = $json_data['gross_value'];
                            $notification_data['created_at'] = $this->current_datetime();
                            if (!$this->create_memo_nofification($notification_data)) {
                                throw new Exception();
                            }
                        } // end memo notifications
                        // $all_product_id=array_column($json_data['memo_details'],'product_id');
    ...
    function stock_check($store_id, $memo_details, $memo_id = 0)
    {
        ...
        foreach ($memo_details as $element) {
            //pr($element)      
            if ($element['virtual_product_id'] > 0) {
                $element['product_id'] = $element['virtual_product_id'];
            }
        ...
        if ($memo_id) {
            $prev_data = $this->MemoDetail->find('all', array(
                ...
                'fields' => array('SUM(MemoDetail.sales_qty) as sales_qty', 'MemoDetail.product_id', 'MemoDetail.virtual_product_id', 'MemoDetail.measurement_unit_id'),
                'group' => array('MemoDetail.product_id, MemoDetail.virtual_product_id,MemoDetail.measurement_unit_id'),
                'recursive' => -1
            ));
            $prev_memo_detail = array();
            foreach ($prev_data as $data) {
                if ($data['MemoDetail']['virtual_product_id']) {
                    $data['MemoDetail']['product_id'] = $data['MemoDetail']['virtual_product_id'];
                }
    ...
    function product_group_check($territory_id, $memo_details)
    {


        // pr($product_id_array);exit;
        $this->loadModel('TerritoryProductGroup');
        $this->loadModel('Product');
        ...
        $territory_id_product_group = $this->TerritoryProductGroup->find('list', array(
            'fields' => array('TerritoryProductGroup.id', 'TerritoryProductGroup.product_group_id'),
            'conditions' => array(
        ...
        foreach ($memo_details as $key => $element) {


            $product_id_array[$key] = $element['product_id'];
        }

        $product_info = $this->Product->find('all', array(
            'fields' => array('Product.id', 'Product.group_id'),
            'conditions' => array(
                'Product.id' => array_values($product_id_array),
                'NOT' => array('Product.group_id' => array_values($territory_id_product_group))
            ...
            try {
                ...
                foreach ($json_data['delete_memos'] as $val) {
                    ...
                    if ($val['memo_no']) {
                        ...
                        if (!$this->StoreBonusCard->deleteAll(array('StoreBonusCard.memo_no' => $val['memo_no']))) {
                            throw new Exception();
                        }
                        //add new
                        $this->LoadModel('SoCreditCollection');
                        if (!$this->SoCreditCollection->deleteAll(array('SoCreditCollection.memo_no' => $val['memo_no']))) {
                            throw new Exception();
                        }
                        //end add new
                        $this->LoadModel('InstallmentNo');
                        if (!$this->InstallmentNo->deleteAll(array('InstallmentNo.memo_no' => $val['memo_no']), false)) {
                            throw new Exception();
                        }             //

                        /*--------------------- Gift Item Delete : Start ---------------*/
                        if (!$this->GiftItem->query("DELETE FROM gift_item_details where gift_item_id = (SELECT id FROM gift_items WHERE memo_no='" . $val['memo_no'] . "')")) {
                            throw new Exception();
                        }
                        if (!$this->GiftItem->deleteAll(array('GiftItem.memo_no' => $val['memo_no']))) {
                            throw new Exception();
                        }
                        ...
                        $store_id_arr = $this->Store->find('first', array(
                            'conditions' => array(
                                'Store.territory_id' => $memo_id_arr['Memo']['child_territory_id'] ? $memo_id_arr['Memo']['child_territory_id'] : $memo_id_arr['Memo']['territory_id']
                            )
                        ));
                        ...
                        if ($punits_pre['sales_measurement_unit_id'] == $punits_pre['base_measurement_unit_id']) {
                            ...
                            $update_type = 'add';
                            if (!$this->update_current_inventory($base_quantity, $product_id, $store_id, $update_type, 12, $memo_id_arr['Memo']['memo_date'])) {
                                throw new Exception();
                            }
    ...
    public function get_memo_list()
    {
        ...
        /*check territory id parent or child */
        ...
        if ($memo_type == 'from_web') {
            $conditions = array('Memo.territory_id' => $territory_id, 'Memo.child_territory_id' => $child_territory_id, 'Memo.action >' => 0, 'Memo.from_app' => 0, 'Memo.status >' => 0);
        } elseif ($memo_type == 'memo_editable') {
            $conditions = array('Memo.territory_id' => $territory_id, 'Memo.child_territory_id' => $child_territory_id, 'Memo.memo_editable' => 1);
        } else {
            ...
            $conditions = array(
                ...
                'Memo.child_territory_id' => $child_territory_id,
                "OR" => array(array('Memo.action' => 0, 'Memo.memo_time >' => $seven_days_pre), array('SoCreditCollection.due_ammount >' => 0, 'Memo.memo_date >=' => '2018-10-01'))
            );
            $joins = array(
                array(
                    'table' => 'so_credit_collections',
                    'alias' => 'SoCreditCollection',
                    'type' => 'LEFT',
    ...
    public function get_child_territories()
    {
        ...
        $so_id = $json_data['so_id'];
        // $group_id = $json_data['group_id'];
        $group_id = 1008; // 1008=SPO user. Only spo user data will be sent
                ...
                if ($this->ReturnChallan->save($challan_data)) {
                    ...
                    foreach ($cdata['challan_details'] as $val) {
                        ...
                        $product_details = $this->Product->find('first', array(
                            ...
                            'conditions' => array('Product.id' => $val['product_id']),
                            'recursive' => -1
                        ));

                        if ($product_details['Product']['is_virtual'] == 1) {
                            $data['virtual_product_id'] = $val['product_id'];
                            $data['product_id'] = $product_details['Product']['parent_id'];
                        } else {
                            $data['virtual_product_id'] = 0;
                            $data['product_id'] = $product_details['Product']['id'];
                        }
    ...
    public function get_credit_collection_list()
    {
        ...
        $credit_collection_list = $this->SoCreditCollection->find('all', array(
            ...
            'conditions' => array(
                'SoCreditCollection.territory_id' => $territory_id,
    ...
    public function get_bonus_campaign_list()
    {
        ...
        $conditions[] = array('BonusCampaign.is_so' => 1);
    ...
    public function so_check_in_out(){
		...
		$this->loadModel('SoCheckInOut');

        $sales_person_id = $json_data['so_id'];
        $attendance_data = $json_data['attendance_data'];
        ...
        foreach($attendance_data as $key => $val){
            ...
            $prev_data = $this->SoCheckInOut->find('first', array(
                'conditions' => array(
                    'SoCheckInOut.so_id' => $sales_person_id,
                    'SoCheckInOut.date' => $date
                ),
                'recursive' => -1
            ));
            ...
            if ($type == 0) {
                ...
                if(empty($prev_data)){
                    ...
                    $this->SoCheckInOut->create();
					$data = array();
					
					$data['so_id'] = $sales_person_id;
					$data['territory_id'] = $territory_id;
					$data['office_id'] = $office_id;
					
                    $data['check_in_time'] = date('Y-m-d H:i:s a', strtotime($datetime));
                    $data['date'] = $date;
                    $data['status'] = 0;
                    $data['approve_by'] = 0;
                    $data['start_lat'] = $lat;
                    ...
                    $this->SoCheckInOut->save($data);

                    $res['status'] = 1;
				    $res['message'] = 'Successfully Checked In';
                }
            }
            ...
            if ($type == 1) {
                ...
                if(!empty($prev_data)){
                    ...
                    $data['id'] = $prev_data['SoCheckInOut']['id'];

                    $datetime = $date . ' ' . $time;

                    $data['check_out_time'] = date('Y-m-d H:i:s a', strtotime($datetime));
                    $data['end_lat'] = $lat;
                    $data['end_lan'] = $lan;
                    ...
                    $res['status'] = 1;
				    $res['message'] = 'Successfully Checked Out';
    ...
    public function so_check_in_out_last_row(){
        ...
        $data = $this->SoCheckInOut->find('first', array(
            'conditions' => array(
                'SoCheckInOut.so_id' => $sales_person_id,
                'SoCheckInOut.date' => date("Y-m-d"),
            ),
            'order'=>array('SoCheckInOut.id'=>'DESC'),
            'recursive' => -1
        ));
		
		if(empty($data)){
			$data['SoCheckInOut']['in_out'] = 3;
		}else{
			
			if(empty($data['SoCheckInOut']['check_out_time'])){
				$data['SoCheckInOut']['in_out'] = 0;
				$data['SoCheckInOut']['time'] = date('h:i a', strtotime($data['SoCheckInOut']['check_in_time']));
			}else{
				$data['SoCheckInOut']['in_out'] = 1;
				$data['SoCheckInOut']['time'] = date('h:i a', strtotime($data['SoCheckInOut']['check_out_time']));
			}
    ...
    public function so_check_in_out_history(){
        ...
        $sales_person_id = $json_data['so_id'];
        $from_date = date('Y-m-d', strtotime($json_data['from_date']));
        $to_date = date('Y-m-d', strtotime($json_data['to_date']));
        
        $data_list = $this->SoCheckInOut->find('all', array(
            'conditions' => array(
                'SoCheckInOut.so_id' => $sales_person_id,
                'SoCheckInOut.date >=' => $from_date,
                'SoCheckInOut.date <=' => $to_date,
            ),
            'order'=>array('SoCheckInOut.id'=>'ASC'),
            'recursive' => -1
        ));
        $res = array();
        foreach($data_list as $val){
			...
			if( $val['SoCheckInOut']['status'] != 1){
				$val['SoCheckInOut']['status'] = 0;
			}
			
			$val['SoCheckInOut']['time'] = date('h:i a', strtotime($val['SoCheckInOut']['check_in_time']));
			
            $res[] = $val['SoCheckInOut'];
        }
	...
	public function get_ncp_type(){
		
		//$json_data = $this->request->input('json_decode', true);
		
		$this->loadModel('NcpType');
		/*
       $res_status = (isset($json_data['all']) != '' ? $json_data['all'] : 1);
        if ($res_status == 1) {
            $conditions = array();
        } else {
            $conditions = array('NcpType.updated_at >' => $last_update_date);
        }
		*/
        
        $data_list = $this->NcpType->find('all');
        $res = array();
        foreach($data_list as $key => $val){
			
			$res[] = array(
				'id'=>$val['NcpType']['id'],
				'name'=>$val['NcpType']['name'],
				'status'=>$val['NcpType']['is_active']
			);
        }
		//echo '<pre>';print_r($res);exit;
		$this->set(array(
			'ncp_type' => $res,
			'_serialize' => array('ncp_type')
		));
