 ...
 */
class ApiData176RetrivesController extends AppController
{
        ...
        if (!empty($json_data)) {
            ...
            if (is_numeric($json_data['market_id']) || true) {
                ...
                foreach ($json_data['memo_details'] as $val) {
                    ...
                    $price = $val['price'] > 0 ? $val['price'] : 0;
                    $product_wise_value += ($sales_qty * $price);
                }
                if (abs($json_data['gross_value'] - ($product_wise_value - $json_data['total_discount'])) > 1) {
                    $res['status'] = 0;
    ...
    function discount_update($memo_no)
    {
        $this->LoadModel('Memo');
        $memo_data = $this->Memo->query("select 
        m.id as memo_id,
        ...
        if (count($memo_data)) {
            ...
            $sql = "update m set m.total_discount=(case when m.total_discount is null then 0 else m.total_discount end)+eligible_memo.discount,m.gross_value=(m.gross_value-eligible_memo.discount),m.cash_recieved=(m.cash_recieved-eligible_memo.discount) from memos m 
            inner join (select 
            m.id as memo_id,
            sum(md.price*md.sales_qty) as total_memo,
            FLOOR((sum(md.price*md.sales_qty)*10/100)) as discount
            ...
            $this->Memo->query($sql);
            $sql = "update cl set cl.memo_value=m.gross_value,cl.collectionAmount=m.cash_recieved from collections cl 
            inner join memos m on m.id=cl.memo_id
            where 
                m.memo_no='$memo_no'
                and cl.type=1
                and (cl.is_credit_collection=0 or cl.is_credit_collection is null)";
            $this->Memo->query($sql);
            $m = $this->Memo->find('first', array(
                'fields' => array('Memo.*'),
