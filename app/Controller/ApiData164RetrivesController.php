 ...
 */
class ApiData164RetrivesController extends AppController {
    ...
    public function user_login() {
        ...
        /*$path = APP . 'logs/';
        $myfile = fopen($path . "login.txt", "a") or die("Unable to open file!");
        ...
        foreach ($result_challan as $row) {
            ...
            foreach ($result_challan_details as $row_details) {
                ...
                $result_up_each = $this->CurrentInventory->updateAll(
                        array('CurrentInventory.qty' => "CurrentInventory.qty+$challan_qty"), array(
                ...
                Challan->updateAll(array('Challan.status' => 2), array('Challan.id' => $challan_id));
            }
        }
            ...
            foreach ($result_memo_details as $row_memo_details) {
                ...
                if (!empty($result_get_id)) {
                    ...
                    $result_up_each_memo = $this->CurrentInventory->updateAll(
                            array('CurrentInventory.qty' => "CurrentInventory.qty-$sales_qty"), array('CurrentInventory.store_id' => $store_id, 'CurrentInventory.product_id' => $product_id, 'CurrentInventory.id' => $current_inventory_id)
                    );
                }
            }
        }
	...
	*/
    ...
    public function challan_received() {
        ...
        if (!empty($json_data)) {
            ...
            foreach ($json_data as $val) {
                ...
                $challans_check = $this->Challan->find('first', array('conditions' => array('Challan.id' => $challan_id, 'challan.status' => 2),
                    'recursive' => -1));
                if ($challans_check) {
                    continue;
                }
                ...
                if ($count == 0) {
                    ...
                    foreach ($json_data['memo_details'] as $val) {
                        ...
                        if ($punits['sales_measurement_unit_id'] == $punits['base_measurement_unit_id']) {
                            $sale_quantity = $val['sales_qty'];
                        } else {
                            $sale_quantity = $this->unit_convert($val['product_id'], $punits['sales_measurement_unit_id'], $val['sales_qty']);
                        }
                        $product_price_id = 0;
                        if ($val['price'] > 0) {
                            $product_price_id = $this->get_product_price_id($val['product_id'], $val['price'], $all_product_id,date('Y-m-d', strtotime($json_data['memo_date'])));
                        }
                    ...
                    $this->MemoSyncHistory->updateAll(array(
                        'MemoSyncHistory.missed_memo' => 'MemoSyncHistory.missed_memo - 1'), array('MemoSyncHistory.id' => $memo_sync_id));
                    //end for memoSync
                } else {
    ...
    function stock_check($store_id,$memo_details,$memo_id=0)
    {
        ...
        if($memo_id)
        {
            $prev_data=$this->MemoDetail->find('all',array(
                ...
                // 'fields'=>array('product_id','sum(MemoDetail.sales_qty) as sales_qty'),
                'fields'=>array('MemoDetail.is_bonus','MemoDetail.sales_qty','MemoDetail.product_id','CASE WHEN MemoDetail.price > 0 then 1 else 0 end as price_bonus'),
                // 'group'=>array('product_id','memo_id','price'),
                'recursive'=>-1
                ));
            $prev_memo_detail=array();
            foreach($prev_data as $data)
            {
                $prev_memo_detail[$data['MemoDetail']['product_id']][$data['0']['price_bonus']][$data['0']['price_bonus']==1?1:$data['MemoDetail']['is_bonus']]=$data['MemoDetail']['sales_qty'];
            }
            // pr($memo_details);
            // pr($prev_memo_detail);exit;
            foreach($memo_details as $data)
            {
                if(isset($prev_memo_detail[$data['product_id']][$data['price']>0?1:0][$data['is_bonus']]))
                {
                    $current_inventory=$this->CurrentInventory->find('all',array(
                        'conditions'=>array('CurrentInventory.store_id'=>$store_id,'CurrentInventory.product_id'=>$data['product_id']),
                        'joins'=>array(
                            array(
                        ...
                        'group'=>array('CurrentInventory.store_id','ProductMeasurement.qty_in_base','CurrentInventory.product_id HAVING (sum(CurrentInventory.qty) + ROUND((case when ProductMeasurement.qty_in_base is null then 1 else ProductMeasurement.qty_in_base end) *'.$prev_memo_detail[$data['product_id']][$data['price']>0?1:0][$data['is_bonus']].',0))  >= ROUND((case when ProductMeasurement.qty_in_base is null then 1 else ProductMeasurement.qty_in_base end) *'.$data['sales_qty'].',0)'),
                ...
                else
                {
                    $current_inventory=$this->CurrentInventory->find('all',array(
                        ...
                        'group'=>array('CurrentInventory.store_id','ProductMeasurement.qty_in_base','CurrentInventory.product_id HAVING sum(CurrentInventory.qty)  >= ROUND((case when ProductMeasurement.qty_in_base is null then 1 else ProductMeasurement.qty_in_base end) *'.$data['sales_qty'].',0)'),
                        'fields'=>array('CurrentInventory.store_id','CurrentInventory.product_id','sum(CurrentInventory.qty) as qty')
                        ));
                    if(!$current_inventory)
                    {
    ...
    function bouns_and_scheme_id_set($b_product_id = 0, $memo_date = '', $memo_products = array()) {
        ...
        if ($bonus_info) {
            ...
            if ($org_bonus_qty < $b_product_qty) {
                $bonus_com_info = $this->OpenCombination->find('first', array(
                    'joins' => array(
                        array(
    ...
    public function create_so_ncp_return_challan() {
        ...
        $challan_data['created_at'] = $this->current_datetime();
        $challan_data['created_by'] = $json_data['so_id'];
        $challan_data['updated_at'] = $this->current_datetime();
        $challan_data['updated_by'] = 0;
   ...
   /*
    ...
    function get_day_close() {
        ...
        $json_data = $this->request->input('json_decode', true);
        $so_id=$json_data['so_id'];
        $day_closes= $this->DayClose->find('first',array(
    ...
    public function get_stamp_target_info() {
        ...
        $target_list = $this->StoreBonusCard->query("SELECT territory_id,market_id,outlet_id,product_id,bonus_card_type_id,bonus_card_id,sum(quantity) as achieve_quantity,sum(no_of_stamp) as stamp     
    		FROM store_bonus_cards where territory_id=$territory_id group by territory_id,market_id,outlet_id,product_id,bonus_card_type_id,bonus_card_id");
    ...
    public function get_ncp_collection_list() {
        ...
        $collection_list = $this->NcpCollection->find('all', array(
    ...
    public function get_collection_list() {
        $this->LoadModel('Collection');
        $last_30_days = date('Y-m-d', strtotime('-1 month'));
        $json_id = $this->request->input('json_decode', true);
        $so_id = $json_id['so_id'];
    ...
    public function get_credit_collection_list() {
        ...
        $credit_collection_list = $this->SoCreditCollection->find('all', array(
			...
			'Memo.memo_date >=' => $memo_start_date
			),
    ...
    public function create_installment_no($list=null) {
        ...
        foreach ($json_data as $data) {
            $chk_exist = $this->InstallmentNo->find('first', array(
                'conditions' => array('installment_no_id' => $data['instrument_no_id'], 'payment_id' => $data['payment_id'], 'so_id' => $data['so_id']),
                'recursive' => -1
            ));
            //pr($chk_exist);die;
            if ($chk_exist) {
            ...
            $data_insert['updated_at'] = $data['updated_at'];
    ...
    public function create_outlet_visit() {
        ...
        $json_data = $json_data['outlet_visit'];
        $outlet_visit_data_all = array();
        if (!empty($json_data)) {
            foreach ($json_data as $each_data) {
                ...
                $outlet_visit_data_all[] = $outlet_visit_data;
            }
        }
    ...
    public function update_territory_wise_collection_deposit_balance($so_id) {
        ...
        foreach ($instrument_type as $key => $ins_data) {
            $collection_data = $this->Collection->find('first', array(
                ...
                'conditions' => array('SalesPerson.territory_id' => $territory_id, 'Collection.type' => $key,'Collection.CollectionDate >='=>'2018-10-01'),
                'joins' => array(
                    array(
                        'table' => 'sales_people',
                        'alias' => 'SalesPerson',
            ...
            $deposit_data = $this->Deposit->find('first', array(
                ...
                'conditions' => array('SalesPerson.territory_id' => $territory_id, 'Deposit.type' => $key,'Deposit.deposit_date >='=>'2018-10-01'),
                'joins' => array(
                    array(
                        'table' => 'sales_people',
                        'alias' => 'SalesPerson',
    ...
    public function get_product_price_id($product_id, $product_prices, $all_product_id,$memo_date) {
    ...
    public function get_deposit_balance() {
        ...
        $balance = $this->TerritoryWiseCollectionDepositBalance->find('all', array(
            ...
            'conditions' => array('TerritoryWiseCollectionDepositBalance.territory_id' => $territory_id, 'TerritoryWiseCollectionDepositBalance.instrument_type_id' => 1),
            'recursive' => -1,
        ));
        $data_array = array();
        if ($balance) {
