 ...
 */
class ApiData171RetrivesController extends AppController {
    ...
    public function user_login() {
        ...
        $path = APP . 'logs/';
        $myfile = fopen($path . "login_171.txt", "a") or die("Unable to open file!");
		...
		$path = APP . 'logs/';
        $myfile = fopen($path . "login_171_response.txt", "a") or die("Unable to open file!");
    ...
    public function get_product_list() {
        ...
        $products = $this->Product->find('all', array(
            'fields' => array('Product.id', 'Product.name', 'Product.product_code', 'Product.product_category_id', 'Product.sales_measurement_unit_id', 'Product.product_type_id', 'Product.updated_at', 'Product.is_pharma', 'Product.order','Product.is_injectable','CASE WHEN ProductMeasurement.qty_in_base is null then 1 ELSE ProductMeasurement.qty_in_base END as qty_in_base'),
            'conditions' => $conditions,
            'joins'=>array(
                array(
    ...
    function stock_check($store_id,$memo_details,$memo_id=0)
    {
        ...
        {
            $result[$element['product_id']] = (isset($result[$element['product_id']])?$result[$element['product_id']]:0)+$element['sales_qty'];
        }
        $this->loadModel('CurrentInventory');
        $this->loadModel('MemoDetail');
        if($memo_id)
        {
            $prev_data=$this->MemoDetail->find('all',array(
                ...
                'fields'=>array('SUM(MemoDetail.sales_qty) as sales_qty','MemoDetail.product_id'),
            ...
            {
                $prev_memo_detail[$data['MemoDetail']['product_id']] = $data['0']['sales_qty'];
            }
    ...
    public function get_memo_list() {
        ...
        foreach ($memo_list as $m) {
            ...
            foreach ($m['MemoDetail'] as $each_memo_details) {
                ...
                $mdata['memo_details'][$mm]['vat'] = $this->get_vat_by_product_id_memo_date($product_id,$m['Memo']['memo_date'],$m['Memo']['is_distributor']);
                $mm++;
            }
       ...
       /* $pending_return_challan_count = $this->ReturnChallan->find('count', array(
        ...
        /*f ($pending_return_challan_count == 0) {*/
            $stock_info = $this->CurrentInventory->find('all', array(
                ...
                'conditions' => $conditions,
                'group'=>array('CurrentInventory.store_id','CurrentInventory.product_id','Product.sales_measurement_unit_id'),
            ...
            foreach ($stock_info as $val) 
            {
                ...
                /*$data['qty'] = $this->unit_convertfrombase($val['CurrentInventory']['product_id'], $val['Product']['sales_measurement_unit_id'], $val['CurrentInventory']['qty']);*/
                /*$data['updated_at'] = $val['CurrentInventory']['updated_at'];*/
                $data_array['CurrentInventory'][$val['CurrentInventory']['product_id']] = $this->unit_convertfrombase($val['CurrentInventory']['product_id'], $val['Product']['sales_measurement_unit_id'], $val['0']['qty']);
            }
       /* }
        else
        {
            ...
            foreach ($json_data['session_list'] as $val) {
                ...
                if (empty($session_attend) || (!empty($session_attend) && $session_attend['ProgramSession']['updated_by'] == $user_id)) {
                    ...
                    if(!empty($session_product))
                    {
                        ...
                        {
                            $prev_stock_array[$data['SessionDetail']['product_id']] = (isset($prev_stock_array[$data['SessionDetail']['product_id']])?$prev_stock_array[$data['SessionDetail']['product_id']]:0)+$data['SessionDetail']['quantity'];
                        }
                    }
                    
                    $stock_okay=$this->stock_check_for_validation_for_other($store_info['Store']['id'],$stock_checking_array,$prev_stock_array);
   ...
   /*
    ...
    public function get_outlet_visit_information_report()
    {
        ...
        for($i=0;$i<=1;$i++)
        {
            ...
            $rpt_data['non_effective_visit']=@$v_data['non_effective_outlet'][$data['0']['is_pharma_type']]?$v_data['non_effective_outlet'][$i]:0;
            ...
            foreach($json as $json_m_data)
            {
                ...
                if (is_numeric($json_data['market_id']) || true)
                {
                    ...
                    if ($count == 0)
                    {
                        ...
                        $memo_data['memo_date'] = date('Y-m-d', strtotime($json_data['memo_date']));
                        $memo_data['memo_time'] = $memo_datas['Memo']['memo_time'];
                        $memo_data['sales_person_id'] = $json_data['sales_person_id'];
                        $memo_data['sales_to'] = 0;
                        $memo_data['outlet_id'] = (is_numeric($json_data['outlet_id']) ? $json_data['outlet_id'] : 0);
                        $memo_data['market_id'] = (is_numeric($json_data['market_id']) ? $json_data['market_id'] : 0);
