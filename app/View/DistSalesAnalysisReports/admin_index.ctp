	...
	<div class="col-xs-12">
		...
		<div class="box box-primary" style="float:left; width:100%;">
			...
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-th-large"></i> <?php echo __('Distributor Sales Analysis Report'); ?></h3>
			...
			<div class="box-body">
				...
				<div class="search-box" style="border:none; float:left; width:100%;">
					...
					<?php echo $this->Form->create('DistSalesAnalysisReports', array('role' => 'form', 'action' => 'index')); ?>
						...
						<div class="col-md-5 col-sm-12 filter_left_div">
							...
							<div class="data_box">
								...
								<div id="market_list" class="input row_list" style="float:left; width:100%; padding-left:0px;">
									...
									<?php echo $this->Form->input('rows', array('legend' => false, 'onclick' => 'indicatorDisable()', 'type' => 'radio', 'id' => 'rows', 'default' => 'sr', 'separator' => '</div><div class="list">', 'class' => 'form-control rows')); ?>
								</div>
							</div>
						...
						<div class="col-md-7 col-sm-12 condition c_first">
							...
							<div style="float:left; width:100%; padding-bottom:20px;">
								<div class="col-md-6 col-sm-12" style="padding:0px">
									...
									<div style="padding-bottom:10px; float:left; width:100%;">
										<label>SR :</label>
										<?php echo $this->Form->input('sr_id', array('label' => false, 'id' => 'sr_id', 'class' => 'form-control sr_id', 'required' => false, 'empty' => '---- All ----')); ?>
									</div>
								</div>
						...
						<div class="table-responsive">
							...
							<div id="xls_body">
								<div style="float:left; width:100%; text-align:center; padding:20px 0; clear:both;">
									...
									<h3 style="margin:2px 0;">Distributor Sales Analysis Report</h3>
									<p>
										Time Frame : <b><?= date('d M, Y', strtotime($date_from)) ?></b> to <b><?= date('d M, Y', strtotime($date_to)) ?></b>
									</p>
									<?php if (in_array('volume', $request_data['DistSalesAnalysisReports']['indicators'])) { ?>
								...
								<div style="float:left; width:100%; height:450px; overflow:scroll;">
									<table id="sum_table" class="text-center report_table" border="1px solid black" cellpadding="10px" cellspacing="0" align="center">
										...
										<?php if ($columns_list) { ?>
											...
											<tr class="titlerow">
												<?php if($request_data['DistSalesAnalysisReports']['rows']=='sr'){?>

												<th style="text-align:left;">
													<div>Area Office</div>
												</th>
												<th style="text-align:left;">
													<div>Area Executive</div>
												</th>
												<th style="text-align:left;">
													<div>Tso</div>
												</th>
												...
												<?php }elseif ($request_data['DistSalesAnalysisReports']['rows']=='dist') { ?>
												...
												<?php }elseif ($request_data['DistSalesAnalysisReports']['rows']=='tso') { ?>
												...
												<?php } elseif ($request_data['DistSalesAnalysisReports']['rows']=='ae') { ?>
												...
												<?php } ?>
												<th style="text-align:left;">
													<div>By <?= $request_data['DistSalesAnalysisReports']['rows'] ?></div>
												</th>
												<?php
												$color = '#f1f1f1';
...
<script>
	$(document).ready(function() {
		...
		$('#DistSalesAnalysisReportsIndexForm').trigger("reset");
	...
	*/
	...
	$('#division_id').selectChain({
		target: $('#district_id'),
		value: 'name',
		url: '<?= BASE_URL . 'dist_sales_analysis_reports/get_district_list' ?>',
		type: 'post',
		data: {
	...
	$('#district_id').selectChain({
		target: $('#thana_id'),
		value: 'name',
		url: '<?= BASE_URL . 'dist_sales_analysis_reports/get_thana_list' ?>',
		type: 'post',
		data: {
	...
	$('#office_id').change(function() {
		date_from = $('#DistSalesAnalysisReportsDateFrom').val();
		date_to = $('#DistSalesAnalysisReportsDateTo').val();


		if (date_from && date_to) {
			$.ajax({
				type: "POST",
				url: '<?= BASE_URL ?>dist_sales_analysis_reports/get_office_sr_list',
...
<script>
	function indicatorDisable() {
		...
		if (rows == 'sr') {
			...
			<?php if (!@$request_data['DistSalesAnalysisReports']['sr_id']) { ?>
				$('#sr_id option:nth-child(1)').prop("selected", true).change();
			<?php } ?>
		}
		...
		if (columns == 'national' || columns == 'outlet_type') {
			$('#DistSalesAnalysisReportsIndicatorsVolume').prop('disabled', true);
		} else {
			$('#DistSalesAnalysisReportsIndicatorsVolume').prop('disabled', false);
		}
	}
	...
	$('#product_type').change(function() {
		...
		$.ajax({
			type: "POST",
			url: '<?= BASE_URL ?>dist_sales_analysis_reports/get_product_list',
...
<script>
	$("#download_xl").click(function(e) {
		...
		a.href = downloadUrl;
		a.download = "dist_sales_analysis_report.xls";
