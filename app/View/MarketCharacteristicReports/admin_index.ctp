						...
						<div class="table-responsive">
							...
							<div id="xls_body">
								...
								<div style="float:left; width:100%; height:430px; overflow:scroll;">
										...
										<tbody>
											...
											<?php if ($results) { ?>
												...
												<?php foreach ($results as $district_name => $thana_datas) { ?>
													...
													<?php
													foreach ($thana_datas as $thana_name => $market_datas) {
													?>
														<tr>
														...
														foreach ($market_datas as $market_name => $outlet_datas) {
															...
															if ($report_type == 'non_visited') {
																...
																$outlet_datas = @$results1[$district_name][$thana_name][$market_name] ? $results1[$district_name][$thana_name][$market_name] : array();
															}
														...
														<?php if ($report_type != 'non_visited') { ?>
															<tr style="background:#ccc;">
																<td style="text-align:center;" colspan="7">
																	<b>
																		Total Market : <?= $total_markets_thana_wise[($thana_name)] ?>,
																		Visited Market : <?= $thana_visited_market ?>,
																		Non Visited Market : <?= $total_markets_thana_wise[($thana_name)] - $thana_visited_market ?>
																	</b>
																</td>
															</tr>
														<?php } else { ?>
															<tr style="background:#ccc;">
																<td style="text-align:center;" colspan="7">
																	<b>
																		Total Market : <?= $total_markets_thana_wise[($thana_name)] ?>,
																		Visited Market : <?= $total_markets_thana_wise[($thana_name)] - count($market_datas) ?>,
									...
									<?php /*?><?php if($report_type=='non_visited'){ ?>
                                <table cellspacing="0" cellpadding="10px" border="1px solid black" align="center" class="text-center report_table" id="sum_table">
                                  <tbody>
                                    <tr class="titlerow">
                                      <th>Market</th>
                                    	...
                                    	<?php foreach($results as $district_name => $thana_datas){ ?>
											...
											?>
                                                ...
                                                <tr>
                                                  ...
                                                  <td><?=$memo_dates?count(explode(', ', rtrim($memo_dates, ', '))):''?></td>
                                                  <td><?=rtrim($memo_dates, ', ')?></td>
                                                  <td style="text-align:right;"><?=@count($outlet_datas)?> of <?=@$total_outlets_market_wise[$market_name]?></td>
                                                  <td style="text-align:right;"><?=$no_of_memo?$no_of_memo:''?></td>
                                                  <td style="text-align:right;"><?=$memo_total?sprintf("%01.2f", $memo_total):''?></td>
                                                  <td><?=$so_name?></td>
                                                </tr>
                                            	...
                                            	<?php 
													if($memo_dates)$thana_visited_market++;
													if($memo_dates)$thana_visited_outlet+= count($outlet_datas);
													$thana_total_outlet+= $total_outlets_market_wise[$market_name];
													$thana_no_memo_total+= $no_of_memo;
													$thana_memo_total+= $memo_total;
												} 
												?>
