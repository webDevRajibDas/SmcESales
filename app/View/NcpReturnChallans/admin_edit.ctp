    ...
    <div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-edit"></i>  <?php echo __('New NCP Return Challan'); ?></h3>
			...
			<div class="form-group">
				<?php echo $this->Form->input('receiver_store_id', array('class' => 'form-control','options'=>$receiverStore,'selected'=>$return_challan_info[0]['ReturnChallan']['receiver_store_id'],'required'=>true)); ?>
			</div>
			<div class="form-group">
				<?php echo $this->Form->input('remarks', array('class' => 'form-control','value'=>$return_challan_info[0]['ReturnChallan']['remarks'])); ?>
			</div>
			<div class="form-group">
				<?php echo $this->Form->input('inventory_status_id', array('id'=>'inventory_status_id','class' => 'form-control','type'=>'hidden','value'=>2)); ?>
			</div>
			<div class="form-group">
				<?php echo $this->Form->input('transaction_type_id', array('id'=>'transaction_type_id','class' => 'form-control','type'=>'hidden','value'=>13)); ?>
			</div>
			...
			<table class="table table-striped table-condensed table-bordered invoice_table">
				...
				<tbody>
				    ...
				    foreach ($return_challan_info[0]['ReturnChallanDetail'] as $value) {
				        ...
				        <tr class="table_row" id="rowCount<?php echo $row_count_for_remove;?>">
				            ...
				            <td align="center" id="<?php echo $row_count_for_remove;?>">
				                <input type="number" step="any" min="0" name="quantity[<?php echo $row_count_for_remove;?>]" id="product_quantity<?php echo $row_count_for_remove;?>" class="p_quantity product_quantity" value="<?php echo $value['challan_qty']; ?>" required >
				            </td>
	...
	$(document).ready(function(){			
		...
		$('#expire_date').change(function(){
			...
			}else if(is_maintain_batch == 1 && batch_no == '')
			{
				...
				$.ajax({
					type: "POST",
					url: '<?php echo BASE_URL;?>current_inventories/get_inventory_details_in_NCP_Return_challan',
					//data: 'product_id='+product_id+'&batch_no='+batch_no+'&expire_date='+expire_date+'&transaction_type_id='+transaction_type_id+'&inventory_status_id='+inventory_status_id,
		...
		$(".add_more").click(function() {	
			...
			}else if(is_maintain_expire_date == 1 && expire_date == '')
			{
				...
				return false;
			}else if(!quantity.match(/^\d+(\.\d{1,2})?$/) || parseFloat(quantity) < 0)
			{
			...
			else
			{
				...
				$.ajax({
					...
					success: function(response){						
						var obj = jQuery.parseJSON(response);
						var recRow = '<tr class="table_row" id="rowCount'+rowCount+'"><td align="center">'+rowCount+'</td><td><input type="hidden" name="selected_product_id[]" class="selected_product_id" value="'+obj.Product.id+batch_no+expire_date+'"/>'+obj.Product.name+'<input type="hidden" name="product_id[]" value="'+obj.Product.id+'"/></td><td align="center"><input type="hidden" name="batch_no[]" value="'+batch_no+'">'+batch_no+'</td><td align="center"><input type="hidden" name="expire_date[]" value="'+expire_date+'">'+display_exp_date+'</td><td align="center">'+obj.ReturnMeasurementUnit.name+'<input type="hidden" name="measurement_unit[]" value="'+obj.Product.return_measurement_unit_id+'"></td><td align="center" id="'+rowCount+'"><input type="number" step="any" min="0" name="quantity[]" class="p_quantity product_quantity" id="product_quantity'+rowCount+'" value="'+quantity+'"></td><td align="center" class="stock_qty" id="stock_qty'+rowCount+'">'+stock_qty+'</td><td align="center"><input type="text" class="full_width form-control" name="remarks[]"></td><td align="center"><button class="btn btn-danger btn-xs remove" value="'+rowCount+'"><i class="fa fa-times"></i></button></td></tr>';
