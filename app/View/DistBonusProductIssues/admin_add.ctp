        ...
        <div class="box box-primary" style="float:left; width:100%; position:relative; padding-bottom:10px;">
            ...
            <div class="box-body">
                ...
                <div class="col-md-7">
                    ...
                    <div class="form-group">
                        <?php echo $this->Form->input('challan_date', array('id' => 'challan_date', 'label' => 'Product Issue Date', 'onChange' => 'getPreviousChallan()', 'type' => 'text', 'class' => 'form-control challan-datepicker', 'required' => true, 'readonly' => true)); ?><span style="color:red">*</span>
                    </div>
            ...
            if (product_id == '') {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var obj = jQuery.parseJSON(response);
                        console.log(obj);
                        var recRow = '<tr class="table_row" id="rowCount' + rowCount + '"><td align="center">' + rowCount + '</td><td> <input type="hidden" name="product_qty[' + rowCount + ']" class="product_qty" value="' + product_qty + '"/> <input type="hidden" name="selected_product_id[' + rowCount + ']" class="selected_product_id" value="' + obj.Product.id + batch_no + expire_date + '"/>' + obj.Product.name + '<input type="hidden" name="product_id[' + rowCount + ']" value="' + obj.Product.id + '"/></td><td align="center"><input type="hidden" name="batch_no[' + rowCount + ']" value="' + batch_no + '">' + batch_no + '</td><td align="center"><input type="hidden" name="expire_date[' + rowCount + ']" value="' + expire_date + '">' + display_exp_date + '</td><td align="center">' + obj.ChallanMeasurementUnit.name + '<input type="hidden" name="measurement_unit[' + rowCount + ']" value="' + obj.Product.challan_measurement_unit_id + '"></td><td align="center"><input name="quantity[' + rowCount + ']" class="' + product_id + '_product_qty p_quantity" value="' + quantity + '" required ></td><td align="center" class="' + product_id + '"></td><td  align="center"><input type="text" name="source[' + rowCount + ']" value="' + source + '" readonly></td><td align="center"><input type="text" class="full_width form-control" name="remarks[' + rowCount + ']"></td><td align="center"><button class="btn btn-danger btn-xs remove" value="' + rowCount + '"><i class="fa fa-times"></i></button></td></tr>';
