	...
	<div class="col-xs-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-th-large"></i> <?php echo __('SR Order List'); ?></h3>
				<div class="box-tools pull-right">
					<?php
					if ($this->App->menu_permission('distOrders', 'admin_create_Order')) { ?>
						<a class="btn btn-primary" href="<?= BASE_URL ?>admin/dist_Orders/create_Order"><i class="glyphicon glyphicon-plus"></i> New SR Order</a>
					<?php } ?>

					<?php if ($this->App->menu_permission('dist_orders', 'admin_order_map')) {
						echo $this->Html->link(__('<i class="glyphicon glyphicon-map-marker"></i> Order on Map'), array('action' => 'order_map'), array('class' => 'btn btn-success', 'escape' => false));
					} ?>
						...
						<div id="rpt_content">
							<div style="float:left; width:100%; padding-bottom:20px;">
								...
								<div style="width:50%;text-align:center;float:left">
									...
									<font><?php if (!empty($this->request->data)) { ?><strong>Between:</strong>&nbsp;&nbsp;<u><?php echo date('d-F-Y', strtotime($this->request->data['DistOrder']['date_from'])); ?>&nbsp;&nbsp;to&nbsp;&nbsp;<?php echo date('d-F-Y', strtotime($this->request->data['DistOrder']['date_to']));
						...
						<table id="DistOrder" class="table table-bordered">
							<thead>
								<tr>
									...
									<th class="text-center"><?php echo $this->Paginator->sort('order_time', 'Order Date Time'); ?></th>
									<th class="text-center">Status</th>
									<!-- <th class="text-center"><?php //echo $this->Paginator->sort('processing_status'); 
																	?></th> -->
									<th width="50" class="text-center"><?php echo __('Actions'); ?></th>
								</tr>
							</thead>
							<tbody>
								<?php
								...
								?>
									<tr style="background-color:<?php echo $Order['DistOrder']['from_app'] == 0 ? '#f5f5f5' : 'white' ?>">
										...
										<td align="center"><?php echo date("d-M-Y h:i a", strtotime($Order['DistOrder']['order_time'])); ?></td>
								...
								?>
										...
										<td class="text-center">
											<?php if ($this->App->menu_permission('dist_Orders', 'admin_view')) {
											...
											<?php if ($Order['DistOrder']['action'] != 0) { ?>
												...
												<?php if ($Order['DistOrder']['status'] == 0) { ?>
													<?php if ($this->App->menu_permission('dist_Orders', 'admin_edit')) {
														echo $this->Html->link(__('<i class="glyphicon glyphicon-pencil"></i>'), array('action' => 'edit', $Order['DistOrder']['id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => 'edit'));
													} ?>
												<?php } ?>

												<?php if ($this->App->menu_permission('dist_Orders', 'admin_delete')) {
													echo $this->Form->postLink(__('<i class="glyphicon glyphicon-trash"></i>'), array('action' => 'delete', $Order['DistOrder']['id']), array('class' => 'btn btn-danger btn-xs', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => 'delete'), __('Are you sure you want to delete # %s?', $Order['DistOrder']['id']));
												} ?>
...
<div id="content" style="width:90%;height:100%;margin-left:5%;margin-right:5%;">
	...
	<div style="width:100%;text-align:center;float:left">
		...
		<h5><?php if (!empty($requested_data)) { ?><strong>Between:</strong>&nbsp;&nbsp;<u><?php echo $requested_data['DistOrder']['date_from']; ?>&nbsp;&nbsp;to&nbsp;&nbsp;<?php echo $requested_data['CsaOrder']['date_to'];
...
<script>
	$(document).ready(function() {
		...
		function get_dist_by_tso_id(tso_id) {
			...
			$.ajax({
				...
				data: {
					'order_date_from': DistOrderDateFrom,
					'order_date_to': DistOrderDateTo,
					'tso_id': tso_id,
					'distributor_id': distributor_id
				},
				type: 'POST',
	...
	$(document).ready(function() {
		$("#download_xl").click(function(e) {
			...
			// console.log(arrStr);
			window.open("<?= BASE_URL; ?>DistOrders/download_xl?" + formData);
		});
	});
</script>