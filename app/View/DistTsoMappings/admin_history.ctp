    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header">
                <h3 class="box-title"><i class="glyphicon glyphicon-th-large"></i> <?php echo __('TSO and Distributor Mapping History'); ?></h3>
                <div class="box-tools pull-right">
                    <?php
                    if ($this->App->menu_permission('DistTsoMappings', 'admin_index')) {
                        echo $this->Html->link(__('<i class="glyphicon glyphicon-plus"></i> TSO and Distributor Mapping'), array('action' => 'index'), array('class' => 'btn btn-primary', 'escape' => false));
                    }
                    ?>
                </div>
            </div>	
            <div class="box-body">
                <div class="search-box">
                    <?php echo $this->Form->create('DistTsoMapping', array('role' => 'form', 'action' => 'history')); ?>
                    <table class="search">
                        ...
                        <tr><td width="50%"><?php echo $this->Form->input('dist_distributor_id', array('label' => 'Distributor', 'id' => 'dist_distributor_id', 'class' => 'form-control', 'required' => false, 'empty' => '---- Select Office ----','options'=>$distDistributors,'selected'=>$dist_distributor_id)); ?></td></tr>
                    ...
                    <tbody>
                        <?php 
                                                        foreach ($data as $key => $value) {
                                                            ...
                                                            ?>
                        <tr>
                            <td><?php echo $value['DistTso']['name'];?></td>
                            <td><?php echo $value['DistDistributor']['name'];?></td>
                            <td><?php echo date("m-d-Y",strtotime($value['DistTsoMappingHistory']['effective_date']));?></td>
                            <td><?php
                            if($value['DistTsoMappingHistory']['end_date'])
                            {
                                echo date("m-d-Y",strtotime($value['DistTsoMappingHistory']['end_date']));
                            }
                            else 
                            {
                            ...
                            ?></td>
                            <td><?php echo $users[$value['DistTsoMappingHistory']['created_by']];?></td>
                            <td><?php echo date("m-d-Y",strtotime($value['DistTsoMappingHistory']['created_at']));?></td>
                            <td><?php echo $users[$value['DistTsoMappingHistory']['updated_by']];?></td>
                            <td><?php echo date("m-d-Y",strtotime($value['DistTsoMappingHistory']['updated_at']));?></td>
                        </tr>
