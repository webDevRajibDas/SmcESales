    ...
    <div class="col-md-12">
		<div class="box box-primary">
			...
			<?php echo $this->Form->create('Combination', array('role' => 'form')); ?>
				...
				<div class="form-group">
					<?php echo $this->Form->input('ProductCombination.effective_date', array('type'=>'text','class' => 'datepicker form-control','required'=>true)); ?>
				</div>
				<br/>
                <br/>
				<div class="form-group">
					...
					echo $this->Form->input('product_id', array('class' => 'form-control product_common_class','id'=>'product_id1','empty'=>'---- Select Product ----','label'=>false,'default'=>$product_id,'name'=>'data[ProductCombination][product_id][]',$disabled)); ?>
					<?php
					echo $this->Form->input('parent_slab_id', array('class' => 'form-control width_15','id'=>'slab_id1','empty'=>'---- Select Slab ----','label'=>false,'options'=>$slab_list,'name'=>'data[ProductCombination][parent_slab_id][]',$disabled)); ?>
                ...
                <div class="form-group"><span>
					<?php echo $this->Form->input('ProductCombination.min_qty', array('class' => 'form-control','required'=>true,'name'=>'data[ProductCombination][min_qty][]','label'=>'Combined Qty : ')); ?><!--<button class="add_qty_button" >Add More</button>--></span>
				</div>	
...
$(document).ready(function (){
    ...
    $(add_product_button).click(function(e){ 
        ...
        if(x < max_fields){ 
            x++;             
			$(product_wrapper).append('<div class="slap_set"><div class="form-group"><label>Product :</label><select class="form-control product_common_class" name="data[ProductCombination][product_id][]" id="product_id'+x+'" required><option value="">---- Select Product -----</option><?php echo $product_list; ?></select><select class="form-control width_15" name="data[ProductCombination][parent_slab_id][]" id="slab_id'+x+'" required><option value="">---- Select Slab -----</option></select><a href="#" class="remove_product_field btn btn-primary btn-xs">Remove</a></div>'); 
        }
