		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div class="search-box">
					<?php echo $this->Form->create('MaxMinSalesOutletReports', array('role' => 'form', 'action' => 'index')); ?>
					<table class="search">
						...
						<tr>
							<td colspan="2">
								<?php echo $this->Form->input('sales_value', array('legend' => 'Sales Value :', 'class' => 'sales_value', 'type' => 'radio', 'default' => 'max', 'options' => $sales_values));  ?>
							</td>
						</tr>
						<tr>
							<td colspan="2">
								<?php echo $this->Form->input('sales', array('legend' => 'Sales :', 'class' => 'sales', 'type' => 'radio', 'default' => 'sales_qty', 'options' => $sales, 'required' => true));  ?>
							</td>
						</tr>
						<tr>
							<td width="50%" class="required">
								<?php echo $this->Form->input('no_of_outlet', array('class' => 'form-control', 'default' => '10', 'type' => 'number', 'required' => true)); ?>
							</td>
						</tr>
						...
						<tr>
							<td colspan="2">
								<?php echo $this->Form->input('memo_value', array('legend' => 'Memo Value :', 'class' => 'memo_value', 'type' => 'radio', 'default' => '0', 'options' => $memo_values, 'required' => true));  ?>
							</td>
						</tr>

						<tr>
							<td class="columns_box selection_div">
						...
						<div class="table-responsive">
							...
							<div id="xls_body">
								<div style="float:left; width:100%; text-align:center; padding:20px 0; clear:both; font-weight:bold;">
									...
									<h3 style="margin:2px 0; text-transform:capitalize;">
										Selected <?= $sales_value ?> <?= $no_of_outlet ?> Sales Outlet(s)
									</h3>
									...
									<p>
										...
										<?php } ?>
										<?php if ($office_id) { ?>
											<span>, Area Office: <?= $offices_all[$office_id] ?></span>
										<?php } ?>
										<?php if ($territory_id) { ?>
								...
								<div style="float:left; width:100%; height:430px; overflow:scroll;">
									...
									<table cellspacing="0" cellpadding="10px" border="1px solid black" align="center" class="text-center report_table" id="sum_table">
										<thead>
											<tr class="titlerow">
												<th style="text-align:left;"><?= ($memo_value) ? 'Memo No' : 'Name' ?></th>
												...
												<th style="text-transform:capitalize;"><?= $sales_type == 'price' ? 'Value' : 'Qty' ?> (<?= $sales_value ?>)</th>
												<th>EC</th>
											</tr>
										</thead>
										...
										<tbody>
											...
											?>
												...
												<?php if ($column == 'product' && !$memo_value) { ?>

													<?php foreach ($results as $key_name => $outlet_datas) { ?>
														<?php
														$i = 0;
														...
														?>
															<tr>
																...
																<td style="text-align:left;"><?= $outlet_data['so_name'] ?></td>
																<td style="text-align:left;"><?= $outlet_data['territory_name'] ?></td>
																<td style="text-align:left;"><?= $outlet_data['office_name'] ?></td>
																<td style="text-align:left;"><?= $outlet_data['district_name'] ?></td>
																<td style="text-align:left;"><?= $outlet_data['thana_name'] ?></td>
																<td style="text-align:left;"><?= $outlet_data['market_name'] ?></td>
																<td style="text-align:left;"><?= $outlet_data['outlet_name'] ?></td>
																<td><?= sprintf("%01.2f", $outlet_data[$sales_type]) ?></td>
																<td><?= $outlet_data['ec'] ?></td>
															</tr>
														<?php
															$i++;
															$total += $outlet_data[$sales_type];
												...
												<?php } else { ?>
													...
													<?php foreach ($results as $qty => $result_datas) { ?>
														<?php foreach ($result_datas as $key_name => $outlet_datas) { ?>
															<?php
															$i = 0;
															foreach ($outlet_datas as $outlet_data) {
															?>
...
<script>
	$(document).ready(function() {
		...
		var memo_value = $('.memo_value:checked').val();
		if (memo_value == 1) {
			$('.selection_div').hide();
		} else {
			$('.selection_div').show();
		...
		$('.memo_value').change(function() {
			...
			if (this.value == 1) {
				...
				$('#MaxMinSalesOutletReportsColumnsProduct').prop('checked', true);
			} else {
				$('.selection_div').show();
				$('.selection_box').hide();
				$('#product').show();
...
<script>
	$(document).ready(function() {
		...
		$("#download_xl").click(function(e) {
			...
			a.download = "max_min_sales_outlet_report.xls";
