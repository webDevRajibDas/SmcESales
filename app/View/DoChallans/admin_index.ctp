		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<table id="Challan" class="table table-bordered table-striped">
					...
					<tbody>
					<?php 
					foreach ($challans as $challan): if($challan['Challan']['sender_store_id']== $this->Session->read('Office.store_id')||($challan['Challan']['receiver_store_id']==$this->Session->read('Office.store_id') && $challan['Challan']['status'] != 0)){?>
					<tr>
						...
						<td align="center"><?php  echo $challan['Challan']['status'] == 1 ? '<span class="btn btn-warning btn-xs">Pending</span>' :($challan['Challan']['status'] == 0?'<span class="btn btn-info btn-xs">Draft</span>':'<span class="btn btn-Sucess btn-xs">Received</span>'); ?></td>
						<td class="text-left"><?php echo h($challan['Challan']['remarks']); ?></td>
						<td class="text-center">
							<?php if($challan['Challan']['status']==0){?>
								<?php if($this->App->menu_permission('challans','admin_edit')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-edit"></i>'), array('action' => 'edit', $challan['Challan']['id']), array('class' => 'btn btn-info btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'edit')); } ?>
							<?php }?>
