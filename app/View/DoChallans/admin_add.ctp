<div class="row">
    <div class="col-md-12">
		<div class="box box-primary">
			...
			<table class="table table-striped table-condensed table-bordered" id="invoice_table">
				<thead>
					<tr>
...
<script>		
	$(document).ready(function(){
        let  invoiceTable = $('#invoice_table')
		...
		$('#requisition_id').change(function (){
			...
			$.ajax({
				...
				success: function(data) {
					//console.log(data);
                    invoiceTable.find('tbody').remove();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (obj.Product.maintain_batch == 0 && obj.Product.is_maintain_expire_date == 0) {
                            ...
                            $('select.batch_no_do').prop('required', false);
                            $('select.expire_date_do').prop('required', false);
                            $('#' + id).html(option);
                            var batch_no = '';
                            var expire_date = '';
                            var requisition_id = $('.requisition_id').val();
                            $.ajax({
                                ...
                                success: function (response) {
                                   var trId = $('#' + id).parent().parent().attr('id');
                                    ...
                                    $('#'+ trId + ' .batch_no_do').next('div').html(label);
                                   ...
                                   // $('#' + id).next().html(label);
                                    $('#'+ trId + ' .expire_date_do').next('div').html(label);
                                    $('#' + id).parent().prev().find('.selected_product_id').val(product_id + batch_no + expire_date);
                                }
                            });
                            ...
                            $.ajax({
                                url: '<?= BASE_URL . 'current_inventories/get_batch_list';?>',
                                type: 'post',
                                data: {'product_id': product_id, 'inventory_status_id': 1, 'with_stock': true},
                                success: function (data) {
		...
		$('body').on('change','.batch_no_do',function(){
			...
			$.ajax({
				...
				success:function(data) {
					...
					$("#expire_date_"+id).html(option);
				}

			});
		});

		$('body').on('change','.expire_date_do',function(){
			var id=$(this).attr('id');
			var requisition_id = $('.requisition_id').val();
			var product_id = id.split('_')[3];
			var batch_no = $("#"+id).parent().prev().children().val();
		...
		$(".draft").click(function(e){
			...
			$(".do_find_qty").each(function(){
				...
				if(quantity > do_quantity) {
					alert('Quantity shuld be match with DO quantity in line no '+sl);
					$(this).val(do_quantity);
					e.preventDefault();
					$(".draft").prop('disabled',true);
				}
