    ...
    <div class="col-md-12">
		<div class="box box-primary">
			...
			<div class="form-group">
				<?php echo $this->Form->input('receiver_store_id', array('id'=> 'receiver_store_id', 'class' => 'form-control','options'=>$distdistributors,'selected'=>$ncp_challan_draft_info[0]['Challan']['receiver_store_id'],'required'=>true)); ?>
			</div>
			<div class="form-group">
				<?php echo $this->Form->input('challan_id', array('label'=>'Challan No.','id'=>'challan_id','class' => 'form-control challan_id','options'=>$challan_list,'selected'=>$ncp_challan_draft_info[0]['Challan']['challan_referance_no'],'required'=>true)); ?>
			</div>
			<div class="form-group">
				<?php echo $this->Form->input('remarks', array('class' => 'form-control','value'=>$ncp_challan_draft_info[0]['Challan']['remarks'])); ?>
			</div>
			<div class="form-group">
				<?php echo $this->Form->input('is_close', array('class' => 'form-control','type'=>'checkbox','checked'=>$ncp_challan_draft_info[0]['Challan']['is_close'],'label'=>'<b>Is Challan Close :</b>')); ?>
			</div>
			<div class="form-group">
			...
			<table class="table table-striped table-bordered">
				...
				<tbody>
					<tr>
						<td>
							<?php echo $this->Form->input('product_id', array('id' => 'product_id','label'=>false, 'class' => 'full_width form-control product_id','empty'=>'---- Select Product ----','options'=>$product_list)); ?>
						</td>
			...
			<table class="table table-striped table-condensed table-bordered invoice_table">
				...
				<tbody>
				    ...
				    foreach ($ncp_challan_draft_info[0]['ChallanDetail'] as $ChallanDetail) {
				        $row_count_for_remove++;

						$value = $ChallanDetail['ChallanDetail'];
				        ?>
				        ...
				        <tr class="table_row" id="rowCount<?php echo $row_count_for_remove;?>">
				            ...
				            <td>
				                ...
				                <?php echo $ChallanDetail['Product']['name']; ?>
				            ...
				            <td align="center">
				                <?php echo $ChallanDetail['MeasurementUnit']['name']; ?>
				            ...
				            <td align="center" id="<?php echo $row_count_for_remove;?>">
				                <input type="number" step="any" min="0" name="quantity[<?php echo $row_count_for_remove;?>]" class="p_quantity product_quantity" id="product_quantity<?php echo $row_count_for_remove;?>" value="<?php echo $value['challan_qty']; ?>" required >
				            </td>
...
<script>		
	$(document).ready(function(){		
		...
		$('#receiver_store_id').selectChain({
			target: $('#challan_id'),
			value:'title',
			url: '<?= BASE_URL.'dist_ncp_product_issues/get_challan_list';?>',
			type: 'post',
		...
		$('#challan_id').selectChain({
			target: $('#product_id'),
			value:'title',
			url: '<?= BASE_URL.'dist_ncp_product_issues/get_challan_product_list';?>',
			type: 'post',
			...
			$.ajax({
				...
				success: function(response){						
					...
					if(obj.Product.maintain_batch == 0 && obj.Product.is_maintain_expire_date == 0){
						...
						$.ajax({
							type: "POST",
							url: '<?php echo BASE_URL;?>dist_ncp_product_issues/get_inventory_details',
		...
		$('#product_id').selectChain({
			...
			target: $('#batch_no'),
			value:'title',
			url: '<?= BASE_URL.'dist_ncp_product_issues/get_batch_list';?>',
			type: 'post',
		...
		$(".add_more").click(function() {	
			...
			if( quantity > ChallanQty ){
				alert('Please change your qty.');
				return false;
			}

			var batch_no = $('.batch_no').val(); 
			var expire_date = $('.expire_date').val();
			var stock_qty = $('.stock_quantity').text();
			...
			else
			{
				...
				$.ajax({
					...
					success: function(response){						
						var obj = jQuery.parseJSON(response);
						var recRow = '<tr class="table_row" id="rowCount'+rowCount+'"><td align="center">'+rowCount+'</td><td><input type="hidden" name="selected_product_id[]" class="selected_product_id" value="'+obj.Product.id+batch_no+expire_date+'"/>'+obj.Product.name+'<input type="hidden" name="product_id[]" value="'+obj.Product.id+'"/></td><td align="center"><input type="hidden" name="batch_no[]" value="'+batch_no+'">'+batch_no+'</td><td align="center"><input type="hidden" name="expire_date[]" value="'+expire_date+'">'+display_exp_date+'</td><td align="center">'+obj.ChallanMeasurementUnit.name+'<input type="hidden" name="measurement_unit[]" value="'+obj.Product.challan_measurement_unit_id+'"></td><td align="center" id="'+rowCount+'"><input type="number" step="any" min="0" name="quantity[]" class="p_quantity product_quantity" id="product_quantity'+rowCount+'" value="'+quantity+'"></td><td align="center" class="stock_qty" id="stock_qty'+rowCount+'">'+stock_qty+'</td><td align="center"><input type="text" class="full_width form-control" name="remarks[]"></td><td align="center"><button class="btn btn-danger btn-xs remove" value="'+rowCount+'"><i class="fa fa-times"></i></button></td></tr>'; 
