    ...
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header">
                <h3 class="box-title"><i class="glyphicon glyphicon-edit"></i>  <?php echo __('Distributor and Route/Beat Mapping'); ?></h3>
            ...
            <div class="box-body">		
                <?php echo $this->Form->create('DistRoute', array('role' => 'form','id'=>'distRouteMapping')); ?>
                <div class="form-group">
                    <?php echo $this->Form->input('office_id', array('class' => 'form-control', 'empty' => '---- Select ----', 'default' => $office_id, 'disabled' => TRUE)); ?>
                </div>
                <div class="form-group">
                    <?php echo $this->Form->input('dist_distributor_id', array('label' => 'Distributor', 'class' => 'form-control', 'empty' => '---- Select ----', 'default' => $dist_distributor_id, 'disabled' => TRUE)); ?>
                </div>
                <?php echo $this->Form->input('office_id', array('class' => 'form-control', 'empty' => '---- Select ----', 'default' => $office_id, 'type' => 'hidden')); ?>
                <?php echo $this->Form->input('dist_distributor_id', array('label' => 'Distributor', 'class' => 'form-control', 'empty' => '---- Select ----', 'default' => $dist_distributor_id, 'type' => 'hidden')); ?>
                <br><br>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                if (array_key_exists($dis_route_id, $mappingData)) {
                                    ...
                                    ?>
                                    <input type="hidden" value="<?= $mappingData[$value['DistRoute']['id']]['dist_route_id'] ?>" name="data[DistRouteMapping][dist_route_id][<?= $dis_route_id ?>][dist_route_id]" />
                                    <input type="hidden" id="pre_effective_id_<?= $dis_route_id ?>"  value="<?= $mappingData[$value['DistRoute']['id']]['effective_date'] ?>" name="data[DistRouteMapping][dist_route_id][<?= $dis_route_id ?>][effective_date]" />
                                    <input type="hidden" value="<?= $mappingData[$value['DistRoute']['id']]['created_at'] ?>" name="data[DistRouteMapping][dist_route_id][<?= $dis_route_id ?>][assign_date]" />
                                    <input type="hidden" id="pre_effective_date_<?= $dis_route_id ?>"  value="<?= $mappingData[$value['DistRoute']['id']]['effective_date'] ?>" name="data[DistRouteMapping][pre_date][<?= $dis_route_id ?>][effective_date]" />
                                    <?php
                                }
                                else {
                                    
                                    $dist_beat_history = $obj->get_dist_mapping_end_date($office_id, $dist_distributor_id,$dis_route_id); 
                                    ...
                                    if(!empty($dist_beat_history) && $dist_beat_history['end_date'])
                                                               ...
                                                               ?>
                                  <input type="hidden" id="end_effective_date_<?= $dis_route_id ?>"  value="<?=$end_at?>" name="data[DistRouteMapping][end_date][<?= $dis_route_id ?>][effective_date]" />
                                <?php 
                            ...
                            <td class="text-center">
                                <div class="form-group">
                                    <?php echo $this->Form->input('effective_date', array('id' => "effective_date_$dis_route_id", 'label' => false, 'value' => (array_key_exists($dis_route_id, $mappingData)) ? $mappingData[$value['DistRoute']['id']]['effective_date'] : '', 'name' => "data[DistRoute][effective_date][$dis_route_id]", 'class' => 'form-control datepicker action_date','autocomplete'=>'off','style' => 'width:100%'));
                                    ?>
                                </div>
                            </td>
                        </tr> 
...
/*Challan Datepicker : End*/
    ...
    $(document).ready(function () {
          ...
          $("input[type='submit']").on("click", function(){
                ...
                $("form#distRouteMapping").submit(function(e){
