    ...
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header">
                <h3 class="box-title"><i class="glyphicon glyphicon-edit"></i> <?php echo __('Add/Edit GSP Program'); ?>
                </h3>
                <div class="box-tools pull-right">
                    <?php echo $this->Html->link(__('<i class="glyphicon glyphicon-th-large"></i> GSP Program List'), array('action' => 'pchp_program_list'), array('class' => 'btn btn-primary', 'escape' => false)); ?>
                </div>
            </div>
            <div class="box-body">
				...
				if(!empty($outlets)){
					//print_r($outlets);	
				?>
                <?php echo $this->Form->create('Program', array('role' => 'form','action'=>'add_pchp_list')); ?>
                <input type="hidden" name="territory_id"
                    value="<?=$this->request->data['Program']['territory_id'];?>" />
                ...
                <table class="table table-bordered table-striped outlet_list">
					...
					$id = $val['Outlet']['id'];
						$program_info = $ProgramsController->get_program_info($doctor_id=0, $request_data, $outlet_id=$id, $program_type_id=1);

					?>
                    <tr>
                        <td class="text-center">

                            <input type="checkbox" class="outlet_id" name="outlet_id[<?=$id;?>]" value="<?=$id;?>"
                                <?=($program_info)?'checked':'';?> />

                            <input type="hidden" name="program_id[<?=$id;?>]"
                                value="<?=($program_info)?$program_info['Program']['id']:'';?>" />
                        </td>
                        ...
                        <td style="display:none;">
                            <input type="radio" class="member_type_<?=$id;?>" name="member_type[<?=$id;?>]" value="1"
                                <?php if($program_info['Program']['member_type'] == 1){ echo 'checked'; } ?> /> &nbsp;
                            <?php echo $val['Outlet']['in_charge']; ?>
                        </td>
                        <td style="display:none;">
                            <input type="radio" class="member_type_<?=$id;?>" name="member_type[<?=$id;?>]" value="2"
                                <?php if($program_info['Program']['member_type'] == 2){ echo 'checked'; } ?> /> &nbsp;
                            <?php echo $val['Outlet']['ownar_name']; ?>
                        </td>


                        <td>
                            <input type="text" class="form-control code_<?=$id;?>" name="code[<?=$id;?>]"
                                value="<?=($program_info)?$program_info['Program']['code']:'';?>" />
                        </td>
                        <td>
                            <input type="text" class="form-control assigned_date_<?=$id;?> datepicker"
                                name="assigned_date[<?=$id;?>]"
                                value="<?=($program_info)?date('d-m-Y',strtotime($program_info['Program']['assigned_date'])):'';?>" />
