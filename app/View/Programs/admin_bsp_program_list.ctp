...
<div class="row">
	<div class="col-xs-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-th-large"></i> <?php echo __('BSP Program List'); ?></h3>
				<div class="box-tools pull-right">
					<?php if ($this->App->menu_permission('programs', 'admin_add_bsp')) {
						echo $this->Html->link(__('<i class="glyphicon glyphicon-plus"></i> Add/Edit BSP Program'), array('action' => 'add_bsp'), array('class' => 'btn btn-primary', 'escape' => false));
					} ?>
				</div>
			</div>
			<div class="box-body">
				<div class="search-box">
					...
					<table class="search">
						<tr>
							<td width="50%">
								<?php echo $this->Form->input('program_type_id', array('type' => 'hidden', 'value' => 2)); ?>
					...
					<tbody>
						<?php foreach ($programs as $program) : ?>
							<tr office-id="<?php echo($program['Office']['id']); ?>" data-id="<?php echo $program['Program']['id']?>">
								...
								<td class="text-center"><?php echo h($program['Program']['program_officer']); ?></td>
								...
								<td class="text-center">
									<?php if ($this->App->menu_permission('programs', 'admin_edit_bsp')) {
										echo $this->Html->link(__('<i class="glyphicon glyphicon-pencil"></i>'), array('action' => 'edit_bsp', $program['Program']['id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => 'Deassigned'));
									} ?>
	...
	<div class="modal fade" id="programOfficerModal">
		<div class="modal-dialog">
			...
			<div class="form-group">
				<label>Program Officer</label>
					<select class="form-control" id="progOfficerName">
				</select>
			</div>
...
<script>
	$(document).ready(function() {
		...
		$(".programOfficerAdd").click(function () {
            ...
            var programOfficerName = currentRow.find("td:eq(9)").text();
            if ((programOfficerName == null || programOfficerName=='') ){
                $.ajax({
                    type: "POST",
			...
			$.ajax({
				...
				success: function (response) {
					//console.log(response);
					$.each(response,function(key, value){
                    $("#progOfficerName").append('<option value=' + key + '>' + value + '</option>');
                });
					
				}
			});
            $('#programOfficerModal').modal('toggle');
        });

		$(".program_officer_update").click(function () {
			var programOfficerId = $('#progOfficerName').val();
