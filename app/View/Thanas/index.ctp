			...
			<div class="box-body">
                <table id="Thanas" class="table table-bordered table-striped">
					...
					<tr>
						...
						<td class="text-center"><?php echo h($thana['Thana']['name']); ?></td>
						<td class="text-center">
			<?php echo $this->Html->link($thana['District']['name'], array('controller' => 'districts', 'action' => 'view', $thana['District']['id'])); ?>
		</td>
						<td class="text-center"><?php echo h($thana['Thana']['is_active']); ?></td>
						<td class="text-center">
							<?php if($this->App->menu_permission('thanas','admin_view')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-eye-open"></i>'), array('action' => 'view', $thana['Thana']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'view')); } ?>
