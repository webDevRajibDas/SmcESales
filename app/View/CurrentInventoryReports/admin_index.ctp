                    ...
                    <tr>
                            <td colspan="2">
                                ...
                                <div id="market_list" class="input select" style="float:left; width:80%; padding-left:20px;">
                                    ...
                                    <div class="selection">
                                        <?php echo $this->Form->input('product_type', array( 'id' => 'product_type_id', 'label' => false, 'class' => 'checkbox product_type', 'multiple' => 'checkbox', 'options' => $product_type_list)); ?>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    <table id="sum_table" style="width:100%;text-align:center; margin-bottom: 50px;" border="1px solid black" cellpadding="0px" cellspacing="0" align="center" class="table table-bordered table-responsive">
                        ...
                        <tbody>
                            ...
                            { 
                                ...
                                <tr>
                                    <td style="text-align:left">Bonus/Gift</td>
                                    <?php
                                    foreach ($products as $value) {
                                        ?>
                                        <td class="qty">
...
<script>
    ...
    $(document).ready(function(){
        $('#searchOfficeId').change(function() {
            $.ajax({
                type: "POST",
                url: '<?=BASE_URL?>current_inventory_reports/get_territory_so_list',
        ...
        $("#download_xl").click(function(e){
            ...
            var html = $("#content").html();
                            ...
                            a.href = downloadUrl;
                            a.download = "current_inventory_reports.xls";
                            document.body.appendChild(a);
                            a.click();
        });

        get_product_list($(".product_type input:checked").serializeArray());
        $(".product_type").change(function() {
            product_type = $(".product_type input:checked").serializeArray();
            console.log(product_type);
            get_product_list(product_type);
        });
        var product_check = <?php echo @json_encode($this->request->data['search']['product_id']); ?>;
        console.log(product_check + 'frabbi');

        function get_product_list(product_type) {
           
           $.ajax({
               type: "POST",
               url: '<?= BASE_URL ?>current_inventory_reports/get_product_list',
               data: product_type,
               cache: false,
               success: function(response) {
