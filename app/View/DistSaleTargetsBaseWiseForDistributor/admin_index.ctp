                    ...
                    <tbody>
                        <tr>
                            <td class="text-center">
                                <div class="form-group">
                                    <?php
                                    echo $this->Form->input('quantity', array('label' => '', 'class' => 'sales form-control sales_target', 'value' => (isset($saletarget['DistSaleTarget']['quantity'])) ? $saletarget['DistSaleTarget']['quantity'] : 0, 'disabled', 'type' => 'text', 'required' => false));
                                    ?>
                                </div>
                            </td>
                            <td class="text-center">
                                <div class="form-group">
                                    <?php
                                    echo $this->Form->input('amount', array('label' => '', 'class' => 'sales form-control sales_target sales_target_amount', 'value' => (isset($saletarget['DistSaleTarget']['amount'])) ? $saletarget['DistSaleTarget']['amount'] : 0, 'disabled', 'type' => 'text', 'required' => false));
                                    ?>
                                </div>
                            </td>
                            <td class="text-center">
...
<script>
    function get_dist_distributor_list(office_id) {
        if (office_id != '') {
            $.ajax({
    ...
    function sales_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId) {
        if (FiscalYearId != '' && product_id != '' && SaleTargetAsoId != '') {
            $.ajax({
                type: "POST",
                url: "<?php echo BASE_URL; ?>admin/DistSaleTargetsBaseWise/sales_base_wise_data",
                data: {FiscalYearId: FiscalYearId, product_id: product_id, SaleTargetAsoId: SaleTargetAsoId},
                success: function (response) {
                    response = jQuery.parseJSON(response);
                    $("input[name='data[DistSaleTarget][amount]'").val(response.amount);
                    $("input[name='data[DistSaleTarget][quantity]'").val(response.qty);
                }
            });
        } else {
    ...
    function get_sales_target_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId, dist_distributor_id) {
        if (FiscalYearId != '' && product_id != '' && SaleTargetAsoId != '' && dist_distributor_id != '') {
            $.ajax({
                type: "POST",
                url: "<?php echo BASE_URL; ?>admin/DistSaleTargetsBaseWise/get_sales_target_base_wise_data/",
                data: {SaleTargetAsoId: SaleTargetAsoId, product_id: product_id, FiscalYearId: FiscalYearId, dist_distributor_id: dist_distributor_id},
                success: function (response) {
                    //var obj = jQuery.parseJSON(response);
                    $('#data_table').html(response)
                    $('.save').show();
                    if ($("input[name='data[DistSaleTarget][amount]'").val() < 1 && $("input[name='data[DistSaleTarget][quantity]'").val() < 1) {
    ...
    $(document).ready(function () {
        $("body").on("change", "#DistSaleTargetProductId", function () {
            ...
            sales_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId);
            get_sales_target_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId,dist_distributor_id);
        });
        $("body").on("change", "#DistSaleTargetFiscalYearId", function () {
            var FiscalYearId = $(this).val();
            var product_id = $("#DistSaleTargetProductId").val();
            var SaleTargetAsoId = $("#DistSaleTargetAsoId").val();
            var dist_distributor_id = $("#dist_distributor_id").val();
            sales_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId);
            get_sales_target_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId,dist_distributor_id);
        });
        $("body").on("change", "#DistSaleTargetAsoId", function () {
            var FiscalYearId = $("#DistSaleTargetFiscalYearId").val();
            var product_id = $("#DistSaleTargetProductId").val();
            var SaleTargetAsoId = $(this).val();
            var dist_distributor_id = $("#dist_distributor_id").val();
            get_dist_distributor_list(SaleTargetAsoId);
            sales_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId);
            get_sales_target_base_wise_data(FiscalYearId, product_id, SaleTargetAsoId,dist_distributor_id);
        });
...
<script>
    $(document).ready(function () {
        ...
        $("body").on("input", ".quantity", function () {
            ...
            var target_quantity_value = $("#DistSaleTargetQuantity").val();
        ...
        $("body").on("input", ".amount", function () {
            ...
            var target_amount_value = $("#DistSaleTargetAmount").val();
        ...
        if ($("#DistSaleTargetFiscalYearId").val() && $("#DistSaleTargetAsoId").val())
        {
            $('#dl_exel').show();
            $('#dl_exel_month').show();
        } else
        ...
        $("#DistSaleTargetAsoId").change(function () {
            var FiscalYearId = $("#DistSaleTargetFiscalYearId").val();
            var SaleTargetAsoId = $("#DistSaleTargetAsoId").val();
            if (FiscalYearId && SaleTargetAsoId)
            {
        ...
        $('#dl_exel').click(function () {
            var fiscalYearid = $("#DistSaleTargetFiscalYearId").val();
            var SaleTargetAsoId = $("#DistSaleTargetAsoId").val();
            window.open("<?= BASE_URL; ?>DistSaleTargetsBaseWise/download_xl/" + fiscalYearid + "/" + SaleTargetAsoId);
        });
        $('#dl_exel_month').click(function () {
            var fiscalYearid = $("#DistSaleTargetFiscalYearId").val();
            var SaleTargetAsoId = $("#DistSaleTargetAsoId").val();
            window.open("<?= BASE_URL; ?>DistSaleTargetsBaseWise/download_xl_month/" + fiscalYearid + "/" + SaleTargetAsoId);
        });
    });
