	...
	<div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-edit"></i> <?php echo __('Edit Product'); ?></h3>
			...
			<div class="box-body">
				...
				<ul class="nav nav-tabs" role="tablist">
					<li role="presentation" class="active product_tab"><a href="#info" aria-controls="info" role="tab" data-toggle="tab">Product Info</a></li>
				...
				<div class="tab-content">
					...
					<div role="tabpanel" class="tab-pane active" id="info">
						...
						<div class="form-group">
							<?php echo $this->Form->input('product_category_id', array('id' => 'category_id', 'class' => 'form-control', 'empty' => '---- Select Category -----')); ?>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<?php echo $this->Form->input('product_type_id', array('class' => 'form-control',  'empty' => '---- Select Type -----')); ?>
						</div>
						<!--div class="form-group gift_sample_product_div" "onchange"=>"gift_sample_product_fuction()",>
							<?php echo $this->Form->input('gift_sample_product_id', array('label' => 'Gift Sample Parent Product', 'empty' => '---- Select ----', 'class' => 'form-control', 'options' => $giftsampleprodutct)); ?>
						</div-->
						<div class="form-group">
							<?php echo $this->Form->input('is_pharma', array('class' => 'form-control', 'type' => 'checkbox', 'label' => '<b>Is Pharma :</b>')); ?>
						</div>
						<div class="form-group">
							<?php echo $this->Form->input('maintain_batch', array('class' => 'form-control', 'type' => 'checkbox', 'label' => '<b>Maintain Batch :</b>')); ?>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<?php echo $this->Form->input('product_image', array('type' => 'file', 'class' => 'form-control', 'accept' => '.jpg, .jpeg,.png')); ?>
							<?php echo $this->Form->input('prev_product_image', array('type' => 'hidden', 'value' => $this->request->data['Product']['product_image'])); ?>
						</div>
					</div>
					<div role="tabpanel" class="tab-pane" id="unit">
						...
						<span class="input_fields_wrap">
							...
							if (!empty($this->request->data['ProductMeasurement'])) {
								foreach ($this->request->data['ProductMeasurement'] as $key => $val) {
							?>
									<div class="form-group">
										<?php echo $this->Form->input('ProductMeasurement.product_measurement_unit_id.', array('type' => 'hidden', 'value' => $val['id'])); ?>
										<?php echo $this->Form->input('ProductMeasurement.update_measurement_unit_id.', array('label' => 'Unit :', 'style' => 'width:15% !important;', 'class' => 'form-control unit', 'type' => 'select', 'value' => $val['measurement_unit_id'], 'options' => $baseMeasurementUnits, 'required' => false)); ?>
										<?php echo $this->Form->input('ProductMeasurement.update_qty_in_base.', array('label' => false, 'style' => 'margin-left:2%;width:13% !important;', 'class' => 'form-control', 'type' => 'number', 'value' => $val['qty_in_base'], 'required' => false)); ?>
									</div>
								<?php
								}
								?>
							...
							<div class="col-xs-6 col-xs-offset-1">
								...
								<table class="table">
									...
									<tbody class="fraction_slab">
										<?php
										if (!empty($this->request->data['ProductFractionSlab'])) {
											$fr = 0;
											foreach ($this->request->data['ProductFractionSlab'] as $key => $val) {
										?>
												<tr>
													<td width="25%" class="text-center">
														<?php echo $this->Form->input('ProductFractionSlab.fraction_sales_unit.' . $fr, array('label' => false, 'required' => false, 'class' => "sales_qty", 'default' => $val['sales_qty'])); ?>
													</td>
													<td width="25%" class="text-center">
														<?php echo $this->Form->input('ProductFractionSlab.fraction_base_unit.' . $fr, array('label' => false, 'required' => false, 'class' => "base_qty", 'default' => $val['base_qty'])); ?>
													</td>
													<td class="text-center">
														<?php echo $this->Form->input('ProductFractionSlab.fraction_is_sales.' . $fr, array('type' => 'checkbox', 'label' => false, 'default' => 0, 'required' => false, 'default' => $val['use_for_sales'])); ?>
													</td>
													<td class="text-center" width="15%" style="border-right: none;">
														<?php echo $this->Form->input('ProductFractionSlab.fraction_is_bonus.' . $fr, array('class' => "checkbox-inline", 'type' => 'checkbox', 'label' => false, 'default' => 0, 'required' => false, 'default' => $val['use_for_bonus'])); ?>
														<?php echo $this->Form->hidden('ProductFractionSlab.id.' . $fr, array('label' => false, 'default' => $val['id'])); ?>
													</td>
													<?php if ($fr == 0) { ?>
...
<script>
	...
	$(document).ready(function() {
		...
		$("a[href='#fraction_slab']").on('shown.bs.tab', function(e) {
			...
			sales_base_value = $(".input_fields_wrap select option:selected[value='" + sales_measurement_unit_id_for_fraction + "']").parent().parent().next().find('input').val();
			// sales_base_value= $(".input_fields_wrap select option[selected][value='"+sales_measurement_unit_id_for_fraction+"']").parent().parent().next().find('input').val();
		...
		$(".chosen").chosen();
		<?php if ($this->request->data['Product']['is_virtual'] != 1) { ?>
