                        ...
                        <tr>
                            <td colspan="2">
                                <?php echo $this->Form->create('Product', array(
                                    'url' => array('controller' => 'products', 'action' => 'admin_view')));  ?>
                                <!------parent product------->
                                <div class='form-group'>
                                    <?php echo $this->Form->input('is_virtual', array('class' => 'form-control is_virtual', 'type' => 'checkbox', 'label' => '<b>Is Virtual :</b>', 'default' => 0)); ?>
                                </div>
                                <div class="form-group mother_product_box">
                                    <?php echo $this->Form->input('Product.id', array('type'=>'select', 'label'=>'Parent', 'options'=>$parent_products, 'default'=>'--------Select--------')); ?>

                                </div>

                                <div class='submit'>
                                    <?php echo $this->Form->button('Update', array('class' => 'btn btn-large btn-primary')); ?>
                                </div>
...
<script>
    ...
    $(document).ready(function() {
        console.log(<?php echo $product['Product']['is_virtual'] ?>);

        $('.chosen').chosen();
        <?php if ($product['Product']['is_virtual'] !=1) { ?>
            $(".mother_product_box").hide();
        <?php } ?>

        if ($('input.is_virtual').is(':checked')) {
            $('.mother_product_box').show();
        }
