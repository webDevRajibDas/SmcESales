				...
				<?php echo $this->Form->create('search', array('role' => 'form')); ?>
					<table class="search">
						...
						<tr>
						  ...
						  <tr>
                         <td colspan="2">
                            <label style="float:left; width:15%;">Territory : </label>
                            <div id="market_list" class="input select" style="float:left; width:80%; padding-left:20px;">
                                ...
                                <div class="selection2 so_list">
                                    <?php echo $this->Form->input('territory_id', array('label'=>false, 'class' => 'checkbox', 'multiple' => 'checkbox', 'options' => $territory_list)); ?>
                                </div>
                            </div>
                        </td>
                    </tr>
					...
					<div class="row" >
						<div id="content" style="height:400px;overflow: scroll;width:90%;margin-left:5%;margin-right:5%;">
							...
							<table class="table  table-striped border" style="font-size:12px;" border="1">
								<thead>
									<tr>
										<td rowspan="2" class="text-center">Territory</td>
										<td rowspan="2" class="text-center">Outlet</td>
										<td rowspan="2" class="text-center">Market</td>
										<td rowspan="2" class="text-center">Thana</td>
								...
								<tbody>
									...
									{
										$last_territory_id=$data['territory_id'];
									} 
									if($last_territory_id && $last_territory_id != $data['territory_id']){ ?>
										<tr>
...
echo $this->fetch('css');?>');
    ...
    $(document).ready(function(){
    	$("#fiscal_year_id").change(function(){
    		$.post('<?=BASE_URL.'bonus_summery_report/get_bonus_card'?>',{'fiscal_year_id':$(this).val()}, function(data,status){
    	...
    	$('#searchOfficeId').change(function() {
            $.ajax({
                type: "POST",
                url: '<?=BASE_URL?>bonus_summery_report/get_territory_so_list',
