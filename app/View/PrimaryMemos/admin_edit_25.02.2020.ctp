...
<div class="row">
    ...
    <div class="box-body">   
        ...
        <div class="form-group">
            <?php echo $this->Form->input('receiver_store_id', array('class' => 'form-control','options'=>$client_name,'default'=>$challan_info[0]['PrimaryMemo']['sender_store_id'],'required'=>true,)); ?>
        </div>
        <div class="form-group">
            <?php echo $this->Form->input('challan_referance_no', array('class' => 'form-control challan_referance_no', 'value'=>$challan_info[0]['PrimaryMemo']['challan_referance_no'], 'required'=>true)); ?>
        </div>
        <div class="form-group">
              ...
              <tbody>
                ...
                foreach ($challan_info[0]['PrimaryMemoDetail'] as $value) {
                  ...
                  <tr class="table_row" id="rowCount<?php echo $row_count_for_remove;?>">
                      ...
                      <td align="center">
                          <input type='text' name="quantity[<?php echo $row_count_for_remove;?>]" class="<?php echo $value['product_id'];  ?>_product_qty p_quantity" value="<?php echo $value['challan_qty']; ?>" required onchange="count_total_value(<?php echo $value['product_id'];  ?>)">
                      </td>
                      <td align="center" id="quantity" class="<?php echo $value['product_id']; ?>">
                          <?php echo $value['challan_qty']; ?>
                      </td>
                      <td align="center">
                          <input type="text" name="price[<?php echo $row_count_for_remove;?>]" class="<?php echo $value['product_id']; ?>product_price" value="<?php echo $value['product_price']; ?>" required  onchange="count_total_value(<?php echo $value['product_id'];  ?>)"> 
                     </td>
                     <td align="center">
                    ...
                    <td align="center">
                      ...
                      <td align="center">
                         <p class="total_price_<?php echo $value['product_id']; ?>"><?php echo $value['challan_qty']*$value['product_price']; ?> </p>
      ...
      
      {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            <td align="center" >\
              <input type="hidden" name="price[' + rowCount + ']" class="total_product_price" value="' + price + '"><p class="total_price_' + product_id +'">' + price + ' </p>\
            </td>\
            <td align="center">\
...
<script>
    $(document).ready(function()
    {
      $('body').on("blur", ".challan_referance_no", function challanReferance(){
        ...
        var primary_memo_id = <?=$challan_info[0]['PrimaryMemo']['id']?>;

      $.ajax({
          url: '<?php echo BASE_URL.'admin/primarymemos/challan_referance_validation' ?>',
          'type': 'POST',
        data: {
          primary_memo_id: primary_memo_id,
        ...
        success: function(result)
        {
          ...
          if(obj == 1){
            alert('Challan Referance Number Already Exist');                       // $('.draft').prop('disabled', true);
            }
              if(obj == 0){
                //$('.draft').prop('disabled', false);
                 }
    ...
    function count_product_total_value(id){
       var price = $('.'+id+'product_price').val();
       var qty = $('.'+id+'_product_qty').val();
       var totalprice = qty * price;
