		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div class="form-group">
					<?php echo $this->Form->input('sender_store_id', array('class' => 'form-control', 'label' => 'Sender :', 'options' => $sender, 'required' => true)); ?>
				</div>
				<div class="form-group">
					<?php echo $this->Form->input('receiver_store_id', array('class' => 'form-control', 'label' => 'Receiver :', 'options' => $receiver, 'required' => true)); ?>
				</div>
				<div class="form-group">
					<?php echo $this->Form->input('challan_referance_no', array('class' => 'form-control challan_referance_no')); ?>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					<?php echo $this->Form->input('delivery_point', array('class' => 'form-control', 'rows' => 1)); ?>
				</div>
				<div class="form-group">
					<?php echo $this->Form->input('remarks', array('class' => 'form-control', 'rows' => 1)); ?>
				</div>
			...
			/*else if (is_maintain_expire_date == 1 && expire_date == '')
			{
				...
				$.ajax({
					...
					success: function(response) {
						...
						<td align="center" class="text-center hidden">\
							<input type="hidden" name="batch_no[' + rowCount + ']" value="' + batch_no + '" required>' + batch_no + '\
						</td>\
						<td align="center" class="text-center hidden">\
							<input type="hidden" name="expire_date[' + rowCount + ']" class="p_expire_date" value="' + expire_date + '">' + expire_date + '\
						</td>\
						<td align="center">\
							<input  name="quantity[' + rowCount + ']" onchange="count_total_value(' + obj.Product.id + ',this)" class="' + product_id + '_product_qty  p_quantity " value="' + quantity + '"  >\
						</td>\
						...
						<td align="center" >\
							<p class="ttprice total_price_' + product_id + '">' + price + ' </p>\
						</td>\
						<td align="center">\
...
<script>
	...
	function product_total_price() {
		...
		//var famount = parseInt(amount) + parseInt(camount) ;
