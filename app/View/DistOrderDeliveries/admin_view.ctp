					...
					<table class="table table-bordered">
						<tbody>
							...
							if (!empty($order_details)) {
								$sl = 1;
								foreach ($order_details as $val) {
									if (!empty($val['DistOrderDetail']['bonus_qty'])) {
										if ($val['DistOrderDetail']['bonus_qty'] > 0) {
							?>
											<tr>
												<td align="center"><?php echo $sl; ?></td>
												<td><?php echo $val['Product']['name']; ?></td>
												<td align="center"><?php echo $val['DistOrderDetail']['bonus_qty']; ?></td>
											</tr>
								<?php
											$sl++;
										}
