        ...
        <div class="box box-primary">
            <div class="box-header">
                ...
                <div class="box-tools pull-right">
					<?php if($this->App->menu_permission('OpeningBalanceCollections', 'admin_index')){ echo $this->Html->link(__('Collection List'), array('action' => 'index'), array('class' => 'btn btn-primary', 'escape' => false)); } ?>
				</div>
            </div>	
            ...
            <div class="box-body">
                ...
                <?php echo $this->Form->create('OpeningBalanceCollection', array('role' => 'form')); ?>
                    ...
                    <div class="form-group">
                        <?php echo $this->Form->input('opening_balance_id', array('onChange' => 'getOutstandingByOpeningBalanceID();', 'id' => 'opening_balance_id', 'selected' => $area_office_id, 'label'=>'Opening Balance Territory', 'class' => 'form-control', 'empty'=>'---- Select ----')); ?>
                    </div>
                    
                    
                    <div class="form-group">
						<?php echo $this->Form->input('due_outstanding', array('label'=>'Market Outstanding', 'readonly'=>'true', 'id' => 'due_outstanding', 'type' => 'text', 'class' => 'form-control', 'required' => TRUE)); ?>
                    </div>
...
{	
	...
	if(office_id)
	{
		$.ajax({
			type: "POST",
			url: '<?= BASE_URL . 'opening_balance_collections/get_outstanding_by_opening_balance_id'?>',
			data: 'opening_balance_id='+opening_balance_id+'&office_id='+office_id+'&fiscal_year_id='+fiscal_year_id,
			cache: false, 
			success: function(response){
				//alert(response);
				$('#due_outstanding').val(response);
			}
		});		
	}
	else
