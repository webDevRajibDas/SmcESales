...
<div class="row">
    <div class="col-md-12">
      ...
      <div class="form-group">
          <?php echo $this->Form->input('order_date', array('class' => 'form-control datepicker', 'type' => 'text', 'required'=>TRUE,  'value'=>$existing_record['order_date'], 'disabled')); ?>
          ...
          if(!empty($existing_record)){
            ...
            foreach($existing_record['OrderDetail'] as $val){
              ...
              <tr id="<?php echo $sl?>" class="new_row_number">
               ...
               <th class="text-center" width="12%">
                <?php if(array_key_exists($val['product_id'],$not_in_stock_product)){?>
                   <input type="number" step="any" min="0"  max="<?=$val['deliverd_qty']?>" name="data[OrderDetail][deliverd_qty][]" class="form-control width_100 min_qty sales_qty" value="0"  style="outline: 2px solid red;" required readonly/>
              <?php } else {?>
...
<script>
    ...
    $("body").on("keyup", ".min_qty", function () {
       ...
       console.log(pro_val);
       //alert(pro_val);
       //checkInventory(pro_val);
       //checkInventoryByProduct(pro_val);
