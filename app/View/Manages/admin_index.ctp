					...
					<tr style="background-color:<?php echo $order['Order']['from_app']==0 ? '#f5f5f5':'white'?>">
					   ...
					   <!--  <td align="center">
						...
						elseif($order['Order']['status']==1){
							echo '<span class="btn btn-warning btn-xs draft">Pending</span>';
						}
						else
						{
							echo '<span class="btn btn-success btn-xs draft">Order Confirmed</span>';
						}*/
						?>
						</td> -->
						...
						<td class="text-center" width="15%">
							...
							<?php if($this->App->menu_permission('manages','admin_view')){ 
								...
								<?php //if($order['Order']['is_closed'] != 1){ //is_closed
								if($order['Order']['confirm_status'] != 2){ //is_closed
								if($this->App->menu_permission('manages','admin_edit')){
									...
									/*array('action' => 'delivery', $order['Order']['id']), 
									array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'Delivery'));*/
								?>
							<?php } ?>
							...
							if($office_parent_id && $dare_diff<=3){
								if($this->App->menu_permission('manages','admin_order_editable') && $order['Order']['order_editable']!=1)
								{ 
							...
							?>
							<?php if($this->Session->read('Office.group_id')== 2){?>
							 <?php //if($this->App->menu_permission('closed','admin_confirm')){ 
								if($order['Order']['is_closed'] != 1){
								//echo $this->Form->button(__('<i class="glyphicon glyphicon-eye-open"></i>'), array('class' => 'btn btn-info btn-xs', 'escape' => false, 'id'=>'closed_'.$order['Order']['id'],'value'=>$order['Order']['id'] ,'data-toggle'=>'tooltip', 'title' => 'closed')); 
							 //} ?>
							<!--  <a class="btn btn-info btn-xs closed" value=<?=$order['Order']['id']?>  onclick='focefullyclosed(<?php echo $order['Order']['id']?>)'><i class="glyphicon glyphicon-eye-open"></i></a> -->
						 <?php } } ?>

							 <?php 

								if($order['Order']['confirmed'] != 1 && $order['Order']['status'] == 2){
