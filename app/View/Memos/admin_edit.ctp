	...
	<div class="col-md-12">
		<div class="box box-primary">
			...
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-edit"></i> <?php echo __('Edit Memo'); ?></h3>
			...
			<div class="box-body">
				...
				<?php echo $this->Form->input('sale_type_id', array('type'=>'text', 'class' => 'form-control', 'id' => 'sale_type_id', 'value' => ($user_group_id==1008)?'SPO Sales':'SO Sales', 'disabled')); ?>
				<?php echo $this->Form->input('sale_type_id', array('class' => '', 'required'=>TRUE, 'type' => 'hidden', 'value'=> '1')); ?>
			</div><?php */
			...
			//  pr($markets);exit;?>
				...
				<div class="form-group">
					<?php echo $this->Form->input('sale_type_id_d', array('class' => 'form-control sale_type_id', 'id' => 'sale_type_id_d', 'selected' => $sale_type_id, 'options' => $sale_type_list, 'disabled')); ?>
				</div>
				<?php echo $this->Form->input('sale_type_id', array('id' => 'sale_type_id', 'class' => '', 'required' => TRUE, 'type' => 'hidden', 'value' => $sale_type_id)); ?>
				<?php
				if($sale_type_id==4){
					?>
					<div class="form-group">
						<div class="input select required">
							<label for="program_officer_id">Programm Officer :</label>
							<?php if( $usergroupid != 1016 ){ ?>
							<select name="program_officer_id" id="program_officer_id" class="form-control select2auto" data-parrent="<?=$existing_record['office_id']?>" data-limit="30" data-route="GetProgramOfficerId" data-allowClear="false" data-placeholder="Select a category first!">
								<?php
								if(isset($prom_officer)){
									echo '<option value="'.$prom_officer['id'].'">'.$prom_officer['name'].'</option>';
								}else{
									echo '<option value=""></option>';
								}
								?>
							</select>
							<input type="hidden" name="exist_program_officer_id" value="<?=$prom_officer['id'];?>"/>
							<?php }else{ ?>
								<input type="hidden"  name="exist_program_officer_id" value="<?=$this->UserAuth->getUserId();?>">
				...
				<div class="form-group" id="territory_id_div">
					<?php echo $this->Form->input('territory_id', array('id' => 'territory_id', 'class' => 'form-control territory_id', 'required' => TRUE, 'options' => $territories, 'selected' => $existing_record['child_territory_id'] == 0 ? $existing_record['territory_id'] : $existing_record['child_territory_id'], 'disabled')); ?>
					<?php echo $this->Form->input('territory_id', array('class' => 'form-control', 'required' => TRUE, 'type' => 'hidden', 'value' => $existing_record['child_territory_id'] == 0 ? $existing_record['territory_id'] : $existing_record['child_territory_id'])); ?>
				</div>
				...
				<div class="form-group">
					<?php echo $this->Form->input('memo_no', array('class' => 'form-control', 'required' => TRUE, 'type' => 'text', 'value' => $existing_record['memo_no'], 'readonly')); ?>
				</div>


				<div class="form-group">
					<?php echo $this->Form->input('memo_reference_no', array('class' => 'form-control memo_reference_no', 'value' => $existing_record['memo_reference_no'], 'maxlength' => '15', 'required' => false, 'type' => 'text')); ?>
				</div>
					...
					<table class="table table-striped table-condensed table-bordered invoice_table">
						...
						<tbody class="product_row_box">
							...
							if (!empty($existing_record)) {
								...
								foreach ($existing_record['MemoDetail'] as $val) {
									if ($val['price'] == 0.0 AND $val['product_type_id'] == 1)
										continue;
							...
							?>
									<tr id="<?php echo $sl ?>" class="new_row_number">
										...
										<th class="text-center">
											<?php
											echo $this->Form->input('product_id', array('name' => 'data[MemoDetail][product_id][]', 'class' => 'form-control width_100 product_id chosen', 'required' => TRUE, 'options' => $product_list, 'empty' => '---- Select Product ----', 'label' => false, 'default' => $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']));
											?>
											<input type="hidden" class="product_id_clone" value="<?php echo $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']; ?>" />
											<input type="hidden" name="data[MemoDetail][product_category_id][]" class="form-control width_100 product_category_id" value="<?php echo $product_category_id_list[$val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']]; ?>" />
										...
										<th class="text-center" width="12%">
											<input type="text" name="data[MemoDetail][Price][]" class="form-control width_100 product_rate <?= 'prate-' . ($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']) ?>" value="<?= $val['price'] ?>" readonly />
										...
										<th class="text-center" width="12%">
											<input type="number" step="any" min="0" max="<?= $val['sales_qty'] + $val['stock_qty'] ?>" name="data[MemoDetail][sales_qty][]" class="form-control width_100 min_qty" value="<?= $val['sales_qty'] ?>" required />
											<input type="hidden" value="<?= $val['sales_qty'] ?>" class="prev_min_qty">
											<input type="hidden" name="data[MemoDetail][combination_id][]" class="combination_id" value="<?= $val['product_combination_id'] ?>" required />
										...
										<th class="text-center" width="12%">
											<input type="text" name="data[MemoDetail][total_price][]" class="form-control width_100 total_value <?= 'tvalue-' . ($val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']) ?>" value="<?= $total_price ?>" readonly />
										</th>

										<th class="text-center" width="12%">
											<input type="text" name="data[MemoDetail][discount_amount][]" class="form-control width_100 discount_value" value="<?= sprintf('%0.2f', $val['discount_amount'] * $val['sales_qty']) ?>" readonly />
											<input type="hidden" name="data[MemoDetail][discount_amount][]" class="form-control width_100 discount_amount" value="<?= $val['discount_amount'] ?>" readonly />
											<input type="hidden" name="data[MemoDetail][disccount_type][]" class="form-control width_100 disccount_type" value="<?= $val['discount_type'] ?>" />
											<input type="hidden" name="data[MemoDetail][policy_type][]" class="form-control width_100 policy_type" value="<?= $val['policy_type'] ?>" />
											<input type="hidden" name="data[MemoDetail][policy_id][]" class="form-control width_100 policy_id" value="<?= $val['policy_id'] ?>" />
											<input type="hidden" name="data[MemoDetail][is_bonus][]" class="form-control width_100 is_bonus" value="<?= $val['is_bonus'] ?>" />
										</th>
										...
										<th class="text-center" width="10%">
											...
											<input type="hidden" id="bonus_product_id<?php echo $sl; ?>" value="<?php echo $val['bonus_product_id']; ?>" name="data[MemoDetail][bonus_product_id][]" class="form-control width_100 bonus_product_id" />
											<input type="hidden" id="bonus_product_qty<?php echo $sl; ?>" value="<?php echo $val['bonus_qty']; ?>" name="data[MemoDetail][bonus_product_qty][]" class="form-control width_100 bonus_product_qty" />
											<input type="hidden" id="bonus_measurement_unit_id<?php echo $sl; ?>" name="data[MemoDetail][bonus_measurement_unit_id][]" value="<?php if (!empty($product_measurement_units[$val['bonus_product_id']])) echo $product_measurement_units[$val['bonus_product_id']]; ?>" class="form-control width_100 bonus_measurement_unit_id" />
										</th>
										<th class="text-center" width="10%">
						...
						<tfoot>
							<tr>
								...
								<td align="center">
									<input name="data[Memo][gross_value]" class="form-control width_100" type="text" id="gross_value" value="<?php echo  sprintf("%01.2f", $existing_record['Memo']['gross_value']); ?>" readonly />
								</td>
								<td></td>
								<td></td>
								<td></td>
							...
							<tr>
								...
								<td align="center">
									<input name="data[Memo][total_discount]" class="form-control width_100 total_discount" type="text" id="total_discount" value="<?php echo  sprintf("%01.2f", $existing_record['Memo']['total_discount']); ?>" readonly />
								</td>
								<td></td>
								<td></td>
								<td></td>
							...
							<tr>
								...
								<td align="center">
									<input name="data[Memo][net_payable]" class="form-control width_100 net_payable" type="text" id="net_payable" value="<?php echo  sprintf("%01.2f", $existing_record['Memo']['gross_value'] - $existing_record['Memo']['total_discount']); ?>" readonly />
								</td>
								<td></td>
								<td></td>
								<td></td>
							...
							<tr>
								...
								<td align="center">
									<input name="data[Memo][cash_recieved]" class="form-control width_100" type="text" id="cash_collection" value="<?php echo  sprintf("%01.2f", $existing_record['Memo']['cash_recieved']); ?>" required />
								</td>
								<td></td>
								<td></td>
								<td></td>
							...
							<tr>
												...
												<div class="modal-body">
													<table class="table-bordered">
														...
														<tbody class="bonus_product">
															<?php $bon = 0;
															foreach ($existing_record['MemoDetail'] as $val) {
																//if ($val['price'] > 0.0 || $val['is_bonus'] == 1 || $val['is_bonus'] == 3)
																	//continue;
															...
															?>
																<tr class="bonus_row">
																	<th class="text-center" id="bonus_product_list">
																		<?= $val['is_bonus'] ?>
																		<?php
																		echo $this->Form->input('product_id', array('name' => 'data[MemoDetail][product_id][]', 'class' => 'form-control width_100 open_bonus_product_id', 'empty' => '---- Select Product ----', 'options' => $open_bonus_product_option, 'label' => false, 'default' => $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id']));
																		?>
																	...
																	<th class="text-center" width="12%">
																		<input type="number" min="0" name="data[MemoDetail][sales_qty][]" value="<?= $val['sales_qty'] ?>" step="any" class="form-control width_100 open_bonus_min_qty" />
															...
															<?php if ($bon == 0) { ?>
																<tr class="bonus_row">
																	<th class="text-center" id="bonus_product_list">
																		<?php
																		echo $this->Form->input('product_id', array('name' => 'data[MemoDetail][product_id][]', 'class' => 'form-control width_100 open_bonus_product_id', 'empty' => '---- Select Product ----', 'options' => $open_bonus_product_option, 'label' => false));
																		?>
																		<input type="hidden" class="product_id_clone" />
																		<input type="hidden" name="data[MemoDetail][product_category_id][]" class="form-control width_100 open_bonus_product_category_id" />
																	</th>
								...
								<td align="center">
									<input name="data[Memo][credit_amount]" class="form-control width_100" type="text" id="credit_amount" value="<?php echo sprintf("%01.2f", $existing_record['Memo']['credit_amount']); ?>" readonly />
								</td>
								<td></td>
								<td></td>
								<td></td>
...
<div id="memo_product_list">
	<?php
	echo $this->Form->input('product_id', array('name' => 'data[MemoDetail][product_id][]', 'class' => 'form-control width_100 product_id', 'options' => $product_list, 'empty' => '---- Select Product ----', 'label' => false, 'required' => TRUE));
	?>
	<input type="hidden" class="product_id_clone" />
</div>
...
<script>
	...
	$("body").on("keyup", ".min_qty", function() {
		...
		var product_price_id = product_box.find("th:nth-child(4) .product_price_id");
		var product_qty = product_box.find("th:nth-child(5) .min_qty");
		var prev_min_qty = product_box.find("th:nth-child(5) .prev_min_qty");
		var total_val = product_box.find("th:nth-child(6) .total_value");
...
<script>
	$(document).ready(function() {
		...
		$("#MemoAdminEditForm").submit(function(e) {
			e.preventDefault();

			var btn = $("input[type=submit][clicked=true]").attr('name');
			stock_check(function(check) {
				if (check) {
					$("#MemoAdminEditForm").append("<input type='hidden' name='" + btn + "' value='" + btn + "' />");
		...
		function stock_check(callback) {
			...
			var memo_id = <?= $existing_record['Memo']['id'] ?>;
			$.ajax({
	...
	$(document).ready(function() {
		...
		$(".min_qty").keyup();
		//$('.bonus_policy_id_'+2064+' .policy_bonus_product_id').val();

		//policyProductSelected();

	});

	function policyProductSelected() {
		//set_product_policy_
		<?php foreach ($existing_record['MemoDetail'] as $val) { ?>
			<?php if ($val['policy_id']) { ?>
				$('.bonus_policy_id_<?= $val['policy_id'] ?> .policy_bonus_product_id').find('option[value=<?= $val['virtual_product_id'] ? $val['virtual_product_id'] : $val['product_id'] ?>]').attr('selected', 'selected'); /*===*/

				$('.bonus_policy_id_<?= $val['policy_id'] ?> .policy_bonus_product_id').change();
			<?php } ?>
		<?php } ?>
	}
