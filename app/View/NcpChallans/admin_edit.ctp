    ...
    <div class="col-md-12">
		<div class="box box-primary">
			...
			<div class="form-group">
				<?php echo $this->Form->input('receiver_store_id', array('id'=>'receiver_store_id', 'class' => 'form-control','options'=>$receiver_store,'selected'=>$ncp_challan_draft_info[0]['Challan']['receiver_store_id'],'required'=>true)); ?>
			</div>
			<div class="form-group">
			...
			<table class="table table-striped table-bordered">
				...
				<tbody>
					<tr>
						<td>
							<?php echo $this->Form->input('product_id', array('label'=>false, 'id'=>'product_id', 'class' => 'full_width form-control product_id','empty'=>'---- Select Product ----', 'options'=>$product_list)); ?>
						</td>
				...
				<tbody>
				    ...
				    foreach ($ncp_challan_draft_info[0]['ChallanDetail'] as $value) {
				        ...
				        <tr class="table_row" id="rowCount<?php echo $row_count_for_remove;?>">
				            ...
				            <td align="center">
				                <input name="quantity[<?php echo $row_count_for_remove;?>]" class="p_quantity" value="<?php echo $value['challan_qty']; ?>" required >
				            </td>
				            <td align="center">
...
<script>		
	$(document).ready(function(){			
		...
		$(".add_more").click(function() {	
            ...
            var stock_check = $.inArray(product_check_id,selected_stock_array) != -1;1;
			...
			//else if(is_maintain_expire_date == 1 && expire_date == '' && isDate(expire_date))
			...
			else
			{
				...
				$.ajax({
					...
					success: function(response){						
						var obj = jQuery.parseJSON(response);						
						var recRow = '<tr class="table_row" id="rowCount'+rowCount+'"><td align="center">'+rowCount+'</td><td>'+obj.Product.name+'<input type="hidden" name="product_check[]" class="selected_product_id" value="'+obj.Product.id+batch_no+expire_date+'"/><input type="hidden" name="product_id[]" value="'+obj.Product.id+'"/></td><td align="center"><input type="hidden" name="batch_no[]" value="'+batch_no+'">'+batch_no+'</td><td align="center">'+obj.ChallanMeasurementUnit.name+'<input type="hidden" name="measurement_unit[]" value="'+obj.Product.challan_measurement_unit_id+'"></td><td align="center"><input type="text" name="quantity[]" class="p_quantity" value="'+quantity+'"></td><td align="center"><input type="hidden" name="expire_date[]" class="p_expire_date" value="'+expire_date+'">'+expire_date+'</td><td align="center"><input type="text" class="full_width form-control" name="remarks[]"></td><td align="center"><button class="btn btn-danger btn-xs remove" value="'+rowCount+'"><i class="fa fa-times"></i></button></td></tr>'; 
