		...
		<div class="box box-primary">
			...
			<?php echo $this->Form->create('DistOpenCombination', array('role' => 'form')); ?>
                ...
                <span class="input_products_wrap">
					<?php
                    $i = 0;
                    $total = count($this->request->data['DistOpenCombinationProduct']);
                    foreach($this->request->data['DistOpenCombinationProduct'] as $key=> $val){
                    $i++;
                    //pr($products);
                    ?>
                    <div class="form-group">
                        ...
                        <select class="form-control" id="product_id<?=$i?>" name="data[DistOpenCombination][product_id][<?=$this->request->data['DistOpenCombinationProduct'][$key]['id'];?>]" required>
                            <?php foreach($products as $okey=>$oval){ ?>
                                <option value="<?=$okey;?>" <?php if($this->request->data['DistOpenCombinationProduct'][$key]['product_id']==$okey){ echo 'selected'; } ?>><?=$oval;?></option>
                            <?php } ?>
                        </select>
