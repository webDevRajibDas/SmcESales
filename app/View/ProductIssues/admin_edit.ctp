        ...
        <div class="box box-primary" style="float:left; width:100%; position:relative; padding-bottom:10px;">
			...
			<?php echo $this->Form->create('Challan', array('role' => 'form')); ?>
                <div class="form-group">
                            <?php echo $this->Form->input('receiver_store_id', array('id'=>'receiver_store_id','class' => 'form-control','options'=>$receiverStore,'required'=>true,'default'=>$challan_info['Challan']['receiver_store_id'], 'disabled')); ?>
                            <?php echo $this->Form->input('receiver_store_id', array('class' => 'form-control', 'required' => TRUE, 'type' => 'hidden', 'value' => $challan_info['Challan']['receiver_store_id'])); ?>
                </div>
                <div class="form-group">
                            <?php echo $this->Form->input('challan_date', array('label'=>'Product Issue Date','type'=>'text','class' => 'form-control challan-datepicker','required'=>true, 'value'=>$challan_info['Challan']['challan_date'], 'readonly' => true)); ?><span style="color:red">*</span>
                </div>
                <div class="form-group">
                            <?php echo $this->Form->input('remarks', array('class' => 'form-control', 'value'=>$challan_info['Challan']['remarks'])); ?>
                </div>
                <div class="form-group">
				<div class="form-group">
                <?php echo $this->Form->input('carried_by', array('class' => 'form-control','value'=>$challan_info['Challan']['carried_by'])); ?>
                </div>
                <div class="form-group">
                    <?php echo $this->Form->input('truck_no', array('class' => 'form-control','value'=>$challan_info['Challan']['truck_no'])); ?>
                </div>
                 <div class="form-group">
                            <?php echo $this->Form->input('driver_name', array('class' => 'form-control','value'=>$challan_info['Challan']['driver_name'])); ?>
                </div>
                ...
                <table class="table table-striped table-condensed table-bordered invoice_table">
                    ...
                    <?php
                        ...
                        foreach ($challan_info['ChallanDetail'] as $value) {
                            ...
                            if( $value['virtual_product_name_show'] == 1 ){
                                $value['Product']['name'] = $value['VirtualProduct']['name'];
                            }
                           // echo '<pre>aa';print_r($value);exit;
                    ?>
                    ...
                    <tr class="table_row" id="rowCount<?php echo $row_count_for_remove;?>">
                        ...
                        <td align="center">
                            ...
                            <?php if($value['expire_date']){echo $this->App->expire_dateformat($value['expire_date']);}else echo ''; ?>
                        </td>
                        <td align="center">
...
/*Challan Datepicker : End*/
    $(document).ready(function () {
        ...
        $('#receiver_store_id').trigger('change');
           ...
           var product_qty = $('.qty').val();
            ...
            } else
            {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        var obj = jQuery.parseJSON(response);
                        var recRow = '<tr class="table_row" id="rowCount' + rowCount + '"><td align="center">' + rowCount + '</td><td>   <input type="hidden" name="selected_product_id[' + rowCount + ']" class="selected_product_id" value="' + obj.Product.id + batch_no + expire_date + '"/>' + pname + '<input type="hidden" name="product_id[' + rowCount + ']" value="' + obj.Product.id + '"/></td><td align="center"><input type="hidden" name="batch_no[' + rowCount + ']" value="' + batch_no + '">' + batch_no + '</td><td align="center"><input type="hidden" name="expire_date[' + rowCount + ']" value="' + expire_date + '">' + display_exp_date + '</td><td align="center">' + obj.ChallanMeasurementUnit.name + '<input type="hidden" name="measurement_unit[' + rowCount + ']" value="' + obj.Product.challan_measurement_unit_id + '"></td><td align="center" id="'+rowCount+'"><input type="number" step="any" min="0" name="quantity[' + rowCount + ']" id="product_quantity'+rowCount+'" class="' + product_id + '_product_qty p_quantity product_quantity" value="' + quantity + '" required ></td><td align="center" id="stock_qty'+rowCount+'" class="stock_qty">'+product_qty+' <input type="hidden" name="product_qty[' + rowCount + ']" class="product_qty" value="' + product_qty + '"/> </td><td  align="center"><input type="text" name="source[' + rowCount + ']" value="'+source+'" readonly></td><td align="center"><input type="text" class="full_width form-control" name="remarks[' + rowCount + ']"></td><td align="center"><button class="btn btn-danger btn-xs remove" value="' + rowCount + '"><i class="fa fa-times"></i></button></td></tr>';
