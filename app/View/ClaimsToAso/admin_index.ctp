    ...
    <div class="col-xs-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-th-large"></i> <?php echo __('Claim List (SO to ASO)'); ?></h3>
			...
			<div class="box-body">
                <div class="search-box">
					...
					<table class="search">
						<tr>
							...
							<td width="50%">
								<?php echo $this->Form->input('transaction_type_id_1', array('type'=>'hidden','class' => 'form-control','required'=>false,'default'=>24)); ?>
								<?php echo $this->Form->input('transaction_type_id_2', array('type'=>'hidden','class' => 'form-control','required'=>false,'default'=>25)); ?>
				...
				<table id="ReturnChallan" class="table table-bordered table-striped">
					...
					<tr>
						...
						<td align="center"><?php if($claim['Claim']['status'] == 1 AND $claim['Claim']['isApproved'] == 1){echo '<span class="btn btn-warning btn-xs">pending </span>';} 
							else if($claim['Claim']['status'] == 1 AND $claim['Claim']['isApproved'] != 1){echo '<span class="btn btn-danger btn-xs">Not Approved</span>';}
							 else{echo '<span class="btn btn-success btn-xs">Received</span>';} ?></td>
						<td align="center"><?php echo h($claim['Claim']['remarks']); ?></td>
						<td class="text-center">
							<?php if($this->App->menu_permission('claims_to_aso','admin_view')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-eye-open"></i>'), array('action' => 'view', $claim['Claim']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'view')); } ?>
							<?php if($this->App->menu_permission('claims_to_aso','admin_view_for_approver')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-share"></i>'), array('action' => 'view_for_approver', $claim['Claim']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'view for approver')); } ?>
						</td>
					</tr>
					<?php endforeach; ?>
					</tbody>
