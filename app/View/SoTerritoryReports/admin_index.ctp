          ...
          <?php if($this->App->menu_permission('nationalSalesReports', 'admin_add')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-plus"></i> New ProjectionAchievement Setting'), array('action' => 'add'), array('class' => 'btn btn-primary', 'escape' => false)); } ?>
            ...
            <div class="box-body">
                ...
                <div class="search-box">
                    <?php echo $this->Form->create('SoTerritoryReports', array('role' => 'form', 'action' => 'index')); ?>
                    <table class="search">
                        <tr>
                            ...
                            <div id="xls_body">
                                ...
                                <div style="float:left; width:100%; height:430px; overflow:scroll;">
                                    ...
                                    <table cellspacing="0" cellpadding="10px" border="1px solid black" align="center" class="text-center report_table" id="sum_table">
                                        <thead>
                                            <tr class="titlerow">
                                                <th style="text-align:left;">Thana Name</th>
                                                <th style="text-align:left;">Market Name</th>

                                                <?php foreach ($outlet_categories as $outlet_categorie) { ?>
                                                    <th colspan="2"><?= $outlet_categorie ?></th>
                                                <?php } ?>
                                        ...
                                        <tbody>
                                            <tr>
                                                <td></td>
                                                <td></td>
                                                <?php foreach ($outlet_categories as $outlet_category_id => $outlet_categorie) { ?>
                                                    <td><b>Visited</b></td>
                                                    <td><b>Non-Visited</b></td>
                                                <?php } ?>
                                            </tr>
                                            ...
                                            <?php if ($results) { ?>
                                                ...
                                                <?php /*?><?php foreach($results as $thana_name => $market_datas){ ?>
												...
												?>
                                                <tr>
                                                <td><b><?=$i==1?$thana_name:''?></b></td>
                                                <td><?=$market_name?></td>
                                                <?php foreach($outlet_categories as $outlet_category_id => $outlet_categorie){ ?>
                                                <td style="text-align:right;"><?=@$outlet_category_datas[$outlet_category_id]['total_visited']?></td>
                                                <td style="text-align:right;"><?=@$outlet_category_datas[$outlet_category_id]['total_non_visited']?></td>
                                                <?php 
												...
												?>
                                                <td style="text-align:right;"><?=@$thana_sub_total_visited[$outlet_category_id]?></td>
                                                <td style="text-align:right;"><?=@$thana_sub_total_non_visited[$outlet_category_id]?></td>
                                                <?php } ?>
                                                </tr>
                                            ...
                                            <?php } else { ?>
                                                <tr>
                                                    <td colspan="<?= count($outlet_categorie) + 2 ?>">No Result Found!</td>
                                                </tr>
                                            <?php } ?>
...
<script>
    $("#download_xl").click(function(e) {
        ...
        a.href = downloadUrl;
        a.download = "so_territory_report.xls";
