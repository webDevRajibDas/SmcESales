...
<div class="row">
    <div class="col-md-12">
		<div class="box box-primary">
			...
			<?php echo $this->Form->create('Requisition', array('role' => 'form')); ?>
				<div class="form-group">
					<?php echo $this->Form->input('title', array('class' => 'form-control','value'=>$requisition_info['Requisition']['title'])); ?>
				</div>
				<div class="form-group">
					<?php echo $this->Form->input('sender_store_id', array('class' => 'form-control','empty'=>'---- Select Sender ----','id'=>'sender_store_id','disabled','selected'=>$requisition_info['Requisition']['sender_store_id'])); ?>
				</div>
				<div class="form-group">
					<?php echo $this->Form->input('receiver_store_id', array('class' => 'form-control','empty'=>'---- Select Receiver ----','id'=>'receiver_store_id')); ?>
				</div>
				<div class="form-group">
					<?php echo $this->Form->input('remarks', array('type' => 'text','class' => 'form-control','value'=>$requisition_info['Requisition']['remarks'])); ?>
				</div>
				...
				<table class="table table-striped table-condensed table-bordered invoice_table">
					...
					<tbody>
						...
						foreach($requisition_details as $req_data){
							
							if($req_data['RequisitionDetail']['virtual_product_name_show'] > 0){
								$req_data['RequisitionDetail']['product_id'] = $req_data['RequisitionDetail']['virtual_product_id'];
								$req_data['Product']['name'] = $req_data['VirtualProduct']['name'];
							}

							?>
							<tr class="table_row" id="rowCount<?=$row_count_for_remove?>">
								<td align="center"><?=$row_count_for_remove;?></td>
								<td>
									<input type="hidden" name="selected_product_id[]" class="selected_product_id" value="<?=$req_data['RequisitionDetail']['product_id'];?>"/>
									<?=$req_data['Product']['name'];?>
									<input type="hidden" name="product_id[]" value="<?=$req_data['RequisitionDetail']['product_id'];?>"/>
								</td>
								<td align="center"><?=$req_data['measurement_unit']['name'];?>
									<input type="hidden" name="measurement_unit<?=$req_data['RequisitionDetail']['product_id']?>" value="<?=$req_data['RequisitionDetail']['measurement_unit_id']?>">
								</td>
								<td align="center">
									<input type="hidden" name="quantity<?=$req_data['RequisitionDetail']['product_id']?>" class="p_quantity" value="<?=$req_data['RequisitionDetail']['qty']?>"><?=$req_data['RequisitionDetail']['qty']?>
								</td>
								<td align="center">
									<button class="btn btn-danger btn-xs remove" value="<?=$row_count_for_remove++;?>"><i class="fa fa-times"></i></button>
								</td>
							</tr>
							<?php }?>
						</tbody>
...
*/						$('.qty').val('');					
		...
		$(".add_more").click(function() {	
			...
			else
			{
				...
				$.ajax({
					...
					success: function(response){						
						var obj = jQuery.parseJSON(response);
						var recRow = '<tr class="table_row" id="rowCount'+rowCount+'"><td align="center">'+rowCount+'</td><td><input type="hidden" name="selected_product_id[]" class="selected_product_id" value="'+obj.Product.id+/*batch_no+expire_date+*/'"/>'+obj.Product.name+'<input type="hidden" name="product_id[]" value="'+obj.Product.id+'"/></td><td align="center">'+obj.ChallanMeasurementUnit.name+'<input type="hidden" name="measurement_unit'+obj.Product.id+'" value="'+obj.Product.challan_measurement_unit_id+'"></td><td align="center"><input type="hidden" name="quantity'+obj.Product.id+'" class="p_quantity" value="'+quantity+'">'+quantity+'</td><td align="center"><button class="btn btn-danger btn-xs remove" value="'+rowCount+'"><i class="fa fa-times"></i></button></td></tr>'; 
		...
		function get_receiver_store(){
			$.ajax({
	    		url: '<?= BASE_URL.'requisitions/get_receiver_store_with_sender_store_product'?>',
				type: 'post',
				data:{'sender_store_id':$('#sender_store_id').val()},
				success:function(data){
					var output="<option value=''>--- Select Receiver ---</option>";
					var json_decode=$.parseJSON(data);
					for (i in json_decode.receiver_store) {
						output+="<option value='"+i+"'>"+json_decode.receiver_store[i]+"</option>";
					}
					$("#receiver_store_id").html(output);
					$("#receiver_store_id").val(<?php echo $requisition_info['Requisition']['receiver_store_id']?>);
					output="<option value=''>--- Select Product ---</option>";
					for (i in json_decode.products) {
						output+="<option value='"+i+"'>"+json_decode.products[i]+"</option>";
					}
					$("#product_id").html(output).trigger("chosen:updated");
				}
	    	})
		}
		
	});
//product_batch_expiredae==<td align="center"><input type="hidden" name="batch_no'+obj.Product.id+'" value="'+batch_no+'">'+batch_no+'</td>*/<td align="center"><input type="hidden" name="expire_date'+obj.Product.id+'" value="'+expire_date+'">'+expire_date+'</td>
</script>