        ...
        <div class="box box-primary">
            <div class="box-header">
                ...
                <div class="box-tools pull-right">
                    <?php if($this->App->menu_permission('MemoEditablePermissions','admin_index')){ echo $this->Html->link(__('Memo Editable Approval List'), array('action' => 'index'), array('class' => 'btn btn-primary', 'escape' => false)); } ?>
                </div>
            </div> 
                    ...
                    <tbody id = "list_table">
                        <?php if(!empty($permited)){
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-info btn-xs toggleBtn add_more_<?=$id?>" onclick="memo_approved(<?=$id?>);"><i class="glyphicon glyphicon-plus"></i></a>
                                    <a class="btn btn-danger btn-xs toggleBtn remove_more_<?=$id?>" onclick="memo_not_approved(<?=$id?>);"><i class="glyphicon glyphicon-minus"></i></a>
                                </td>
                            </tr>
                        <?php }
                         }
...
<script>
    $('body').on('click','.find',function() {
        ...
        ({
            type: "POST",
            url: '<?=BASE_URL?>memo_editable_permissions/get_memo_approved_list',
            ...
            {          
                ...
                for(var i = 0; i<obj.length; i++){
                    ...
                    var recRow = '<tr id ="table_row_'+memo_id+'"><td id = "memo_id_'+memo_id+'" class= "memo_id" value = "'+memo_no+'"><a href="<?=BASE_URL?>admin/MemoEditablePermissions/view/'+memo_id+'" target="_blank">'+memo_no+'</a></td><td class ="office_name" value ="'+office_name+'">'+office_name+'</td><td class ="market_name" value ="'+market_name+'">'+market_name+'</td><td class ="outlet_name" value ="'+outlet_name+'">'+outlet_name+'</td><td class ="thana_name" value ="">'+thana_name+'</td><td class ="territory_name" value ="'+territory_name+'">'+territory_name+'</td><td class ="remarks" value ="'+remarks+'">'+remarks+'</td><td class ="recommender_note" value ="'+recommender_note+'">'+recommender_note+'</td><td class ="recommender_note" ><input type ="text" class="form-control rcm_note  approval_note" name="data[MemoEditablePermissions][approval_note]" id="approval_note_'+id+'"></td><td><a class="btn btn-info btn-xs toggleBtn add_more_<?=$id?>" onclick="memo_recommended('+id+');"><i class="glyphicon glyphicon-plus"></i></a><a class="btn btn-danger btn-xs toggleBtn remove_more_'+id+'" onclick="memo_not_approved('+id+');"><i class="glyphicon glyphicon-minus"></i></a></td></tr>';
    ...
    function memo_approved(id){
        var approval_note = $("#approval_note_"+id).val();
        $.ajax
        ({
            type: "POST",
            url: '<?=BASE_URL?>memo_editable_permissions/memo_approved',
    ...
    function memo_not_approved(id){
        ...
        if(approval_note == '')
        {
            ...
            ({
                type: "POST",
                url: '<?=BASE_URL?>memo_editable_permissions/memo_not_approved',
                data: {id:id,approval_note:approval_note},
                cache: false, 
                success: function(response)
                {          
