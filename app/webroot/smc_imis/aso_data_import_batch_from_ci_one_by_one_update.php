<?php	
	...
	// $Reader = new SpreadsheetReader('upload/aso/Aso_Aug_Barisal.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_bogra.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/Aso_Aug_Chittagong.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/Aug_Comilla_ASO.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_dhaka_east.xlsx');
	 $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_dhaka_west.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/Aug_Khulna_ASO.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_kustia.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_mymensingh.xlsx'); // later
	// $Reader = new SpreadsheetReader('upload/aso/Aso_Aug_Rajshahi.xlsx'); // later
	// $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_rangpur.xlsx');
	// $Reader = new SpreadsheetReader('upload/aso/aso_ims_data_sylhet.xlsx');
	/*echo '<pre>';
	print_r($Reader);exit;*/
	$i=0;
	$full_sql="";
	   ...
	   if($maintain_batch || $is_maintain_expire_date)
	   {
		   ...
		   if(!empty($batch_list_info))
		   {
				...
				foreach($batch_list_info as $each_batch_list)
				{
					...
					/*$inner_sql="insert into current_inventories (store_id,inventory_status_id,product_id,batch_number,expire_date,qty,updated_at,transaction_date,transaction_type_id) 
					 values ($store_id,1,$product_id,'$batch_no','$expire_date',$batch_qty,'$updated_at','$transaction_date',4)";*/
					 $inner_sql=" UPDATE current_inventories set qty=$batch_qty,updated_at='$updated_at',transaction_date='$transaction_date',transaction_type_id=4 where product_id=$product_id and store_id=$store_id and batch_number='$batch_no' and expire_date='$expire_date' and inventory_status_id=1";
		   ...
		   else 
		   {
			  /*$inner_sql="insert into current_inventories (store_id,inventory_status_id,product_id,batch_number,expire_date,qty,updated_at,transaction_date,transaction_type_id) 
				...
				if($check_previous_stock)
	   				{
						 $inner_sql="UPDATE current_inventories set qty=$qty,updated_at='$updated_at',transaction_date='$transaction_date',transaction_type_id=4 where product_id=$product_id and store_id=$store_id  and inventory_status_id=1";
	...
	{
		$sql = "select p.id as product_id,p.maintain_batch as maintain_batch,p.is_maintain_expire_date as is_maintain_expire_date, pm.qty_in_base as product_base_qty from products p left join product_measurements pm on p.id=pm.product_id and p.sales_measurement_unit_id=pm.measurement_unit_id where LTRIM(p.name) = '".trim($product_name)."'";
