 ...
 */
	...
	public function filter($params, $conditions) {
		...
		if (!empty($params['Store.store_type_id'])) {
            $conditions[] = array('Store.store_type_id' => $params['Store.store_type_id']);
        }
		if (!empty($params['Store.office_id'])) {
            $conditions[] = array('Store.office_id' => $params['Store.office_id']);
        }				
        return $conditions;
    }
	...
	{		
		if($this->data[$this->name]['store_type_id']==1 OR $this->data[$this->name]['store_type_id']==2)
		{
			...
			$unique['store_type_id'] = $this->data[$this->name]['store_type_id'];
			$unique['office_id'] = $this->data[$this->name]['office_id'];
			$unique['territory_id'] = $this->data[$this->name]['territory_id'];
		}		
	...
	{		
		if($this->data[$this->name]['store_type_id']==3 AND $this->data[$this->name]['territory_id']=='')
		{
			return false;
		}else{
			return true;
