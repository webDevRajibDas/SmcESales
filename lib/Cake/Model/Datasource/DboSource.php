...
class DboSource extends DataSource
{
	 ...
	 * @var int
	 */
	protected $_queriesLogMax = 99999;
	...
	public function value($data, $column = null)
	{
		...
		switch ($column) {
			...
			default:
				...
				if ((is_int($data) || $data === '0') || (is_numeric($data) && strpos($data, ',') === false &&
	...
	public function name($data)
	{
		...
		if (
			preg_match(
				'/^([\w-]+(\.[\w-]+|\(.*\))*)\s+' . preg_quote($this->alias) . '\s*([\w-]+)$/i',
				$data,
				$matches
			)
		) {
